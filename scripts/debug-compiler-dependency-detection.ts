...
async function debugDependencyDetection() {
  console.log('='.repeat(80))
  console.log('DEBUG: IR Config → Params → Dependency Detection')
  console.log('='.repeat(80))
  console.log()
  ...
  console.log('STEP 1: IR destinations with config')
  console.log('='.repeat(80))
  const destinations = irResult.ir.delivery_rules?.multiple_destinations || []
  destinations.forEach((dest, idx) => {
    console.log(`\nDestination ${idx + 1}: ${dest.operation_type}`)
    console.log('Config from IR:')
    console.log(JSON.stringify(dest.config, null, 2))
  })

  console.log('\n\n' + '='.repeat(80))
  console.log('STEP 2: What buildDeliveryParams SHOULD create')
  console.log('='.repeat(80))
  console.log()
  console.log('The compiler\'s buildDeliveryParams at line 3744 does:')
  console.log('  if (deliveryRule.config && paramName in deliveryRule.config) {')
  console.log('    params[paramName] = configValue  // ← Copies {{step_result.*}}')
  console.log('  }')
  console.log()
  console.log('So params SHOULD have the same {{step_result.*}} references')
}

debugDependencyDetection().catch(console.error)
