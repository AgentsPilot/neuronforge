...
async function main() {
  ...
  // EXPERIMENT: Use raw narrative prompt directly
  // Convert config to user context
  if (enhancedPrompt.config) {
    vocabulary.userContext = Object.entries(enhancedPrompt.config).map(([key, value]) => ({
      key,
      value: String(value)
    }))
    console.log(`   User context: ${vocabulary.userContext.length} configuration values`)
  }

  const intentGenStart = Date.now()

  // Generate with raw prompt text - import the system prompt builder
  const { buildIntentSystemPromptV2 } = await import('@/lib/agentkit/v6/intent/intent-system-prompt-v2')
  const Anthropic = (await import('@anthropic-ai/sdk')).default
  const client = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY })

  const systemPrompt = buildIntentSystemPromptV2(vocabulary)
  const userPrompt = enhancedPrompt.prompt +
    (vocabulary.userContext ? '\n\n## Workflow Configuration\n\n' +
      vocabulary.userContext.map((c: any) => `- ${c.key}: ${c.value}`).join('\n') : '')

  console.log('   Calling LLM with narrative prompt...')
  ...
  const intentContract = JSON.parse(jsonText)
  ...
  // Hardcoded workflow config for narrative prompt experiment
