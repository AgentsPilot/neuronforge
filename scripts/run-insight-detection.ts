 ...
 *
 * Script to test the insight detection system with correlation.
 * Usage: npx tsx scripts/run-insight-detection.ts [userId]
 */
...
if (!supabaseUrl || !supabaseServiceKey) {
  ...
  console.error('Make sure .env.local exists and has these variables set');
  process.exit(1);
}
...
async function main() {
  ...
  if (!userId) {
    // Get the first active user with recent activity
  ...
  try {
    ...
    if (summary.standaloneInsights.length > 0) {
      ...
      for (const result of summary.standaloneInsights) {
        ...
        if (result.processParameters) {
          console.log(`   Details: ${JSON.stringify(result.processParameters).substring(0, 200)}...`);
        }
      }
    } else {
    ...
    if (totalInsights === 0) {
      ...
      console.log('     - The user has no data in the system');
