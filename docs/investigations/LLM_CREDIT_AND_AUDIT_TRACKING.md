...
**Status:** Draft — BA investigation. Line numbers were accurate on 2026-09-16 (branch `feature/business-os-purge-slice-2`). Findings refined by SA code checks are recorded in the Layer 1 requirement's SA Review.
...
| 2. Credit cost tracked | Partly. Every call through the provider layer writes a `token_usage` row with tokens and `cost_usd`. Business OS credits are only derived when the usage screen is read, and nothing is deducted. Only agent runs deduct credits. |
...
- Feature-to-category mapping is at `:49-69`.
...
|---|---|---|---|---|---|
| 1 | Chat v4 planner `lib/business-os/bizql/planner/Planner.ts:443` | DB | Real (`request.userId`) | business-os-chat / BizQLPlanner (+turnId) | Chat |
...
| 4 | Verified questions embedding `bizql/planner/VerifiedQuestions.ts:81` | DB (shared) | Real | business-os-chat / EmbeddingService | Chat |
...
**Found at SA review, not in the original 18:** the onboarding conversation, `lib/services/OnboardingConversationManager.ts:950`, `:995`, `:1097`, `:1397` (4 calls per session, via `complete()`, recorded as `system` / `onboarding`). **Excluded from Layer 1 and moved to Layer 1.5** (E.8).
...
- "Sync pricing" upserts a hardcoded catalog (`pricing/sync/route.ts:31-375`) with no embedding models.
...
- **Gaps:**
  ...
  - some calls recorded under the system user or not recorded at all;
...
1. Correct attribution (Layer 1; onboarding conversation Layer 1.5; V6 separate).
...
- **Layer 1:** every in-scope Business OS AI call is recorded against the right business, with area, call name and UUID grouping id, through a new call catalog and attribution builder. The usage category mapping is updated in the same release. Proof is QA test evidence and code review.
- **Layer 1.5:** (a) the extended admin data report (I.5); (b) onboarding conversation attribution (E.8.1).
...
| `console.*` logging in touched or admin files | `lib/ai/providerFactory.ts` (8), `lib/services/EmbeddingService.ts` (16): flagged for Pino conversion in the Layer 1 workplan. Also `token-usage/stats`, `users/[id]/stats`, `system-config`, `system-config/pricing` routes | Layer 1 workplan flags the first two; others convert when touched |
...
- [x] **Q8 — Legacy and broken calls** (raised by: BA | status: **resolved 2026-09-16**) Excluded from Layer 1 with reasons.
