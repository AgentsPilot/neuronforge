...
4. Per-call model configuration (Layer 2, delivered — eight area rows, resolved at run time, with a per-area kill switch; the admin screen is still later).
...
| **2** | JSON model configuration per area, keyed by the Layer 1 call names — including the onboarding conversation's `'gpt-4o'` literals (Layer 1.5 KI-C / F-9, **closed**). Eight `system_settings_config` rows (one per area) carry `provider`, `model`, `temperature`, `enabled` and per-call overrides; all 22 catalogued calls resolve them at run time and pick a change up within ~60 s, with a per-area kill switch and a one-shot retry on the code default if a configured model is refused. Guardrails refuse an unpriced model, so configuration cannot silently zero cost tracking. Requirement: [BUSINESS_OS_LLM_MODEL_SETTINGS_LAYER2_REQUIREMENT.md](/docs/requirements/BUSINESS_OS_LLM_MODEL_SETTINGS_LAYER2_REQUIREMENT.md); operator runbook: [BUSINESS_OS_LLM_MODEL_SETTINGS_RUNBOOK.md](/docs/runbooks/BUSINESS_OS_LLM_MODEL_SETTINGS_RUNBOOK.md). **Not included:** the admin screen (later), and chat-v2 / chat-v1, which still choose their own model and are stopped by the chat switch instead | Steps 0–3 merged (PRs #61, #79, #85, #87); Step 4 (the `check:bos-llm-literals` CI gate + docs) in review |
...
| Onboarding conversation hardcodes `model: 'gpt-4o'` | `OnboardingConversationManager.ts:951`, `:996`, `:1098`, `:1398` (CLAUDE.md rule 5) | Carried unchanged by Layer 1.5 (KI-C) and handed to Layer 2's per-area model configuration (F-9). ✅ **Fixed 2026-09-21:** all four resolve the `onboarding` area row; `npm run check:bos-llm-literals` fails CI if a literal returns |
...
| 2026-09-21 | Layer 2 delivered — mirrored | H.3 Layer 2 row rewritten as built (eight `system_settings_config` area rows; all 22 catalogued calls resolve provider, model, temperature and on/off at run time, ~60 s propagation, per-area kill switch, one-shot retry on the code default, guardrails that refuse an unpriced model) and links the requirement and the new operator runbook; steps 0-3 merged (PRs #61, #79, #85, #87), step 4 in review. H.2 item 4 marked delivered. The `model: 'gpt-4o'` row in the findings table is marked fixed: Layer 1.5 **KI-C is closed**, and `npm run check:bos-llm-literals` (a second step in the existing `bos-llm-typecheck` job) now fails CI on a model or temperature literal at any non-test file that imports the call catalog. Still not delivered: the admin screen, and chat-v2 / chat-v1's own model choice (stopped by the chat switch, not resolved) |
