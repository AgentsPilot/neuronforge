...
| 2026-04-08 | D-B17 documented | **ai_processing steps ignore orchestration routing decision.** Orchestration initializes and routes ALL steps (IntentClassifier + RoutingService — costs tokens), selecting e.g. `gpt-5.2` for balanced tier. But `callLLMDirect` (WP-3) skips orchestration at execution time and uses `agent.model_preference` → hardcoded `gpt-4o` default — completely ignoring the routing decision. Fix needed: `callLLMDirect` should read the routing decision from `context.orchestrator` metadata for the step, or the orchestration should skip classifying/routing `ai_processing` steps since they won't use orchestration. Saves wasted IntentClassifier tokens and ensures consistent model selection. |
