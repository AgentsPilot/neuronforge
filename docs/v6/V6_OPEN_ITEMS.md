...
| [WP-56](./V6_WORKFLOW_DATA_SCHEMA_WORKPLAN_EXECUTION_WEAK_POINTS.md#wp-56-phase-1-references-the-wrong-field-name-on-a-scatterloop-iteration-variable-container-id-reused-for-items) | Phase 1 references a container's id field (`folder_id`) on a scatter/loop iteration variable whose items use a different name (`id`) → `undefined` at runtime → Docs API 400 (same incident agent as WP-49/53/54/55) | 🟡 Partial — prompt steering done | Root-cause **FIELD FIDELITY (WP-56)** rule added to § 6.9 LOOP in `intent-system-prompt-v2.ts` (2026-06-08). Still open: (i) deterministic reconciliation for iteration-variable field refs — WP-2 doesn't cover scatter/loop item-refs; calibration-side detector tracked as **P3** in `docs/workplans/CALIBRATION_FALSE_SUCCESS_FIX_WORKPLAN.md`; (ii) pin the `id`→`folder_id` data_schema mutation (now recoverable via WP-55 persistence). **In progress (2026-07-11):** Phase 0 (shared reconciliation core) + Phase 1 (calibration detect/correct/verdict) delivered pending SA/QA — see `docs/workplans/V6_FIELD_FIDELITY_CALIBRATION_PHASE0_PHASE1_WORKPLAN.md`; Phases 2–3 (runtime clamp + generation/compiler reconcile) still open. |
| [WP-63](./V6_WORKFLOW_DATA_SCHEMA_WORKPLAN_EXECUTION_WEAK_POINTS.md#wp-63-flatten-field-shape-fidelity--declared-snake_case-item-schema-vs-plugins-native-camelcase) | `flatten` declares snake_case item fields (`mime_type`) but the runtime emits the plugin's native camelCase (`mimeType`) → downstream filter/scatter reference non-existent fields → empty pipeline (agent `0ee53785`; **re-surfaced in WP-62 live testing**, agent `2ffcd7bf`) | ✅ Fixed (uncommitted) | Field-fidelity **family** of WP-56 but a **distinct mechanism** (transform-declared-schema re-casing, not iteration-variable wrong-name) — not double-tracked. Fixed 2026-07-16 (Gap A `DataSchemaBuilder` reconcile + A2 ref rewrite + Gap B compiler O10a + Gap C warning; shared `field-name-reconciliation.ts`; 27 tests). **Q4 (calibration `StructuralRepairEngine` scatter-`itemVariable` detector) OUT of scope → tracked as a follow-up in [`V6_FIELD_FIDELITY_CALIBRATION_PHASE0_PHASE1_WORKPLAN.md`](../workplans/V6_FIELD_FIDELITY_CALIBRATION_PHASE0_PHASE1_WORKPLAN.md).** Full detail in [WEAK_POINTS § WP-63](./V6_WORKFLOW_DATA_SCHEMA_WORKPLAN_EXECUTION_WEAK_POINTS.md#wp-63-flatten-field-shape-fidelity--declared-snake_case-item-schema-vs-plugins-native-camelcase). Open: live Phase E validation. |
...
| [WP-61](./V6_WORKFLOW_DATA_SCHEMA_WORKPLAN_EXECUTION_WEAK_POINTS.md#wp-61-search-subject-dropped-by-schema-mapping-action-compiles-with-empty-params-topic-drop) | A required plugin input the user supplied in prose is dropped because the search subject arrives as `query` but the action's required text param is named differently (no `query` property) → `research_topic` compiles with `params:{}` → runtime "Topic is required" → empty digest (live agent `df67bf69`, 2026-07-05). EP required-plugin-param class (sibling of Sheet1). | 🟡 Fix implemented (uncommitted) | **P3.2** converter bind (`bindSearchSubjectToRequiredParam`, plugin-agnostic, 9/9 tests) + **P3.1** compiler warn (O16 sibling) + **B5** convertDataSource parity. **Still open:** P3.1 gate escalation (needs converter→creation provenance channel), formal `tests/v6-regression/scenarios/` snapshot (capture from fresh generation — df67bf69's IC not persisted), P3.4 output-contract fix. Cycle: `docs/workplans/EP_PRODUCTION_SHEETS_RANGE_FIX_WORKPLAN.md` Part 3. |
...
_WP-62 (Gmail-attachment `extract` non-deterministically routed to AI vs document-extractor — coverage decided by Phase-1 phrasing; WP-57 family, proven by `95f791ed` vs `0ee53785`) ✅ **Fixed + committed `05eca70`** (routing 2026-07-13 + live-validation hotfix 2026-07-16) — routing AND input-ref granularity now decided in reliable code (schema-driven coverage predicate + binder-authors/converter-honors + auto-split + bytes-field navigation), AI net preserved; live re-validation PASSED (calibration `passed`, runtime got the real base64 bytes). **Residual follow-ups** (low-priority; N1, N3, live phase2/phase4 snapshot capture, JPG/PNG image-OCR AC-9 fixture) are listed in the [workplan follow-ups section](../workplans/DETERMINISTIC_VS_AI_EXTRACTION_ROUTING_WORKPLAN.md#documented-low-priority-follow-ups-do-not-block-commit) — pointer only, not restated here. See [WEAK_POINTS § WP-62](./V6_WORKFLOW_DATA_SCHEMA_WORKPLAN_EXECUTION_WEAK_POINTS.md#wp-62-gmail-attachment-extract-non-deterministically-routed-to-ai-vs-document-extractor-coverage-decided-by-phase-1-phrasing)._
...
- **`hardcode_detected` parameterization nags — UX question, LOW / likely-expected.** Calibration on the WP-62 live-test Gmail-expense agents (`2ffcd7bf`, and historically `0ee53785`'s `max_results:"500"`) flags literals like `folder:"inbox"` and `max_results:50/500` (step1) as medium/cosmetic `hardcode_detected`. These are **non-blocking** and may be **intended calibration UX** (surface the literal so the user can decide to parameterize) rather than a generation defect. ⬜ Open question, not a WP: *should generation auto-parameterize these at build time, or is surfacing-in-calibration the desired behavior?* Decide before treating as a bug — no work until a product call is made.
