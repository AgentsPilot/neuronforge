...
> **Last Updated**: 2026-07-16 (WP-62 ✅ committed `05eca70`; WP-63 ✅ Fixed uncommitted — flatten field-shape fidelity, Gap A/B/C, 27 tests)
...
| [WP-57](#wp-57-google-drive--document-extractor-has-no-base64-byte-source) | **Google Drive → `document-extractor` path has no base64 byte source — extraction silently fails on every file, wrong data emailed.** `document-extractor.extract_structured_data` requires base64 file **bytes** ([document-extractor-plugin-executor.ts:48-103](../../lib/server/document-extractor-plugin-executor.ts#L48-L103)): `file_content` as a base64 string, or a file object exposing `.content`/`.data`; a real http(s) `file_url` is **not implemented** (executor throws, L244). But Google Drive's `list_files` returns **metadata-only** file objects (id, name, mimeType, webViewLink — no bytes), and **no `google-drive` action returns base64 bytes**: `read_file_content` [storage/fetch_content] does `downloadResponse.text()` on non-Google-Doc files ([google-drive-plugin-executor.ts:312-327](../../lib/server/google-drive-plugin-executor.ts#L312-L327)), corrupting binary PDFs/images, and is a *text* extractor by contract (output field `content` = "Text content extracted from the file"). **Two failure modes, selected by Phase 1 non-determinism:** (a) document-extractor bound directly on the `list_files` item → fed the metadata object → `_extraction_metadata.success=false`, all fields missing; (b) Phase 1 inserts `read_file_content` before extract → the IR converter's **O-WP12** `inputLooksLikeFileAttachment` heuristic ([IntentToIRConverter.ts:2016-2034](../../lib/agentkit/v6/compiler/IntentToIRConverter.ts#L2016-L2034)) classifies the read output as **text** (field `content` ∈ TEXT_MARKERS; `mime_type` ≠ the camelCase `mimeType` FILE_MARKER) → reroutes to `ai_processing`, which then receives `read_file_content`'s corrupted binary-as-text. Neither path yields working OCR. **Working contrast:** Gmail's `get_email_attachment` returns `{ data: <base64>, mimeType, filename }` → recognized as a file via `mimeType`, document-extractor pulls `.data` → works; Drive has no equivalent action. **Inverse of [WP-12](#wp-12-document-extractor-bound-to-free-text-email-body-instead-of-ai_processing)** (which correctly reroutes *text* away from document-extractor); here a genuine file that *needs* OCR is starved of bytes or wrongly rerouted. Related: WP-34 (document-extractor fabricates "Unknown" on parse failure), WP-53/WP-56 (same incident family). **Surfaced live 2026-06-10** on agent `6ef48513-f49c-4635-b0dc-bdd9bf2f80a6` ("Google Drive Invoice Summary Email Agent"); captured AS-IS in [tests/v6-regression/scenarios/drive-invoice-summary-extractor-asis](../../tests/v6-regression/scenarios/drive-invoice-summary-extractor-asis); live report `output/vocabulary-pipeline/live-execution-report.json`. User received an email with "Extraction failed: missing …" on every line, from the **wrong files** (compounded by an empty `list_files.params` folder-binding miss — WP-53 family). **Calibration would catch but not fix:** Layer 3 `DryRunValidator` would see `success=false`/empty extraction and Layer 2 `MultiStepStructuralDetector` could flag the missing content-fetch step, but a real fix needs a base64 source that doesn't exist (inserting `read_file_content` still corrupts binary) → it would raise non-convergence (G-CAL-3) and surface, not silently repair. This agent was **never calibrated** (0 `calibration_history` rows, status `active`). | P1 | ✅ **Fixed** — Option A delivered on branch `fix/v6-drive-extractor-flow`; **Phase E live PASS 2026-06-15** (agent `6ef48513`: real email received, folder routing + Textract extraction of 4 files incl. a PNG verified). Commits: `cb7545f`/`0af2489`/`d08698b` (download_file + converter `semantic_type` + compiler auto-insert), `166318d` (read_file_content real-PDF text), `63f6ef2` (firmed §6.4 nudge), `85d63bd` (2B-1 executor URL→ID tolerance), `21a416b` (2B-2 `reconcileInputsToDsl` + 2C regen snapshot). Note: 2A′ binder fix proved **moot** — the compiler auto-insert + §6.4 nudge produce `download_file → document-extractor` without it. Follow-ups split out: [WP-58](#wp-58-multi-input-aigenerate-steps-bind-only-one-input) (low), [WP-59](#wp-59-document-extractor-deterministic-field-quality-on-real-receipts). **Original plan:** (1) new `google-drive.download_file` action — downloads raw bytes via `alt=media` → `Buffer.from(await res.arrayBuffer()).toString('base64')`, returns the Gmail-style shape `{ file_content: <base64>, mimeType, filename, file_id }` with **`x-semantic-type: file_attachment`** on the output; `read_file_content` stays the clean text extractor. (2) The `x-semantic-type` annotation makes `inputLooksLikeFileAttachment` recognize the download output as a file (per the WP-12 comment that the field-name heuristic exists only "until plugin annotations are complete") so the converter **keeps** document-extractor instead of rerouting to AI. (3) Produce the shape `list_files → download_file(base64) → document-extractor(file_content)` (Phase 1 nudge + a generic, plugin-agnostic compiler auto-insert of a content-fetch step before a file-input action lacking bytes). (4) Bonus: fix `read_file_content`'s stale `required: ["id","name","mimeType"]` (references non-existent output fields). **Files:** `lib/plugins/definitions/google-drive-plugin-v2.json`, `lib/server/google-drive-plugin-executor.ts`, `lib/agentkit/v6/compiler/IntentToIRConverter.ts`; document-extractor executor needs no change. |
| [WP-58](#wp-58-multi-input-aigenerate-steps-bind-only-one-input) | **Low / cleanliness.** AI/`generate` IR operations are single-input by grammar: `AIConfig` exposes only `input?: string` ([declarative-ir-types-v4.ts:286-312](../../lib/agentkit/v6/logical-ir/schemas/declarative-ir-types-v4.ts#L286-L312)), unlike `TransformConfig` which has `input` **+ `additional_inputs?: string[]`** (L260-268). So a `generate` step that needs several inputs (an array **plus** aggregates) can't express them: `IntentToIRConverter` keeps all inputs in the node-level `inputs` (graph deps, so they're computed + ordered) but sets `ai.input` to the first only, and `AIOperationResolver` injects just `{{input_source}}` into the prompt; the rest survive as prose. Also Phase-1 IC omits config values referenced in the prompt (`folder_link`) from the step's declared `inputs`. **Benign in practice** (verified Phase E 2026-06-15): the email was complete because (a) the runtime `ai_processing` exposes config scope to the LLM so `folder_link` is available anyway, and (b) the LLM re-derives count/sum/missing from the array. Net effect today = dead aggregate steps + dishonest data flow, not broken output. **Re-surfaced non-benign as requirement Item 11** (agent `0ee53785` step7 row-builder inside a scatter references `attachment_item.subject/.from/.filename` in prose but its declared input is `{{extracted_fields}}` only → blank From/Subject/Filename columns). | P3 | ✅ **Fixed** — folded into requirement **Item 11 (Batch 3, sub-phase 3A)**, branch `agent-failure-troubleshooting`, 2026-07-12. Delivered exactly the documented fix shape: added `additional_inputs?: string[]` to `AIConfig` ([declarative-ir-types-v4.ts](../../lib/agentkit/v6/logical-ir/schemas/declarative-ir-types-v4.ts)); a shared deterministic detector [`ai-input-context.ts`](../../lib/agentkit/v6/compiler/ai-input-context.ts) (`detectReferencedInScopeVariables` / `extractBaseVarName` — structural, no plugin/field hardcoding); root-cause populate in `IntentToIRConverter.convertGenerate` (new `loopItemVarStack` makes the scatter item var discoverable to loop-body AI steps); labelled-object injection in `ExecutionGraphCompiler.compileAIOperation` (consumes IR `additional_inputs`, falls back to the same detector against `ctx.loopContextStack` for legacy IR — Principle 4). Runtime already exposes an object-valued `input` as labelled "Data for Analysis" blocks (existing multi-input branch + transform loop-var injection L801-827). In-place edit for `0ee53785` via [`scripts/fix-ai-scatter-loopvar-input.ts`](../../scripts/fix-ai-scatter-loopvar-input.ts) (dry-run default; owner-scoped `AgentRepository.updatePilotSteps`). Content-asserting + scoping + negative + genericity tests: [`ai-input-context.test.ts`](../../lib/agentkit/v6/compiler/__tests__/ai-input-context.test.ts). Note: the AI-resolver leg the original diagnosis named (`AIOperationResolver`) is NOT on the active declarative-IR path — the live resolver is `compileAIOperation`; fix placed there. Deferred (SA §4): declaring config/aggregate inputs (`folder_link`/`total_sum`) in the Phase-1 IC + dead aggregate-step cleanup. |
| [WP-60](#wp-60-compiler-param-binder-cant-match-folder_id-to-folder_link-stem-so-list_files-defaults-to-drive-root) | **Compiler param-binder leaves `list_files.folder_id` UNBOUND when the only folder config key is `folder_link` → lists the Drive root.** `ExecutionGraphCompiler.normalizeActionConfigWithSchema` third pass ([ExecutionGraphCompiler.ts:6208-6247](../../lib/agentkit/v6/compiler/ExecutionGraphCompiler.ts#L6208-L6247)) auto-injects missing required params from workflow config via `findBestConfigMatch` (token-Jaccard, threshold **0.4**). For the action param `folder_id` the only folder-ish config key is `folder_link`; `calculateTokenOverlap('folder_id','folder_link')` = `{folder,id}`∩`{folder,link}` / `{folder,id,link}` = 1/3 = **0.333 < 0.4** → no match → `folder_id` left unbound → at runtime `list_files` lists from root ("If not provided, lists from root or recent files"). The namespaced input key `google-drive__list__folder_link` (which explicitly tags the folder for this step) is **not in `ctx.workflowConfig`** at compile (it's a runtime-owned input), so the binder never sees it. **Threshold can't just be lowered** — 0.4 deliberately blocks `file_id`↔`sheet_id` (also 0.333, per the inline comment). **Distinct from WP-57 2B:** 2B Part 2 (`reconcileInputsToDsl`) bridges a value onto an EXISTING `{{input.folder_id}}` ref at runtime, and 2B Part 1 (`extractDriveId`) converts a URL in `folder_id`; both assume the compiler emitted a `folder_id` binding. Here the compiler emits **none**, so neither fires. **Surfaced live 2026-06-16** on a freshly-created agent (`48d587d4-e21a-4439-a186-86b71318c042`) during batch calibration: config had `folder_link` + `google-drive__list__folder_link` but **no `folder_id`**; compiled `list_files` had no folder binding; `list_files` returned 20 root files (`parents:["18WFGybj…"]`, mixed zip/etc.), 0 delivered, user emailed a summary of the wrong files. **Why the WP-57 regression scenario passed but this didn't:** the captured scenario's IntentContract config declared an explicit `folder_id` key (exact-match bind → `{{input.folder_id}}` emitted → reconcile bridged); the live creation flow emitted only `folder_link`, exposing the binder gap. | P1 | ✅ **Fixed (compiler)** 2026-06-16 (uncommitted) — stem-aware match added to `findBestConfigMatch` ([ExecutionGraphCompiler.ts](../../lib/agentkit/v6/compiler/ExecutionGraphCompiler.ts)): a config key whose stem (trailing `id`/`link`/`url` dropped) equals the param's stem is treated as a full match, so `folder_id` binds to `folder_link` → emits `folder_id: {{config.folder_link}}`; at runtime `StructuralRepairEngine` rewrites `{{config.X}}`→`{{input.X}}`, resolves to the URL, and `extractDriveId` (2B-1) converts URL→ID. Exact stem check (not a Jaccard relaxation) so `file_id`↔`sheet_id` and `invoice_id`↔`invoice_date` stay unmatched. +7 unit tests; typecheck clean. **⚠ Existing agents need a DSL rebuild** for the compiler fix alone — so the **decision (2026-06-17) is to ship A + B as layered defense** (same shape as 2B Part 1 + Part 2): **(A) compiler stem match ✅ done** (future agents correct on disk); **(B) runtime param-injector ✅ done** (`lib/pilot/injectUnboundActionParams.ts`, +12 tests) — a new schema-aware function run after `reconcileInputsToDsl` in `WorkflowPilot.execute()` that targets the step by the namespaced key's **plugin** (the capability segment is an unreliable join key — appears as both `list` and `storage/list`), picks the unbound param from the **action schema by stem** (`folder_link`→`folder_id`), and injects the value **only when a single schema param stem-matches a single step-tagged key** (conservative, backward-safe, in-memory per execution; raw value injected, executor `extractDriveId` converts URL→ID). B repairs already-saved agents (incl. `48d587d4`) on next execution without a rebuild and catches future compiler misses. Deepest root cause (noted, out of scope): creation flow tags the input `folder_link` not the action's `folder_id` — emitting `google-drive__list__folder_id` would bind by exact match with zero heuristics. **A's mechanism: stem-aware param matching** in `findBestConfigMatch`/`calculateTokenOverlap` — strip role-suffix tokens (`_id`/`_link`/`_url`/`_ids`) before comparison so `folder_id`↔`folder_link` collapse to stem `folder` (match), while `file_id`↔`sheet_id` stay distinct (`file`≠`sheet`). Compiler then emits `folder_id: {{config.folder_link}}`; executor's `extractDriveId` (2B-1) converts the URL→ID at runtime. Plugin-agnostic. **Execution impact: requires a DSL REBUILD** — a compiler change only affects future compilations; the already-created agent's stored `pilot_steps` stays unbound until regenerated. **Optional backward-repair (fixes existing agents on next execution, no rebuild): a schema-aware runtime param-injector** — in `WorkflowPilot.execute()`, for each action step look up required params unbound in the step config and inject a stem-matching namespaced/flat input value (extends the WP-57 2B-2 runtime hook with action-schema awareness). Bigger change than reconcile. **Considered & rejected:** blanket threshold drop to 0.33 (re-enables `file_id`↔`sheet_id` false matches); `x-context-binding` on the plugin def (plugin-specific, needs the config key name baked in). |
| [WP-59](#wp-59-document-extractor-deterministic-field-quality-on-real-receipts) | **document-extractor field quality (deterministic mode).** On the Phase E live run (2026-06-15) the extractor read all 4 files incl. a PNG (Textract OCR, confidence 0.80–0.96) but parsed several fields wrong: `tax_amount` garbage on every file (`"es & fees"`, `"One-time credit purchase1$50.00$50.00"`), `due_date` prefix bleed (`"paidMarch 16, 2026"`), `invoice_number` null-char merge (`"ATJYUG83 0001"`), inconsistent `total_amount` format. Root: the extractor runs deterministic OCR field-matching; the compiler emitted a no-op `deterministic` param (now fixed → `use_ai`, see WP-57 follow-up commit), and **`use_ai: true` is NOT a fix** — the executor maps `ocrFallback: !use_ai` and the LLM-fallback path is explicitly *not implemented* ([document-extractor-plugin-executor.ts:125](../../lib/server/document-extractor-plugin-executor.ts#L125)), so enabling it would disable OCR and break image extraction. Real fix lives in [lib/extraction/DeterministicExtractor.ts](../../lib/extraction/DeterministicExtractor.ts) (field segmentation) or by implementing the `use_ai` LLM path. **Hidden today** only because the downstream LLM summarizer smooths the values into the email. | P2 | ⬜ Documented — full diagnosis in [WP57 summary § WP-59 finding](../WP57_DRIVE_DOCUMENT_EXTRACTION_SUMMARY.md). **Decision pending:** invest only if accurate per-field invoice data is required (vs. the LLM-written summary being the deliverable). Real extractor-engine work, not a config flip. |
| [WP-61](#wp-61-search-subject-dropped-by-schema-mapping-action-compiles-with-empty-params-topic-drop) | **A required plugin input the user supplied in prose is dropped because the search subject arrives as `query` but the action's required text param is named differently (and the action has no `query` property).** `mapParamsToSchema` copies a generic param through only if its key is in the action schema; `research_topic` declares `required:["topic"]` with **no `query` property**, so `params.query` (set in `buildDataSourceParams` L376) is silently dropped, the `data`→firstRequired fallback (L2316) doesn't rescue `query`, and `convertDataSource` had no required-param validator → `step1.params:{}` → runtime "Topic is required" → steps 2-5 cascade → empty digest emailed. **Sibling of the Sheet1 range case** (a required plugin input supplied by the user but never materialized into `params` — Sheet1 *fabricated* a default, this *drops* it). **Surfaced live 2026-07-05** on agent `df67bf69`; see [V6_RCA_df67bf69_missing-topic.md](../investigations/V6_RCA_df67bf69_missing-topic.md). §5A empirical: prompt-only structuring is non-deterministic in BOTH v15 (4/5) and v16 (3/5) → not a prompt regression, not fixable by prompt tuning. | P1 | 🟡 **Fix implemented (uncommitted, deterministic-first).** P3.2 (`IntentToIRConverter.bindSearchSubjectToRequiredParam`) binds a dropped subject to the single unbound required string param — schema-driven, plugin-agnostic, no-guess on 0/≥2 targets; 9/9 unit tests incl. real-schema. P3.1 (`ExecutionGraphCompiler.detectEmptyRequiredParams`, Phase 3.9b, O16 sibling) warns on any empty required param (never fabricates — Principle 2). B5 adds the required-param warning to `convertDataSource` for parity. **Deferred:** P3.1 gate escalation (needs a converter→creation provenance channel), formal `tests/v6-regression/scenarios/` snapshot (df67bf69's intent-contract was never persisted — capture from a fresh generation), P3.4 output-contract fix. |
| [WP-62](#wp-62-gmail-attachment-extract-non-deterministically-routed-to-ai-vs-document-extractor-coverage-decided-by-phase-1-phrasing) | **An email-attachment `extract` inside a scatter non-deterministically bound field-extraction to either `document-extractor.extract_structured_data` (deterministic OCR) or `ai_processing/llm_extract`, on structurally identical agents** — the deterministic-vs-AI choice was effectively decided by Phase-1 phrasing (input-ref granularity + field framing), not reliable code. Proven by two same-owner, same-task agents with byte-identical structure (`search_emails → flatten → filter → scatter[get_email_attachment → extract]`): `0ee53785` bound document-extractor (surface fields + separate normalize); `95f791ed` fell to the AI branch (whole-object input, meta fields folded). Both had a genuinely-covering connected plugin. Image attachments on the AI path get base64-as-text → fabrication (WP-13/34 family). WP-57 family. | P1 | ✅ **Fixed (uncommitted)** 2026-07-13 — the deterministic-vs-AI decision is now an **ordered coverage-then-fallback judgment made in reliable code, with the AI net preserved**. New schema-driven predicate `ExtractionCoverage.evaluateExtractionCoverage` (CC-1..CC-4 / CC-3a); `CapabilityBinderV2` Phase 2c (`routeExtractionCoverage`) **authors** the verdict (covered → live document-extractor binding for the surface subset; uncovered → unbound); `IntentToIRConverter.convertExtract` **honors** it (single decision-maker) and **auto-synthesizes** a downstream `generate` for the meta/computed residual (the `0ee53785` shape). Companions: B1 Phase-1 `source` steer, B2 `get_email_attachment` `x-semantic-type: file_attachment`, B3 `inputLooksLikeFileAttachment` loop-internal + producer-shape hardening. **Zero field-name/plugin-identity branches** (Principle 6). AI branch unchanged + reachable (AC-6). 28 unit/scenario tests incl. 5×-repeated determinism (AC-1) + uncovered→AI (AC-2). **R1 materialized:** the grammar lacked a per-field source signal → added optional `ExtractFieldSource` (`document`\|`meta`\|`computed`) to `extract.fields[]` (schema-driven, no name list). **Hotfix 2026-07-16 (live validation, agent `2ffcd7bf`):** routing worked but the fix was *incomplete* — `convertExtract`'s param mapping copied a whole-object `extract.input` verbatim into the `type:"string"` `file_content` param → runtime *"should be string, got object"* → all scatter items failed. Closed by resolving **input-ref granularity in reliable code too** (bytes-field navigation, idempotent, schema-driven), de-duplicating `BYTES_FIELDS` (3 copies → 1), and adding `from_base64_content: "data"` to the plugin def. |
| [WP-63](#wp-63-flatten-field-shape-fidelity--declared-snake_case-item-schema-vs-plugins-native-camelcase) | **A generated `flatten` step declares its item `output_schema` in snake_case (`mime_type`, `message_id`, …) but the runtime `transformFlatten` emits the source plugin's native camelCase keys verbatim (`mimeType`), so downstream `filter`/`scatter` reference non-existent fields → filter empties → scatter iterates nothing → empty report.** Field-fidelity **family** of WP-56 but a **distinct mechanism** (transform-declared-schema re-casing planted into the canonical `WorkflowDataSchema` via WP-18 Bug A, vs WP-56's iteration-variable wrong-name). Proven on agent `0ee53785`; **re-surfaced during WP-62 live testing** (agent `2ffcd7bf`, same Gmail scatter-attachment shape). Separate bug from WP-62 — not folded in. | P1 | ✅ **Fixed (uncommitted)** 2026-07-16 (branch `agent-failure-troubleshooting`) — reconciliation now runs in reliable code, schema-driven, no field-name/plugin-identity lists. **Gap A** (`DataSchemaBuilder` Pass 2d): reconciles ai_declared/inferred FIELD-PRESERVING transform + loop item_ref item schemas to the producer's real names (declared `mime_type` → producer `mimeType`) via the shared normalized-key matcher, resolving DOTTED producer inputs (M3) against the flatten universe child ∪ parent(`_parentData`) ∪ builtins (M2); **A2** (`CapabilityBinderV2.applyTransformFieldRenames`): applies the SAME rename map to ALL downstream ref shapes — `{kind:"ref"}`, `{{var.field}}` templates, bare `key_field` literals (M4) — scoped by the reconciled data_schema (no second fuzzy match). **Gap B** (`ExecutionGraphCompiler` O10a): refactored onto the shared helper, dotted-input base-var fallback + bare `condition.field` rewrite (belt-and-suspenders for legacy IR). **Gap C**: warning-by-default plugin-truth surfacing, op-scoped (preserving ops only — never flags computed fields). **M1** collision safety (`message_id`/`messageId` → ambiguous → never guess). FIELD-SYNTHESIZING ops (`map`/`with_fields`/…) untouched (WP-18 Bug A preserved). Shared helper `field-name-reconciliation.ts` is the single normalizer for Gap A + Gap B. **27 unit/e2e tests** (AC-1..AC-11). Full diagnosis: [RCA](../investigations/AGENT_RCA_CONCLUSION_gmail-expense-attachment-flatten.md); workplan: [`WP63_FLATTEN_FIELD_SHAPE_FIDELITY_WORKPLAN.md`](../workplans/WP63_FLATTEN_FIELD_SHAPE_FIDELITY_WORKPLAN.md). **Q4 (calibration `StructuralRepairEngine` itemVariable detector) out of scope → tracked in the field-fidelity calibration workplan.** Live Phase E not run (Dev env). |
...
### WP-61: search subject dropped by schema-mapping, action compiles with empty params (topic-drop)

**A required plugin input the user supplied in prose is never bound, because the search subject arrives as `query` but the bound action names its required text param differently and has no `query` property.** `IntentToIRConverter.buildDataSourceParams` sets a free-text subject as `params.query` ([L376-377](../../lib/agentkit/v6/compiler/IntentToIRConverter.ts#L376)). `mapParamsToSchema` then copies a generic param through **only if its key exists in `schema.parameters.properties`** ([L2270](../../lib/agentkit/v6/compiler/IntentToIRConverter.ts#L2270) / [L2348](../../lib/agentkit/v6/compiler/IntentToIRConverter.ts#L2348)). `chatgpt-research.research_topic` declares `required:["topic"]` (`topic.minLength=3`) with **no `query` property**, so `query` is silently dropped; the `data`→firstRequired last-resort fallback ([L2316](../../lib/agentkit/v6/compiler/IntentToIRConverter.ts#L2316)) fires only on `genericParams.data`, not `query`; and `convertDataSource` — unlike `convertArtifact`/`convertDeliver` — had **no required-param validator**, so it emitted `params:{}` with not even a warning. At runtime the executor rejects the call (`Topic is required for research`); steps 2-5 cascade on empty input; an empty digest is emailed.

**Sibling of the Sheet1 range case** (the EP required-plugin-param class): both are "a required plugin input the user genuinely supplied but the pipeline never materialized into `params`." Sheet1 *fabricated* a default (`"Sheet1"`); here the subject is *dropped* entirely. Same root, two symptoms.

**Surfaced live 2026-07-05** on agent `df67bf69-c2ec-45e7-8d77-0301caa1ae54` ("Daily Retail Solutions (Israel) Blog Digest Email"): the subject "retail solutions in Israel" was in the original prompt + EP `sections.data` prose + `step1.name`/`description`, but never in `resolved_user_inputs`; compiled `step1.params = {}`. Full diagnosis: [V6_RCA_df67bf69_missing-topic.md](../investigations/V6_RCA_df67bf69_missing-topic.md). **Empirical (§5A):** re-running df67bf69's conversation through both prompt versions structured the subject non-deterministically in **BOTH** v15 (4/5) and v16 (3/5) — so this is **not** a prompt-version regression and **not** fixable by prompt tuning; the fix must be a deterministic downstream layer.

**Fix (2026-07-05, uncommitted — Part 3 of the EP required-plugin-param cycle, deterministic-first):**
- **P3.2 (converter — the resolve):** `IntentToIRConverter.bindSearchSubjectToRequiredParam`, called in `convertDataSource` after `mapParamsToSchema` — binds a dropped `query` subject to the action's **single unbound required `string` param** (schema-driven, plugin-agnostic; no-guess on 0/≥2 targets — Principle 2/Anti-pattern C; respects `minLength`/`maxLength` on literals, skips the gate for `{{refs}}`). Binds `research_topic.topic` deterministically. 9/9 unit tests incl. one against the real plugin schema (`IntentToIRConverter.p32.test.ts`).
- **P3.1 (compiler — the backstop):** `ExecutionGraphCompiler.detectEmptyRequiredParams` (Phase 3.9b, sibling of the O16 pass) — plugin-agnostic detection that **warns** on any empty required param; never fabricates. Gate escalation (marker/hard-fail per provenance — SA Q1) deferred pending a converter→creation provenance channel.
- **B5:** required-param warning added to `convertDataSource` for parity with `convertArtifact`/`convertDeliver`.
- **Compounding (P3.4, deferred, sequenced-after):** the EP also promised `research_topic` output fields the action doesn't return (`source name`, `publication date`; `sources[]` is only `{title,url,snippet}`) → the step2/3/4 shape/phantom-field defects.

**Why this wasn't caught earlier:** the intent prompt (`intent-system-prompt-v2.ts` §6.1) routes a text subject into `query` by design; `mapParamsToSchema` drops any key not in the action schema; `convertDataSource` had no required-param validator (only `convertArtifact`/`convertDeliver` did); no compiler guard failed an action step with `params:{}`. The `ConstrainedSemanticValidator` detected the missing `topic` pre-execution, but detection did not gate the compile or the delivery.

**Files:** `lib/agentkit/v6/compiler/IntentToIRConverter.ts` (P3.2 + B5), `lib/agentkit/v6/compiler/ExecutionGraphCompiler.ts` (P3.1), `lib/agentkit/v6/compiler/__tests__/IntentToIRConverter.p32.test.ts`.

---

### WP-62: Gmail-attachment `extract` non-deterministically routed to AI vs document-extractor (coverage decided by Phase-1 phrasing)

**For an email attachment inside a scatter (`get_email_attachment → extract`), the pipeline NON-DETERMINISTICALLY bound field-extraction to either `document-extractor.extract_structured_data` (deterministic OCR) or `ai_processing/llm_extract`, on structurally identical agents.** Both had a genuinely-covering connected deterministic plugin; the AI net was used anyway because the "deterministic vs AI" choice was effectively decided by how the Phase-1 IntentContract phrased the extract step (input-ref granularity + field framing) rather than by reliable code. Image attachments on the AI path receive base64-as-text → fabrication (WP-13/16/34 family). WP-57 family.

**Evidence:** agents `95f791ed-…` (AI) vs `0ee53785-…` (document-extractor), same owner + task, byte-identical structure. Failing step-6 `prompt` = `IntentToIRConverter.ts` AI-branch template. Full RCA: [`AGENT_RCA_CONCLUSION_gmail-expense-attachment-ai-extract.md`](../investigations/AGENT_RCA_CONCLUSION_gmail-expense-attachment-ai-extract.md) (§§ 8/10/11/12). Requirement: [`DETERMINISTIC_VS_AI_EXTRACTION_ROUTING_REQUIREMENT.md`](../requirements/DETERMINISTIC_VS_AI_EXTRACTION_ROUTING_REQUIREMENT.md). Workplan + SA Q1–Q3: [`DETERMINISTIC_VS_AI_EXTRACTION_ROUTING_WORKPLAN.md`](../workplans/DETERMINISTIC_VS_AI_EXTRACTION_ROUTING_WORKPLAN.md).

**Fix (2026-07-13, uncommitted — ordered coverage-then-fallback in reliable code; AI net preserved):**
- **Coverage predicate (new):** `lib/agentkit/v6/capability-binding/ExtractionCoverage.ts` — `evaluateExtractionCoverage` implements CC-1 (document/file input), CC-2 (a CONNECTED deterministic document-extraction capability, discovered by `domain=document`+`capability=extract_structured_data`, not by plugin key), CC-3/CC-3a (requested-field partition by **declared source** — Q2), CC-4 (file-type support derived from the plugin's `must_support` flags). Conservative in the safe direction. Zero field-name / plugin-identity branches.
- **Binder authors (Q1):** `CapabilityBinderV2` Phase 2c `routeExtractionCoverage` — resolves CC-1 from the input's producer output schema (loop-internal aware — fixes the RCA gate-1 miss), runs the predicate, and either authors a live `document-extractor` binding (surface subset) or leaves the step unbound. Single decision-maker.
- **Converter honors + splits (Q1/Q3):** `IntentToIRConverter.convertExtract` consumes the verdict (skips the O-WP12 / Phase-2b heuristic rerouters so nothing strips a binder-authored binding), and when the extract folds meta/computed fields, **auto-synthesizes** a downstream `generate` for the residual (the `0ee53785` shape). Surface data is copied through UNCHANGED; unknown residual → `null`, never a placeholder (Principle 2/11, G2).
- **Companions:** B1 Phase-1 `source` steer in `intent-system-prompt-v2.ts` §6.4 (steer-not-decide); B2 `x-semantic-type: file_attachment` on `google-mail-plugin-v2.json` `get_email_attachment.output_schema`; B3 hardened `inputLooksLikeFileAttachment` (loop-internal producer resolution + bytes-field/producer-shape signal that the whole-graph text short-circuit can't override).
- **SA review round 1 (Finding #1, HIGH):** the binder's authoritative CC-1 now **normalizes the input ref to its base variable** (shared `baseVarOfRef` — strip `{{}}` + drop dotted tail) so the *well-phrased* B1 shape (`extract.input = {{attachment_content.data}}`, pointed at the bytes field) still resolves to its producer and is judged a file (previously → AI inversion for the well-phrased plan). CC-1 and the converter's `inputLooksLikeFileAttachment` now **share one classifier** (`classifySchemaFileness`) + `baseVarOfRef`, so B3 is not dead code and the two cannot diverge. Also added AC-3 phrasing-invariance tests (whole-object vs dotted-bytes × folded vs already-split) and R1 absent-`source` degradation tests.
- **R1 (grammar dependency) materialized:** the IntentContract had no per-field source signal → added optional `ExtractFieldSource` (`document`\|`meta`\|`computed`) to `extract.fields[]` in `intent-schema-types.ts` (schema-driven grammar extension, the R1-preferred mitigation — never a field-name list). Absent → conservatively residual (AI-safe).
- **Tests:** `ExtractionCoverage.test.ts`, `CapabilityBinderV2.coverage.test.ts`, `IntentToIRConverter.coverage.test.ts`, and regression scenario `tests/v6-regression/scenarios/gmail-scatter-attachment-extract/` (binder→converter over the folded fixture, 5×-repeated determinism — AC-1 — plus uncovered→AI — AC-2). 28 tests pass.

**Why this wasn't caught earlier:** WP-57's Gmail "working contrast" was a single non-scatter observation; the document-vs-AI outcome was Phase-1-non-deterministic, so a passing capture didn't prove robustness. No regression scenario ran the scatter-attachment shape repeatedly or asserted the covered-vs-uncovered boundary.

**Hotfix (2026-07-16) — the param-granularity gap the live validation exposed.** Live validation (agent `2ffcd7bf-3afc-45f5-8315-5ff00eb0d8a2`) proved the routing fix works: all 5 phases green, `document-extractor` bound, IC + data_schema persisted, Phase-1 emitted per-field `source` (3 `document` / 1 `computed` — the previously-unproven B1 link), CC-3a split + `synthesizeResidualGenerateNode` wiring correct. TS regression-tested all six WP-62 components — **none defective**. But calibration's dry-run correctly blocked the agent: `step6` failed all 3 scatter items with **`Parameter file_content should be string, got object`**.

**Diagnosis:** WP-62 was **incomplete, not wrong**. It removed Phase-1 dependence from the *routing* decision but left the *input-ref granularity* Phase-1-dependent. Phase 1 legitimately emitted a whole-object `extract.input: "attachment_content"` (the grammar types it as a `RefName`); `convertExtract`'s `x-input-mapping` loop copied it **verbatim** into `file_content` (declared `type: "string"`). Latent since before WP-62 — pre-WP-62 this shape rerouted to AI (the very bug WP-62 fixes), so it never executed; the known-good `0ee53785` only worked because Phase 1 *happened* to emit `.data`. Full RCA: [`AGENT_RCA_CONCLUSION_wp62-live-validation-file-content-object.md`](../investigations/AGENT_RCA_CONCLUSION_wp62-live-validation-file-content-object.md).

**Hotfix implemented:**
- **Bytes-field navigation in reliable code (root cause).** When a param's `x-input-mapping.accepts` includes `file_object` and the input slot resolves to a bytes-bearing object, `convertExtract` now emits `{{<baseVar>.<bytesField>}}` (→ `attachment_content.data`). **Idempotent by construction** (ref rebuilt from `baseVarOfRef` + the schema's bytes key — `.data` stays `.data`). **Schema-driven** (bytes key from the producer slot via shared `bytesFieldOf`; zero plugin/field-name branches). Slot resolution reuses CC-1's normalization so mapping and verdict cannot disagree. **Safe direction:** a bytes-less object emits no broken ref — it reroutes to the AI net with a structured warning (Principle 11). WP-62's thesis extended one step: *ref granularity, like routing, must be decided by reliable code — not inherited from Phase-1 phrasing.*
- **`BYTES_FIELDS` de-duplicated 3 → 1.** It had three identical copies (`ExtractionCoverage.ts`, `slotHasBytes`, `findDownloadAction`) — the same divergence class SA flagged as the Round-1 HIGH. Now a single `BYTES_FIELDS_PRIORITY`/`BYTES_FIELDS` in `ExtractionCoverage.ts`, consumed via `bytesFieldOf()`/`schemaHasBytes()`.
- **Plugin def — RCA premise corrected.** `from_file_object` is **not** dead code: `ExecutionGraphCompiler` (L6273/L6281) reads it and its no-schema fallback is **load-bearing for the WP-57 Drive path** (Drive's bytes key genuinely *is* `content`), so removing it would regress WP-57. Kept it and **added `from_base64_content: "data"`** — a hint the compiler already supported but no plugin def declared — so the compiler's schema-aware pass resolves Drive→`content` and Gmail→`data`. Also declared `from_base64_content?` on the `InputMapping` type. Compiler-side defense-in-depth; the converter is the root-cause fix (Principle 4/7).
- **Tests (+8):** whole-object input → `.data`; braced variant; idempotency (`.data` / `{{.data}}` unchanged); Drive-like producer → `.content` (proves schema-driven, not hardcoded); safe direction (no bytes → AI net); unknown slot → verbatim (WP-57 path unchanged); plus an end-to-end regression-scenario assertion. **50 WP-62 tests pass.**

**Why the 42 pre-hotfix tests missed it:** the fixtures **encoded the same assumption as the code under test** — all pointed `extract.input` at the already-navigated `.data` shape, so the verbatim copy was never exercised with the whole-object ref live Phase 1 actually produces. Green fixtures proved the routing, not the param granularity. Captured as a new anti-pattern in [V6_DESIGN_PRINCIPLES](./V6_DESIGN_PRINCIPLES.md) § Section 2.

**Not run in this environment (needs the user / QA):** live Phase A/D/E (Phase-1 LLM + Gmail + Textract OCR) — no live credentials in the Dev self-test env. **Re-validation required:** re-create the agent and confirm compiled `file_content: {{attachment_content.data}}` + a green calibration. The determinism/granularity proofs that are runnable here are exercised by the regression driver; a full Phase E capture + committed phase2/phase4 snapshots remain a QA follow-up.

**Files:** `lib/agentkit/v6/capability-binding/ExtractionCoverage.ts` (new), `lib/agentkit/v6/capability-binding/CapabilityBinderV2.ts` (Phase 2c), `lib/agentkit/v6/compiler/IntentToIRConverter.ts` (honor + split + B3), `lib/agentkit/v6/semantic-plan/types/intent-schema-types.ts` (R1 grammar), `lib/agentkit/v6/intent/intent-system-prompt-v2.ts` (B1), `lib/plugins/definitions/google-mail-plugin-v2.json` (B2), + 4 test files.

---

### WP-63: `flatten` field-shape fidelity — declared snake_case item schema vs plugin's native camelCase

**✅ Fixed (uncommitted) 2026-07-16, branch `agent-failure-troubleshooting`. Owner: `v6-pipeline` (Phase 2 `DataSchemaBuilder`, primary) + `ExecutionGraphCompiler` (defense-in-depth).** Full diagnosis: [`AGENT_RCA_CONCLUSION_gmail-expense-attachment-flatten.md`](../investigations/AGENT_RCA_CONCLUSION_gmail-expense-attachment-flatten.md) (Gap A / Gap B / Gap C analysis + why calibration cannot catch it). Implementation: [`WP63_FLATTEN_FIELD_SHAPE_FIDELITY_WORKPLAN.md`](../workplans/WP63_FLATTEN_FIELD_SHAPE_FIDELITY_WORKPLAN.md).

**Fix (2026-07-16):** shared helper `lib/agentkit/v6/capability-binding/field-name-reconciliation.ts` (the single normalized-key matcher + M1 collision guard, used by BOTH Gap A and Gap B). **Gap A** `DataSchemaBuilder` Pass 2d `reconcileAiDeclaredTransformSchemas`/`reconcileSlotAgainstProducer` — reconciles FIELD-PRESERVING transform + loop item_ref item schemas to the producer's real casing before they enter `WorkflowDataSchema`; dotted-input producer resolution (M3); flatten universe = child ∪ parent(`_parentData`) ∪ builtins (M2). **A2** `CapabilityBinderV2.applyTransformFieldRenames` — applies the SAME rename map to `{kind:"ref"}` + `{{var.field}}` templates + bare `key_field` literals (M4), scoped by the reconciled schema (no second fuzzy match). **Gap B** `ExecutionGraphCompiler.reconcileTransformSchemaWithUpstream` refactored onto the shared helper + `rewriteConditionFieldLiterals` (bare `condition.field`) + dotted-input base-var fallback in `buildSchemaMap`. **Gap C** op-scoped plugin-truth warning (warning-by-default). FIELD-SYNTHESIZING ops untouched (WP-18 Bug A preserved). 27 tests (AC-1..AC-11). **Q4 out of scope** (calibration detector → field-fidelity calibration workplan). Live Phase E not runnable in the Dev env.

**Problem.** A generated `flatten` step declares its item `output_schema` with **normalized snake_case** field names (`mime_type`, `message_id`, …) while the runtime `transformFlatten` emits the source plugin action's **native** keys **verbatim** (Gmail `search_emails` → `attachments[].mimeType`, camelCase — `StepExecutor.transformFlatten` spreads `...child` with no rename). Downstream `filter`/`scatter` then reference the declared (non-existent) names → the filter matches nothing → `eligible_attachments` empties → the scatter iterates nothing → empty/degraded report. Also emits `broken_variable_reference` noise on the scatter `itemVariable`.

**Distinct from WP-56 (do not double-track).** WP-56 is the *iteration-variable wrong-name* case (LLM reuses a **container's** id field `folder_id` for **items** that expose `id`). WP-63 is the *transform-declared-schema re-casing* case (LLM declares a re-cased `mime_type` shape that its own camelCase plugin producer contradicts, planted into the canonical `WorkflowDataSchema` via WP-18 Bug A and never reconciled). Same **field-fidelity family**, different mechanism/phase. The in-progress field-fidelity work in [`V6_FIELD_FIDELITY_CALIBRATION_PHASE0_PHASE1_WORKPLAN.md`](../workplans/V6_FIELD_FIDELITY_CALIBRATION_PHASE0_PHASE1_WORKPLAN.md) is calibration-side (WP-56); WP-63's primary owner is generation-side (Phase 2 authoring).

**Evidence.** Agent `0ee53785-44d0-4b46-85dd-367551a657ba`, session `e5dd96fd-…`. Plugin truth `google-mail-plugin-v2.json` L408–435 (`mimeType`); runtime `StepExecutor.ts` L5064–5096 (verbatim `...child`, no rename); DSL step2 declares `mime_type`, step3 filters on `field:"mime_type"` with `_on_empty:"throw"`. **Re-surfaced during WP-62 live testing (2026-07-16):** the WP-62 live-test agent `2ffcd7bf` is the same Gmail scatter-attachment shape (`search_emails → flatten attachments → filter → scatter[get_email_attachment → extract]`); calibration flagged the flatten step (`step2`) missing/mis-shaped its required `field` (`attachments`) from `expense_emails` — the same class — confirming it is live and independent of the WP-62 routing fix.

**Fix shape.** (Gap A, primary) In Phase 2 `DataSchemaBuilder`, reconcile an `ai_declared` transform slot's item field names against the `produced_by` input slot's schema (plugin source of truth) **before** admitting it to `WorkflowDataSchema.slots` — never accept a re-cased/renamed shape that contradicts its producer. (Gap B) Extend `ExecutionGraphCompiler` O10a to bare `condition.field` literals + dotted-path input keys. (Gap C) A compile-time gate that validates any plugin-lineage declared `output_schema` field name against the producing action's real `output_schema`. Plus two `calibration` detector improvements (recognize scatter `itemVariable` in the broken-ref detector; flag flatten-declared-vs-plugin-actual field divergence).

**Why not caught earlier.** `ScatterItemFieldValidator` resolves element fields from the **stored** schema when the scatter source is a transform (not a plugin action), so the mutated snake_case schema *agreed* with the broken refs (schema-mutation trap); WP-18 Bug A lets an LLM-declared transform `output_schema` win for **any** op (incl. shape-changing `flatten`) without reconciliation; and no compiler/calibration pass compares a transform's declared item fields to the plugin's real item fields (Gap C — plugin-truth validation doesn't exist outside the narrow `x-variable-mapping` path). Calibration's dry-run runs on real `mimeType` data but the throw-guard (`inputLength > 0`) is bypassed by an empty upstream, so the emptiness never raises.
...
| Date | Change | Details |
|------|--------|---------|
| 2026-07-16 | **WP-63 ✅ Fixed (uncommitted)** — `flatten` field-shape fidelity reconciled in reliable code. | Shared helper `field-name-reconciliation.ts` (single normalizer + M1 collision guard). Gap A `DataSchemaBuilder` Pass 2d reconciles FIELD-PRESERVING transform + loop item_ref item schemas to the producer's real casing (M2 flatten universe child∪parent∪builtins; M3 dotted input). A2 `CapabilityBinderV2.applyTransformFieldRenames` rewrites all ref shapes from the one rename map (M4, no second fuzzy match). Gap B compiler O10a refactored onto the shared helper + dotted-input + bare `condition.field`. Gap C op-scoped plugin-truth warning. Synthesizing ops untouched (WP-18 Bug A preserved). Q4 (calibration detector) out of scope → field-fidelity calibration workplan. 27 tests (AC-1..AC-11); v6 suite unchanged at the 43-failure baseline (passed +27), zero `console.*`. Live Phase E not run in Dev env. Branch `agent-failure-troubleshooting`. |
| 2026-07-16 | **WP-63 filed (⬜ Not started)** — `flatten` field-shape fidelity (declared snake_case item schema vs plugin native camelCase). | Logged from the standalone RCA `AGENT_RCA_CONCLUSION_gmail-expense-attachment-flatten.md` (agent `0ee53785`). Re-surfaced during WP-62 live testing (agent `2ffcd7bf`, same Gmail scatter-attachment shape: flatten step missing/mis-shaped required `field`). **Distinct from WP-56** (iteration-variable wrong-name) and **from WP-62** (extraction routing) — not folded into either. Owner `v6-pipeline`: Phase 2 `DataSchemaBuilder` reconcile `ai_declared` transform item fields to the plugin producer (Gap A, primary) + compiler O10a widening (Gap B) + a plugin-truth validation gate (Gap C). |
| 2026-07-16 | **WP-62 committed (`05eca70`)** + hotfix — live validation exposed a param-granularity gap; WP-62 was incomplete, not wrong. | Live run on agent `2ffcd7bf` proved the routing fix works (5 phases green, `document-extractor` bound, IC persisted, Phase-1 `source` emitted, CC-3a split correct; all six WP-62 components regression-tested by TS, none defective) — but calibration correctly blocked `step6`: *"Parameter file_content should be string, got object"*. Cause: `convertExtract`'s `x-input-mapping` loop copied a whole-object `extract.input` **verbatim** into the `type:"string"` `file_content` param (latent pre-WP-62 — that shape used to reroute to AI). **Fix:** bytes-field navigation in reliable code (idempotent, schema-driven via shared `bytesFieldOf`, safe-direction reroute to AI when no bytes field resolves) — extending WP-62's thesis from *routing* to *ref granularity*; `BYTES_FIELDS` de-duplicated 3→1; plugin def gained `from_base64_content: "data"` (**kept** `from_file_object: "content"` — contrary to the RCA it is NOT dead code and is load-bearing for the WP-57 Drive fallback); `InputMapping` type gained `from_base64_content`. +8 tests → 50 WP-62 tests pass; v6 suite unchanged at the 43-failure baseline; pilot/plugin suites 547 pass. **Why fixtures missed it:** they encoded the same `.data` assumption as the code under test → new anti-pattern added to V6_DESIGN_PRINCIPLES § 2. RCA: `docs/investigations/AGENT_RCA_CONCLUSION_wp62-live-validation-file-content-object.md`. **Live Phase E re-validation still required.** |
| 2026-07-13 | **WP-62 ✅ Fixed (uncommitted)** — deterministic-vs-AI extraction routing moved out of Phase-1 phrasing into reliable code; AI net preserved. | New coverage predicate `ExtractionCoverage.evaluateExtractionCoverage` (CC-1..CC-4/CC-3a, schema-driven, connected-only, no field-name/plugin-identity lists). `CapabilityBinderV2` Phase 2c (`routeExtractionCoverage`) authors the covered/uncovered verdict (Q1: binder decides); `IntentToIRConverter.convertExtract` honors it and auto-synthesizes a downstream `generate` for the meta/computed residual (Q3 split, the `0ee53785` shape). Companions B1 (Phase-1 `source` steer), B2 (`get_email_attachment` `x-semantic-type: file_attachment`), B3 (`inputLooksLikeFileAttachment` loop-internal + producer-shape hardening). **R1 materialized** → added optional `ExtractFieldSource` grammar field (the schema-driven mitigation, never a name list). 28 unit/scenario tests incl. 5×-repeated determinism (AC-1) + uncovered→AI (AC-2). Live Phase E deferred to QA (no credentials in the Dev self-test env). RCA: `docs/investigations/AGENT_RCA_CONCLUSION_gmail-expense-attachment-ai-extract.md`; workplan: `docs/workplans/DETERMINISTIC_VS_AI_EXTRACTION_ROUTING_WORKPLAN.md`. Branch `agent-failure-troubleshooting`. |
| 2026-07-12 | **WP-58 ✅ Fixed** — folded into requirement Item 11 (Batch 3, sub-phase 3A). | AI/processing step inside a scatter now RECEIVES the loop variable its instruction references (was prose-only → blank From/Subject/Filename columns on `0ee53785` step7). Shared deterministic detector `lib/agentkit/v6/compiler/ai-input-context.ts` reused by 3 call sites: root-cause populate of `AIConfig.additional_inputs` in `IntentToIRConverter.convertGenerate` (new `loopItemVarStack`), labelled-object injection in `ExecutionGraphCompiler.compileAIOperation` (with detector fallback for legacy IR — Principle 4), and the in-place `scripts/fix-ai-scatter-loopvar-input.ts` (dry-run default; `AgentRepository.updatePilotSteps`). Content-asserting + scoping + negative + genericity tests. Corrected the diagnosis's named resolver: active path is `compileAIOperation`, not `AIOperationResolver`. Branch `agent-failure-troubleshooting`; uncommitted. Deferred (SA §4): Phase-1 IC config/aggregate input declaration + dead aggregate-step cleanup; always-on calibration-corrector generalisation. |
| 2026-07-05 | **WP-61 filed + fix implemented (uncommitted)** — search subject dropped by `mapParamsToSchema` → `research_topic` compiles with `params:{}` → runtime "Topic is required" → empty digest. | Surfaced live on agent `df67bf69`. Sibling of the Sheet1 range case (EP required-plugin-param class). §5A: prompt-only structuring non-deterministic in both v15 (4/5) + v16 (3/5) → deterministic downstream fix. **Fix:** P3.2 `bindSearchSubjectToRequiredParam` (converter — bind subject→single unbound required string param, plugin-agnostic, 9/9 tests) + P3.1 `detectEmptyRequiredParams` (compiler warn, O16 sibling) + B5 (convertDataSource parity warning). Deferred: P3.1 gate escalation, formal regression scenario, P3.4 output-contract. RCA: `docs/investigations/V6_RCA_df67bf69_missing-topic.md`. Cycle: `docs/workplans/EP_PRODUCTION_SHEETS_RANGE_FIX_WORKPLAN.md` Part 3. |
| 2026-06-17 | WP-60 — layered A+B fix **implemented** | A (compiler stem match) ✅ +7 tests; **B (runtime param-injector) ✅** `lib/pilot/injectUnboundActionParams.ts` +12 tests — run after `reconcileInputsToDsl` in `WorkflowPilot.execute()`; targets the step by the namespaced key's plugin + action schema (stem param pick), conservative single-match injection, backward-safe, in-memory per execution. Repairs existing agents (incl. `48d587d4`) on next execution without a rebuild. Also fixed a stale QA integration test (the regenerated scenario fixture now carries an exact `folder_id`; the test now reconstructs the unmet shape). Uncommitted. |
| 2026-06-16 | **WP-60 filed** (P1) — compiler param-binder leaves `list_files.folder_id` unbound when only `folder_link` is in config → Drive-root listing | Surfaced live on freshly-created agent `48d587d4` in batch calibration. `findBestConfigMatch` token-Jaccard `folder_id`↔`folder_link` = 0.333 < 0.4 threshold → unbound → root. Distinct from WP-57 2B (which assumes a `folder_id` binding exists). Fix: stem-aware matching (strip `_id`/`_link`/`_url`) in the compiler binder → emits `folder_id: {{config.folder_link}}`, executor `extractDriveId` converts URL→ID. **Requires DSL rebuild** for existing agents; optional runtime schema-aware param-injector repairs them without rebuild. |
| 2026-06-16 | **WP-57 ✅ Fixed** + WP-58/WP-59 filed | Option A delivered; Phase E live PASS (agent `6ef48513`, 2026-06-15): real email received, folder routing (2B) + Textract extraction of 4 files incl. a PNG verified. Final commits `85d63bd` (2B-1 executor URL→ID), `21a416b` (2B-2 `reconcileInputsToDsl` + 2C regen). 2A′ binder fix proved **moot** (compiler auto-insert + §6.4 nudge produce `download_file → document-extractor` without it). Split out two follow-ups: **WP-58** (P3, multi-input AI/`generate` steps bind only one input — benign in practice, runtime config scope + LLM re-derivation compensate) and **WP-59** (P2, document-extractor deterministic field-quality — `tax_amount`/`due_date`/`invoice_number` mis-parsed on real receipts; `use_ai` path not implemented so it's real extractor-engine work). Also a small compiler cleanup: `IntentToIRConverter` now translates the IR's `deterministic` flag to the plugin's real `use_ai` param (or drops it) instead of emitting a no-op param — removes the Phase-A "Unknown parameter" warning. |
| 2026-06-10 | WP-57 filed (documented; fix Option A planned) — Google Drive → document-extractor has **no base64 byte source**: `read_file_content` `.text()`s binary files (corrupts PDFs/images) and the O-WP12 field-name heuristic misclassifies its `content` output as text → extraction silently fails / wrong-data email. | Surfaced live on agent `6ef48513` ("Google Drive Invoice Summary Email Agent"), captured AS-IS in `tests/v6-regression/scenarios/drive-invoice-summary-extractor-asis`. Fix on branch `fix/v6-drive-extractor-flow`: new `google-drive.download_file` (base64 + `x-semantic-type: file_attachment`) so the converter keeps document-extractor; shape `list_files → download_file → document-extractor`. Inverse of WP-12; related WP-34/53/56. |
