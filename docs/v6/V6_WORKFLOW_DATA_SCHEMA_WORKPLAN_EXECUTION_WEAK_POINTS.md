...
| Date | Change | Details |
|------|--------|---------|
| 2026-06-17 | WP-60 — chose layered A+B fix | A (compiler stem match) ✅ implemented + 7 tests; B (runtime param-injector keyed on the namespaced `plugin`+`capability`, schema-first param pick w/ stem fallback, conservative single-match injection) **designed, not yet implemented** — repairs existing agents (incl. `48d587d4`) without a rebuild. Documented in WP57 summary + this WP. Implementation pending. |
