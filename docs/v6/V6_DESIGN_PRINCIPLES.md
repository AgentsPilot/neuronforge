...
- [WP-39](./V6_WORKFLOW_DATA_SCHEMA_WORKPLAN_EXECUTION_WEAK_POINTS.md): ExecutionGraphCompiler's D-B18 alias logged `"Aliased 'select' → 'map'"` but only updated `config.type`, not `step.operation`. Compiler logs claimed success while emitting a step that the runtime rejected at execution time. Anyone reading the alias log would assume the conversion was complete and look elsewhere for the failure cause.
