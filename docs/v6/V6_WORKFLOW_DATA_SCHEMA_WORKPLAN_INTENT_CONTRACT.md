...
> **Status**: ✅ All issues resolved — 15 issues fixed (I1, I2, I4, I5, I6, I8, I9, I10, I11, I12, I13, I14, I15), I3 deferred (runtime), I7 auto-resolved by I6, O6+O7 implemented
> **Date**: 2026-03-13
...
| O8 | **Config value validation — user-provided config values may contain invalid plugin-native syntax** — Even when config keys resolve correctly (O7), the VALUES themselves may be invalid for the target plugin. Example: `gmail_search_query` contains pseudo-natural-language (`"subject include: Invoice or Expenses or Bill and has:attachment filename:pdf"`) instead of valid Gmail search syntax (`"subject:(Invoice OR Expenses OR Bill) has:attachment filename:pdf newer_than:1d"`). Additionally, related config values may not be composed — `scan_time_window: "last 24 hours"` exists as a separate config but is never folded into the Gmail query as `newer_than:1d`. This affects any config value passed directly to a plugin parameter where the plugin expects domain-specific syntax. **Options:** (A) Enhanced prompt phase normalizes values before they become config — a pre-processing step translates natural-language values to plugin-native syntax using vocabulary parameter hints (format, enum, description). (B) LLM rewrites values during IntentContract generation using vocabulary hints — but this partially violates Architecture B (LLM declares *what*, deterministic phases handle *how*). (C) Deterministic query builder per plugin — a post-compilation transform that takes semantic intent (`search emails about invoices from last 24h`) and produces native syntax (`subject:(Invoice OR Bill) has:attachment newer_than:1d`), composing related config values automatically. | Config quality / plugin compatibility |
