...
> **Status**: Complete (all 6 phases implemented). ⚠️ **Task 7.3 flagged interim (2026-05-06)** — auto-repair hides the upstream WP-15 grammar gap; retire after [WORKPLAN_INTENT_CONTRACT.md](./V6_WORKFLOW_DATA_SCHEMA_WORKPLAN_INTENT_CONTRACT.md) tasks 0.4–0.6 + 2.11 land and regression shows 0 firings.
...
| 7.3 | **Auto-repair shallow AI-declared schemas in Phase 3 validation** — The LLM non-deterministically omits `properties` on object-typed slots or `items` on array-typed slots in data_schema, causing `validateDataSchema()` to hard-fail and kill the pipeline. Since the depth enforcement was added in task 3.8, any LLM run that produces a shallow schema is a total pipeline failure with no recovery. **Fix:** Change `validateSchemaDepth()` from error-and-throw to warn-and-repair: (a) `type: "object"` without `properties` → auto-set `properties: {}` (permissive, allows any fields at runtime), (b) `type: "array"` without `items` → auto-set `items: { type: "any" }`. Both cases log as warnings (visible in dev.log) but no longer block the pipeline. **⚠️ INTERIM (added 2026-05-06):** This safety net hides an upstream LLM grammar/prompt gap — the IntentContract `generate.outputs[]` / `extract.fields[]` schema today has no slot for `items` (arrays) or `properties` (objects), so the LLM physically cannot declare depth even when its `instruction` enumerates the fields. Tracked as [WP-15](./V6_WORKFLOW_DATA_SCHEMA_WORKPLAN_EXECUTION_WEAK_POINTS.md#wp-15-ai-declared-output-slots-lose-item-level-shape). Once [WORKPLAN_INTENT_CONTRACT.md](./V6_WORKFLOW_DATA_SCHEMA_WORKPLAN_INTENT_CONTRACT.md) tasks 0.4–0.6 + 2.11 land and regression on the 10 scenarios shows 0 firings of `validateSchemaDepth` warn-and-repair, change `validateSchemaDepth()` back to error-and-throw and remove the auto-fill code paths. Removing it before the upstream fix will re-introduce hard pipeline failures. | `lib/agentkit/v6/semantic-plan/IRFormalizer.ts` | Done (interim — schedule retirement post-WP-15) |
...
| Date | Change | Details |
|------|--------|---------|
| 2026-05-06 | Task 7.3 annotated as interim — pending WP-15 root-cause fix | `complaint-email-logger` regression review surfaced that the auto-repair safety net was masking a structural gap in the IntentContract grammar (no `items` / `properties` slot on `generate.outputs[]` and `extract.fields[]`). Don't remove 7.3 before the upstream fix lands — see [WP-15](./V6_WORKFLOW_DATA_SCHEMA_WORKPLAN_EXECUTION_WEAK_POINTS.md#wp-15-ai-declared-output-slots-lose-item-level-shape) and [WORKPLAN_INTENT_CONTRACT.md](./V6_WORKFLOW_DATA_SCHEMA_WORKPLAN_INTENT_CONTRACT.md) tasks 0.4–0.6 + 2.11 for the layered fix and retirement criteria. |
