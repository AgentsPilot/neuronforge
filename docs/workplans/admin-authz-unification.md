...
## Slice status — updated 2026-09-21

> ## ⚠️ READ THIS BEFORE ANY SLICE SECTION BELOW
>
> Slices **0, 1, 2, 3, 5, 6 and 8 are DONE.** Slices **1L, 4 and 7 remain PARKED** (user decision, 2026-09-20; 2/3/5 were un-parked on 2026-09-21 and completed). A section below describing a parked slice is a **plan that was not executed** — retained as the decision record, not as work in flight.
...
| **1L** Logging conformance | 🔴 **PARKED** | 236 `console.*` calls across the files slices 1–3 touched — inventory in the Change History |
| **2** Stop cross-tenant reads | ✅ **Done 2026-09-21** | — 23 handlers gated (14 cross-tenant incl. 3 `HEAD` probes + 9 internal-config GETs) |
| **3** Customer-visible config reads | ✅ **Done 2026-09-21** | — 4 catalogue GETs gated; `reward-config` replaced by `GET /api/rewards/agent-sharing` in the same commit |
| **4** One gate, one implementation | 🔴 **PARKED** | 7 handlers still hand-roll the admin check. **Correct, but duplicated** — so "one way to validate" is still not literally true in use |
| **5** Server guard for `/admin` pages | ✅ **Done 2026-09-21** | — all 21 pages guarded by inheritance; none of them edited |
| **6** CI gate | ✅ **Done** | Now a **required status check** on `main` (`enforce_admins`, `strict`) |
| **7** Retire admin management surface | 🔴 **PARKED** | The Settings screen still exists: **gated but non-functional**, writing to a store that grants nothing |
| **8** Correct the documentation | ✅ **Done**, re-verified 2026-09-21 | — |

### What is closed, and what is honestly not

**Closed:** all **72** admin handlers require an admin (**65** via `requireAdmin`, 7 via an equivalent hand-rolled check), **0 open**; all **21** `/admin` pages are guarded on the server; the guard is a required status check, so a new ungated surface cannot merge.

**Not closed, and not claimed:**

- **"One way to validate" is still not literally true in use** — 7 correct-but-inline copies remain (slice 4).
- The surface is **gated, not isolated**: most admin routes still construct a service-role client directly, bypassing RLS and the repository layer (OI-9 / access-doc item 9).
- **26 of 44** admin route files still leak a raw error `.message` to the client with no `NODE_ENV` guard (OI-19).
- R1 proves a gate is **present**, not that it runs **first** (OI-20), and nothing forces a stale exemption to be deleted (OI-22).

> **The ratchet fired as designed.** Gating 27 handlers and guarding the pages forced **R1 34 → 7** and **R6 1 → 0** in the same commit, and every document that publishes those numbers moved with them. That is the mechanism working, not drift.
...
| 2 | `app/api/admin/audit-trail/route.ts` | `GET` — ✅ **done 2026-09-25** outside this programme: admin reorganisation slice 2c had to change the route (account filter), so it moved to `requireAdmin` and R1/R2 caps went 7 → 6. **Slice 4 now has 6 files.** OI-18 (`.from('users')`) is still open in it |
...
| **OI-25** | **Nothing enforces the PROSE counts the way the caps enforce the exemption counts.** Now that the guard is a required status check, the weak link has moved. The caps are asserted by equality, so an exemption list cannot silently drift — but `CLAUDE.md` and the access doc publish figures ("72 = 65 + 7 + 0", "all 21 pages guarded", "a red check blocks the merge") that **only a human keeps true**. **D7-2, D7-3 and QA Finding 1 are the first instances of exactly this gap, and all three appeared in the very first commit after enforcement went live** — one doc still called the gate advisory hours after it became mandatory; two present-tense claims still said 38 when the number was 65; and **the guard's own header still described 27 open handlers and a parked slice 2 in the commit that closed them** — the artefact a reviewer opens when the now-required check goes red. Three documents drifted in one commit, each in the conservative direction, none caught by anything but a human reading them. **A fourth instance in the same commit, on the same theme:** Dev published a repo-wide `tsc` error delta that QA could not reproduce at any scope — it came from build-cache artefacts in a reused worktree, not from the change. The durable claim, measured directly, held: **0 new type errors and 0 errors in touched files.** The withdrawn figures are deliberately not repeated here, because a number written down is a number someone will later quote. Counts in prose decay the moment they are written, whether they describe the tree or a tool run — which is the whole argument for having CI derive them. | **New guard task, the natural successor to OI-22:** have the guard assert the published figures the same way it asserts the caps — re-derive the census and fail the build when a document states a number that no longer matches the tree. Do NOT build it in this PR. |
| ~~**OI-24**~~ | ~~`CLAUDE.md:228` names `AdminAccessService` as the route-level instruction~~ **✅ CLOSED 2026-09-21, at the user’s request, in this PR.** The Security Rules row now names `requireAdmin` / `requireAdminPage` as the route and page gate, keeps `admin_users` via `AdminAccessService` as the **source of truth**, names `public.is_platform_admin()` as the SQL-side predicate, and states that a direct `AdminAccessService` call inside a `route.ts` **fails the CI guard**. Kept terse deliberately: it is a rules table, and duplicating the detail from the Key Documentation row would itself have created a second source of truth. | **Closed.** The sibling instance in `bos-llm-call-standards/SKILL.md` (OI-23) stays open — outside these files. |
...
## SA Code Review — Slice 2 + 3 + 5

**Reviewed by SA — 2026-09-21**
**Status:** ✅ **APPROVED WITH CHANGES** — 4 defects, all small, **three of them stale published numbers that the required status check now makes load-bearing.**
**Scope:** 34 modified + 4 new, reviewed via `git diff HEAD` on `feature/admin-authz-close-remaining`.

Every claim re-derived, not trusted. **All of Dev's claims are true.** The application code is clean — no defect in any of the 27 newly gated handlers, the page guard, or the reward projection. All four defects are in a test file and in the documents.

### Claim verification

| # | Claim | SA verification |
|---|---|---|
| 1 | 72 = 65 `requireAdmin` + 7 inline + **0 open** | ✅ **Re-derived with my own parser: `TOTAL 72 = requireAdmin 65 + inline 7 + OPEN 0`.** `OPEN: NONE`. |
| 2 | Gate precedence — 0 of 65 with a side effect before the gate | ✅ **Swept all 65 independently** for `request.json(`, `req.json(`, `.from(`, `searchParams`, `new URL(`, `listUsers(`, `.rpc(` occurring before `requireAdmin(`. **`PRECEDENCE VIOLATIONS: NONE`.** |
| 3 | Caps ratcheted R1 34 → 7, R6 1 → 0 | ✅ Counted directly: `R1_PARKED 7 / R1_PERMANENT 0`, `R2 7/1`, `R3 0`, `R4 2`, `R5 0`, `R6 0/0` — and `CAPS` matches each exactly, so the equality test is genuinely satisfied rather than loosened. R1's residual 7 are the inline copies (slice 4 parked), which is the correct reading of that cap. |
| 4 | Guard 74/74, `app/api/admin lib/admin` 508/508 | ✅ Ran both: guard **74/74**; `app/api/admin lib/admin app/api/rewards` **508/508 across 13 suites**. |
| 5 | `settings/platform-users` — the weaker check **replaced**, not layered | ✅ **Verified two ways.** The diff deletes the `getUser` import and the entire 401-only block, substituting the gate; and `grep` for `getUser|isAdmin|Unauthorized` in the file returns **nothing**. No residual path. |
| 6 | Three `HEAD` probes gated, not allow-listed | ✅ `dashboard:274`, `users:159`, `token-usage:210` each call `requireAdmin` before returning the empty 200. The existence-probe that E-4 identified is closed, and R1's allow-list no longer carries them. |
| 7 | 72-row register regenerated | ✅ **Set-equal to the tree in both directions** — 72 rows, contiguous 1–72, `tree−doc: []`, `doc−tree: []`, classifying 65 gated / 7 inline / **0 open**. |

---

### (a) The `reward-config` replacement — **approved, and better than what I specified**

Both of my rejections are respected: no mirror in `system_settings_config`, not routed through `/api/system-config`, **no migration**. And the shape improves on the "projection" I asked for.

**The repository method is the right place for it.** `ConfigRepository.isRewardActive()` selects `is_active` for one key and returns a boolean. The reasoning in its docstring — *"A projection you have to remember to apply is one you can forget"* — is the actual argument: a route-level projection over `getRewardConfig` would have pulled amounts and caps into memory and trimmed them on the way out, which works until someone adds a field or a log line. This way the ruleset is never fetched. That is a structural answer to a discipline problem, which is the distinction this whole programme has been about.

Three further things I checked rather than assumed:

- **Atomic.** The gate on `GET /api/admin/reward-config`, the new route, and both page repoints are in the **same change set**. There is no intermediate state where the indicator is dark. RC-11 satisfied.
- **Leaks nothing beyond the boolean.** The response is `{ success: true, data: { isActive } }`; the repository's `select` is `is_active` alone. **`REWARD_KEY` is a module constant, not a query parameter** — correct and load-bearing: a caller-supplied key would have turned a single-fact read back into the enumeration oracle that `/api/system-config` already is (OI-15).
- **Both pages still work, with their fallback intact.** `app/(protected)/agents/[id]/page.tsx:405` and `app/v2/agents/[id]/page.tsx:360` now read `result?.data?.isActive === true` — the same *"anything but an explicit true hides the prompt"* semantics as before. The v2 page keeps the change inside the existing `Promise.all`, so no extra round trip. Failure behaviour is consistent at all three layers: repository returns `false` on error, the route returns a 200 with `false` rather than a 500 (correct — the caller is deciding whether to render a prompt, and "no prompt" is the safe answer to "I could not find out"), and the pages' `catch` sets `false`.

**One observation, not a defect.** `GET /api/rewards/agent-sharing` is unauthenticated. Both callers are pages that already require a session, so requiring `getUser()` would cost nothing and would be marginally tighter. What it would protect is close to nothing — one boolean about a platform-wide incentive — so I am not asking for it. Recording it so the next reader sees the choice was noticed rather than missed.

### (b) The pages guard — **approved; the indistinguishability claim holds and inheritance is genuinely total**

**Inheritance covers all 21, and I verified the ways it could fail rather than trusting the shape:**

| Check | Result |
|---|---|
| `page.tsx` under `app/admin/` | **21** — all descendants of the single guarded layout |
| `route.*` under `app/admin/` | **0** — E1 closed; guard rule R3 keeps it so |
| Nested `layout.tsx` below the root | **0** — nothing can interpose |
| Route groups / parallel routes (`(…)`, `@…`) | **none** — no second valid position for a page |
| `page.tsx` files edited by this diff | **none** — which is the point |

**AdminChrome is verbatim.** I diffed it against `HEAD:app/admin/layout.tsx` with the component name normalised: the only differences are the added docblock, the two import paths adjusted for the deeper directory, and a trailing newline. **No markup, no state, no behaviour change.** A reviewer can confirm that in one command.

**The indistinguishability test is real, and the fixture trap is handled exactly right.** The test compares the same `outcome()` helper for an anonymous caller and an **onboarded** non-admin and asserts `toEqual`. The header documents why the fixture is onboarded — `middleware.ts` runs first and sends a non-onboarded user to `/onboarding-chat` — and, crucially, states *why that is not a disclosure* and why the response is to **fix the fixture, not weaken the assertion**. That was my W-5 finding; it has been implemented with the reasoning attached rather than the symptom suppressed.

I also checked the guard itself rather than only its test: both denials reach the *same* `redirect(NON_ADMIN_REDIRECT)`; the only asymmetry is a **server-side log line** for the signed-in case, which is not in the response. `redirect()` sits **outside** both `try/catch` blocks, which are scoped narrowly to the `getUser` and `isAdmin` calls. And **making the `redirect` mock throw** is the detail that gives the suite teeth — a guard that logs a redirect and falls through now fails, which a recording mock would have passed.

> *Completeness note, not a defect:* a signed-in non-admin costs one extra `admin_users` round trip, so the two cases differ in **timing**. BQ-3 is about what a prober can conclude from the response, and the responses are identical. Out of scope; noted so it is a known limit rather than an unexamined one.

### (c) The documents — **the inversion is handled, with three stale numbers**

The risk did invert, and Dev anticipated it. The headline now reads *"unified for enforcement, and still duplicated in implementation"*, and **"What is NOT true" grew from five entries to six** while the work was being completed — which is the opposite of the usual direction. All three caveats you asked me to confirm survive, explicitly:

- *"One way to validate"* → **"Still not literally true in use"** — 7 hand-rolled checks; *"until slice 4 lands, 'one way' describes the standard, not the codebase."*
- *"Fully hardened"* → **"Gated, not isolated"** — service-role clients bypassing RLS and the repository layer, plus the 26-of-44 raw error-message leak.
- Settings screen → **"non-functional and gated… Never describe this as retired."**

Plus two that guard against the *new* failure mode: *"the guard proves every gate actually runs"* (presence, not precedence) and *"the published counts cannot drift"* (they can still drift **conservatively** — OI-22). Adding that last one unprompted is the right instinct: it names the way this document could become wrong next.

**Does anything overclaim completeness? No — with the exception of the three stale figures below, which all err in the *conservative* direction.** Every ✅ in "What is true" is earned and checkable: I confirmed 0 open, 21 pages guarded by inheritance, and the required-status-check state. The workflow header and the `CLAUDE.md` row both carry the ✅ *and* the ⚠️.

---

### Defects

> **D7-1 — LOW — six type errors introduced in the oracle. `tsc` rejects them; `ts-jest` does not.**
> `app/api/admin/__tests__/adminGate.writes.test.ts:282, 284, 287, 290, 299, 300` — `error TS2554: Expected 0 arguments, but got 1`.
>
> Six handlers are correctly declared `export async function GET()` with a generated correlation id (a deliberate, commented choice — right, since there is no request to propagate one from). The oracle calls them as `agentGenerationConfig.GET(req(…))`, `backfillEmbeddings.GET(req(…))`, `memoryConsolidation.GET(req(…))`, `uiConfig.GET(req(…))`, `executionTiers.GET(req(…))`, `storageTiers.GET(req(…))`. **JavaScript discards the extra argument, so the tests genuinely exercise the handlers and pass** — but the call sites do not type-check.
>
> This is exactly the class Dev flagged and caught once (the duplicate import); it slipped six times in the same file. No runtime or security impact, and `next.config.js` ignores TS errors at build — but **CLAUDE.md § Build is explicit: *"TypeScript errors ignored — fix them anyway"***, and this is the programme's regression oracle. Fix: drop the `req(…)` argument on those six lines.
>
> *(Verified separately that `app/api/admin/token-usage/drill-down/route.ts:220-223` — the only other in-scope file with errors — is **pre-existing**: Dev's diff touches only the import and the gate block at ~93-104.)*

> **D7-2 — MEDIUM — `CLAUDE.md` and the access doc now contradict each other on whether the CI check is enforced.**
> `CLAUDE.md:781` still reads: *"it only blocks a merge once `Admin authz surface guard` is a required status check on `main`, which is a manual repo setting; **until then a red check does not stop an ungated admin route from merging**."*
>
> The access doc (`:56`, `:183`) states it **is** now a required status check with `enforce_admins` and `strict`. Both are canonical documents; they disagree on the single most load-bearing fact about the gate. The error is conservative — it understates enforcement — but a reader deciding how much to trust a green run is sent in two directions. **Update the `CLAUDE.md` clause to match.**

> **D7-3 — LOW — two stale "38" figures survive the re-flip, in current-state claims.**
> `docs/admin/ADMIN_IDENTIFICATION_AND_ACCESS.md:190` — *"All **38** gated handlers are correct today"* — and `:402` (Open Item 8) — *"Applies to all **38** gated handlers."*
>
> Both should read **65**. They sit in the "Known gaps in the guard itself" table and the open-items register, i.e. present-tense statements, not history. Dev regenerated the numbers everywhere else; these two are inside prose that was otherwise unchanged. The direction is conservative, but the access doc's own preamble tells readers to trust the measured figures — so a stale one costs more here than elsewhere. *(All other `27` / `34` / `38` occurrences I swept are legitimately historical — change-history rows, "this slice gated 27 handlers", and my own earlier review text.)*

> **D7-4 — TRIVIAL — `CLAUDE.md:781` "Last Updated" cell still says `2026-09-20`** although the row content was rewritten today. Doc-convention slip; the date is how a reader judges whether to re-measure.

---

### Why these matter more than they would have last week

The guard is now a **required status check with `enforce_admins: true` and `strict: true`**. Three consequences worth stating once, because they change the cost of a stale number:

1. **A wrong cap blocks the merge for everyone**, including PRs unrelated to admin authz. That is the ratchet working — and this branch is the proof: gating 27 handlers forced R1 34 → 7 and R6 1 → 0 *in the same commit*, because leaving them stale would have failed the build.
2. **`strict: true` means the branch must be up to date with `main` before merging.** `main` has moved eight times today, and with equality caps **the guard's result is a function of the whole tree, not of this diff** — so re-run it after the final rebase, not before.
3. **Documentation is now the weakest link, not code.** The caps enforce the *exemption* counts; nothing enforces the *prose* counts in `CLAUDE.md` or the access doc. D7-2 and D7-3 are the first instances of exactly that gap, appearing in the first commit after the check went live. Worth considering — not now, not in this PR — whether the guard should assert the published figures the way it asserts the caps. That would be the natural successor to OI-22.

---

### Out of scope — confirmed not pulled in

Verified this branch does **not** touch slice 4 (the 7 inline copies remain, correctly still on R1/R2's parked lists), slice 7, R7, OI-20 or OI-22. **236 `console.*` calls across 17 touched files were inventoried and none converted — correct**, per the user's parking of 1L/2L, and I am not treating it as a defect. The three `console.log` lines in `reward-config`'s gated GET and the `console.error` in the v1 agent page are part of that parked set.

### Code Review Comments

| # | Location | Issue | Priority |
|---|---|---|---|
| 1 | `app/api/admin/__tests__/adminGate.writes.test.ts:282, 284, 287, 290, 299, 300` | **D7-1** — six `TS2554` errors: request passed to zero-parameter handlers. Jest green, `tsc` red. | Low |
| 2 | `CLAUDE.md:781` | **D7-2** — contradicts the access doc: says the check does not block a merge; it now does. | **Medium** |
| 3 | `docs/admin/ADMIN_IDENTIFICATION_AND_ACCESS.md:190, :402` | **D7-3** — "38 gated handlers" should be **65**, in present-tense claims. | Low |
| 4 | `CLAUDE.md:781` | **D7-4** — "Last Updated" still `2026-09-20`. | Trivial |

> 🔺 **[QA 2026-09-21 — correction to comment 1, recorded not rewritten.]** The **fix** is right and was applied: passing an argument to a handler that declares no parameters is wrong regardless. But the **diagnosis** does not reproduce. QA measured `TS2554` at **16 on both `main` and this branch, none of them in an admin file**, and `adminGate.writes.test.ts` **clean on both sides**. So "Jest green, `tsc` red" was not true of these six sites, and Dev's corroborating delta turned out to be build-cache artefacts in a reused worktree. SA's text is left intact above because a review record should show what was actually claimed; this note is the correction. The claim that holds, measured directly: **0 new type errors, and 0 errors in touched files.** See **OI-25**.

### Code Approved for QA: **Yes.**

**No application-code change is required** — the 27 gated handlers, the page guard, the chrome split and the reward projection are all approved as they stand. D7-2 and D7-3 should be fixed **in this PR**: they are one-line documentation edits, and leaving a document that tells readers the CI gate is advisory when it is mandatory undercuts the thing this PR delivers. D7-1 and D7-4 are trivial and may ride along. **Rebase onto `origin/main` and re-run the guard as the last step before merge.**
...
| Date | Change | Details |
|------|--------|---------|
| 2026-09-21 | **SA review fixes (D7-1 … D7-4) + OI-24 closed** (Dev) | **D7-1**: six zero-parameter `GET()` handlers were being called with a request argument. Removed — passing an argument to a function that declares none is wrong and misleading regardless of whether a given tsconfig surfaces it. ⚠️ **Correction (QA, 2026-09-21):** the accompanying claim that this made `tsc` red was **not reproducible** and is withdrawn — QA measured `TS2554` at 16 on both sides, none in an admin file, and `adminGate.writes.test.ts` clean on `main` *and* on the branch. The claim that does hold, measured directly: **0 new type errors, 0 errors in touched files** (55 on `main`, 55 on the branch), and all four new files contribute zero. A delta I reported from a scoped run turned out to be build-cache artefacts in the primary worktree, not a result — recorded because publishing an unreproducible number is the same failure mode this programme keeps legislating against. **D7-2**: `CLAUDE.md` still described the guard as advisory ("until then a red check does not stop an ungated admin route from merging") hours after the user made `Admin authz surface guard` a required status check — two canonical docs contradicting each other on the gate’s most load-bearing fact. **D7-3**: two present-tense "all **38** gated handlers" → **65**. **D7-4**: Last Updated → 2026-09-21. **OI-24 closed at the user’s request**: `CLAUDE.md`’s Security Rules row named `AdminAccessService` as the route-level instruction — the pattern R2 now **fails the build on**. It now names `requireAdmin`/`requireAdminPage` as the gate, keeps `admin_users` as the source of truth and `public.is_platform_admin()` as the SQL predicate, and stays terse so it does not become a second source of truth. **OI-25 logged**: the caps enforce the exemption counts, but nothing enforces the **prose** counts — D7-2/D7-3 being the first instances, in the first commit after enforcement went live. |
| 2026-09-25 | **Slice 4 shrinks to 6 files** (Dev, admin reorganisation slice 2) | `audit-trail#GET` converted to `requireAdmin` as part of admin reorganisation slice 2c, because that slice changed the route. R1/R2 parked entries removed, both caps 7 → 6 in the same commit. The other six inline copies are untouched and slice 4 stays parked. |
| 2026-09-21 | **Slices 2, 3 and 5 delivered — the admin surface is closed** (Dev) | **27 open handlers gated, all 21 `/admin` pages guarded, 0 open.** Census re-derived independently and reconciles exactly: 23 (slice 2) + 4 (slice 3) = 27; slice 2’s 14 cross-tenant = 11 route names + the 3 `HEAD` probes. **Slice 2**: `users`(+HEAD), `users/[id]/stats`, `onboarding-users`, `settings/platform-users` (its weaker signed-in-only check REPLACED, not layered), `token-usage`(+HEAD), `token-usage/drill-down`, `token-usage/stats`, `dashboard`(+HEAD), `execution-stats`, `storage-stats`, `messages`, plus 9 internal-config GETs. **Slice 3**: `boost-packs`, `execution-tiers`, `storage-tiers`, and `reward-config` — the risky one. Its gate ships in the **same commit** as `GET /api/rewards/agent-sharing`, a customer-readable projection returning `{ isActive }` only, backed by a new narrow `ConfigRepository.isRewardActive()` so the amounts, thresholds and anti-abuse caps never enter the route at all. **No migration; no mirrored value in `system_settings_config`; not routed through `/api/system-config`** — both SA-rejected alternatives avoided. Both agent-detail pages repointed. **Slice 5**: `app/admin/layout.tsx` is now an async Server Component awaiting `requireAdminPage()`; the chrome moved **verbatim** to `AdminChrome.tsx`; **none of the 21 `page.tsx` files was edited**. BQ-3 option A, with the identical-response assertion using an **ONBOARDED** non-admin fixture — the middleware onboarding redirect would otherwise redden it for a non-security reason (fixture fixed, assertion kept). E1/E2/E3 written into the guard’s docstring. **Ratchet: R1 34 → 7, R6 1 → 0** in the same commit, with every published figure updated. Tests: guard **74/74**, oracle **293/293** (58 handlers × 5 + 3), admin surface **508/508** across 13 suites. **Not done and not claimed:** slice 4’s 7 inline copies, so "one way to validate" remains untrue in use. |
