...
| **3. Register** | Add the key to `PLUGIN_PROFILES.all` **and** `PLUGIN_PROFILES.business_os` (`lib/server/plugin-profile.ts`) and the executor to `executorRegistry` (`plugin-executer-v2.ts`). BOS modules always go in both profiles. | — |
...
- 🟢 **Plugin-route identity hardening** (was "Execute-route hardening") — **implemented, awaiting user code review**: [workplan](/docs/workplans/BUSINESS_OS_PLUGIN_ROUTE_IDENTITY_HARDENING_WORKPLAN.md). The item was understated: `POST /api/plugins/execute` had **no authentication at all** (middleware is a subdomain/onboarding layer, not an auth gate), and the same shape ran through **eight handlers** across `/api/plugins/*` and `/api/plugin-connections` — including an unauthenticated `GET /api/plugin-connections?plugin_key&user_id` that returned **decrypted stored credentials**, and an unauthenticated service-role route that **wrote OAuth tokens into any account**. All now resolve identity server-side through `lib/server/route-identity.ts` (session → self, or admin-gated + audited act-as).
- 🔴 **OAuth `state` is unsigned and carries `user_id` — same class as the holes just closed, still open** (SA sweep, 2026-08-19). `app/oauth/callback/[plugin]/route.ts` → `handleOAuthCallback` parses `user_id` out of a **client-built, unsigned `state`** (`lib/server/user-plugin-connections.ts:225,670-677`) and writes plugin connections/tokens for that account **with no session check** — the same shape as the deleted `plugin-connections/save` route. **Deliberately out of scope of the identity-hardening PR** (OAuth callbacks are a distinct flow: the provider redirects the browser, so the fix is binding `state` to the session — verify `state.user_id === getUser().id` in the callback — and/or signing it, not the act-as resolver). Do not read the item above as "the cluster is closed": `/api/plugins/*` and `/api/plugin-connections/*` are, `/oauth/*` is not.
- ⬜ **Chunk-aware identity cache inside the route-identity resolver** (deferred from the hardening cycle, SA C11.4). `lib/cachedAuth.ts` was deleted: it looked up a single unchunked cookie name while this project's Supabase cookies are chunked, so it silently missed live sessions and pushed routes onto caller-supplied-`userId` fallbacks. Any future auth cache must live **inside the resolver**, be chunk-aware, and come with an explicit staleness/revocation decision. The resolver already logs `{ durationMs, route }` so the perf case can be measured rather than argued. `docs/PERFORMANCE_OPTIMIZATION_PLAN.md` §4A is marked superseded.
...
**Status:** ✅ **Shape decision RESOLVED (user, 2026-08-11) — service, not a plugin.** It's a cron-driven **compute-and-persist analytics** subsystem, not user-invokable CRUD. The minimal G2 slice is merged; full plugin re-evaluation is deferred to post-Step-3. See **Decisions** at the end of this section.

> 📖 **The as-built map is [BUSINESS_OS_INSIGHTS_MODULE.md](/docs/architecture/BUSINESS_OS_INSIGHTS_MODULE.md)** (2026-09-11), with the `business-os-insights` skill fronting it. The assessment below is from 2026-08-11 and predates the 2026-08-26 → 09-07 rebuild; **where the two disagree, the module doc wins.**
...
- Subsystem `lib/business-os/insight/**`: events, metrics, **detectors (28 — was 5 at assessment time; 22 added 2026-08-26, one 2026-09-07)**, prioritizer, kernel, automation, projection, reporting, **plus a correlation engine (10 patterns), vertical config, journey timeline and funnel-gap modules added post-assessment**. Live via 3 Vercel crons (`insight-detect` 15min, `insight-metrics` daily, `insight-automations` 5min). **Count detectors from `new *Detector(` in `DetectorEngine.ts` — `detectors/catalog/index.ts` is a stale barrel exporting only 7.**

**Key findings:**
- **Liveness / build-but-dark:** `business_events` still has **ZERO emitters** (re-verified 2026-09-11 — `BusinessEventService` is imported by exactly one file, `MetricsComputeService`). The 4 detectors that read it (`SalesStalled`, `SalesReplySlow`, `RetNoShowSpike`, `OpsUtilizationLow`) + most of metrics compute over an empty table. **Step 3 (event-driven architecture) is what lights this up** and explicitly overrides the insight emission mechanism ([event-driven arch §1 override](/docs/architecture/BUSINESS_OS_EVENT_DRIVEN_ARCHITECTURE.md), lines 37/44-53). **Corrected since assessment:** the other 24 detectors read module tables directly (`crm_*`, `scheduling_bookings`, `payment_*`, `website_*`) and do fire on real data — the "only 2 detectors are live" claim was true of the 6-detector catalog, not the 28-detector one. Of those 24, **3 are broken on phantom columns** ([phantom-column workplan §P3](/docs/workplans/business-os-phantom-column-remediation.md)).
...
- ~~⚠️ **Data-integrity flag:** `CashArOverdueDetector.ts:88` reads `payment_invoices.amount`; `MetricsComputeService.ts:405` reads `payment_invoices.total_amount` — **different column names for the same value; at least one is a phantom column**.~~ **RESOLVED by the G2 slice (PR #22):** `total_amount` was the phantom; both sites now use `amount` via `PaymentInvoiceRepository.getOverdueInvoices`. The underlying *class* of bug is still open across the codebase — see [phantom-column workplan](/docs/workplans/business-os-phantom-column-remediation.md).
...
| 2026-09-11 | Insights section re-verified against `main` | The Insights assessment dated from 2026-08-11 and predated the 2026-08-26 → 09-07 rebuild. **Corrected:** detector count **5 → 28** (+ correlation engine, vertical config, journey timeline, funnel gap, channel insights — all added post-assessment); the "only 2 detectors fire on real data" claim (true of the 6-detector catalog, not the 28-detector one — 4 read the still-empty `business_events`, 24 read module tables, 3 of those are broken on phantom columns); the stale 🟠 "decision pending user" status, which contradicted the RESOLVED decisions in the same section. **Struck through as resolved:** the `payment_invoices.total_amount` data-integrity flag, fixed by the G2 slice (PR #22). **Added** a pointer to the new as-built map, [BUSINESS_OS_INSIGHTS_MODULE.md](/docs/architecture/BUSINESS_OS_INSIGHTS_MODULE.md), which now wins where the two disagree. Separately, the seven agent-shadow-insight docs were moved to `docs/archive/` with banners. |
| 2026-09-19 | Register step points at the plugin profile | Step 3 of the Conversion Recipe now registers a module key in `PLUGIN_PROFILES.all` and `PLUGIN_PROFILES.business_os` (`lib/server/plugin-profile.ts`) instead of the removed `corePluginFiles` array. See [BUSINESS_OS_PLUGIN_PROFILE_WORKPLAN.md](/docs/workplans/BUSINESS_OS_PLUGIN_PROFILE_WORKPLAN.md). |
