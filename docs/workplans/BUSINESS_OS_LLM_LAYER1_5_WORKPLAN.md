...
- **Q-6 — the freeze is the right default; the decision is the user's.** Convert nothing in S1–S10. Technical risk per file, for that decision:
  ...
  - **`openaiProvider.ts` (4) — low.** All four sit in `getInstance`'s configuration guards, outside the image path, so a conversion cannot affect a tracked call. Still its own commit.
    - *SA correction, 2026-09-18 (logging clean-up workplan Q-8):* only two of the four (`:114`, `:119`) are in `getInstance`. The other two (`:403` `deleteThread`, `:435` `createThreadWithSystemPrompt`) are thread-helper error branches. They are also outside the image path and every tracked call, so the **low** rating stands.
...
| 2026-09-18 | SA correction (Q-8 of the logging clean-up workplan) | §11 Q-6 note on `openaiProvider.ts`: only `:114`/`:119` are in `getInstance`; `:403`/`:435` are thread-helper error branches. Risk rating unchanged (low) |
