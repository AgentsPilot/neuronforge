...
**Component 5 — admin ops + docs** — ✅ **built 2026-09-22, uncommitted, awaiting SA review** (§4.30)
- [x] **S1-T12b** ✅ *(moved here from component 4)* The **`GET /api/admin/business-os/entitlements/shadow-report` route**: `requireAdmin` first, Zod over `from` / `to` / `asTier` / `includeSetupAi`, then one call to `buildShadowReport()`. The report builder is done (§4.27); this is the gate in front of it, and it belongs with the other admin routes so the whole admin surface lands in one reviewable PR. Add the route to `ALLOWED` in the RC-15 guard — and **not** to `NO_STATE_WRITE_REFERRERS`.
- [x] **S1-T12** ✅ `accounts/[accountId]` GET + POST union (`ensure_plan_row` with explicit cohort per R2-2, `set_cohort`, `set_expiry` incl. `tier_expires_at`, `assign_tier` with the required `expiresAt` key per A-1, `add_override`, `end_override`, **`reset_plan_state`** per A-3, `would_leave_no_basis` per R2-3) and the **`launch` route with `dryRun`** (R2-1). **`requireAdmin` first, and no `AdminAccessService` import** (guard R1/R2). Audit events in `lib/audit/events.ts`.
- [x] **S1-T12a** ✅ *(binding, from QA's component 1 review)* Route-level obligations the repository layer cannot own:
...
- [x] **S1-T16** ✅ `docs/architecture/BUSINESS_OS_ENTITLEMENTS.md` written, and the **CLAUDE.md Key Documentation row is APPLIED** — the user approved it and TL added it (one line; QA verified every claim in it).
...
### 4.30 Component 5 as built (2026-09-22) — admin operations and docs, for SA review

The last part of Slice 1. Three routes, one operations module, one architecture doc.

| File | What it is |
|---|---|
| `lib/business-os/entitlements/adminOps.ts` | The Zod `op` union and `executeAdminOp` — every validation, pre-check and write. |
| `app/api/admin/business-os/entitlements/accounts/[accountId]/route.ts` | GET inspect + POST ops. |
| `app/api/admin/business-os/entitlements/shadow-report/route.ts` | S1-T12b — the gate in front of component 4's builder. |
| `app/api/admin/business-os/entitlements/launch/route.ts` | R2-1 — the launch dry run. |
| `docs/architecture/BUSINESS_OS_ENTITLEMENTS.md` | S1-T16. |
| `lib/audit/events.ts`, `lib/audit/types.ts` | Eight events + the `business_os_account_plan` entity type. |

**Why the logic is not in the routes.** `requireAdmin` must be the **first statement** of every exported handler, and the CI guard reads each handler's own body — so a handler with two hundred lines of rules under that line is a handler where the gate is easy to lose in a refactor. The routes own the gate, the HTTP shape and the audit entry; everything else is in `adminOps.ts`, where it is testable without a request.

#### 4.30.1 Route-by-route auth

Every exported handler opens with the same two lines, written out in each (never factored into a helper — that is what the guard forbids):

```ts
const gate = await requireAdmin(logger.child({ route: '…' }));
if (gate instanceof NextResponse) return gate;
```

| Route | Handler | Gate is statement #1 | No `AdminAccessService` import |
|---|---|---|---|
...
Asserted three ways: `npm run test:authz-guard` (**74 passed, no new exemption** — the new handlers are recognised as gated, so R1's parked list did not grow); a `grep` for `AdminAccessService` in the four route files returns **0**; and the route suite runs **all four handlers** through 401 (signed out), 403 (signed in, not an admin) and 403 (**the admin check threw** — fail closed), asserting in each case that **no repository call and no audit entry happened**. AC-6 has its own case: a user whose `profiles.role` says `admin` still gets 403, because the gate reads `admin_users`.

#### 4.30.2 How each carried-in obligation is met

| Obligation | Where | Test |
|---|---|---|
| **R2-2** — `ensure_plan_row` takes an explicit cohort | No default in the schema; `cohort` is required | "has no default cohort" |
| **RC-4 / A-1** — a champion and a tier must both say when they end | `expiresAt` is a **required key** on `assign_tier`; `set_cohort`/`ensure_plan_row` refuse a champion without it (400) | three cases, each with the passing counterpart |
| **R2-3** — no op may leave an account with no basis | `wouldLeaveNoBasis` projects the state the patch WOULD produce (including the paired-expiry clearing `updatePlan` does), and refuses 409 | four cases: clearing a cohort with no tier, with a tier, with an **expired** tier, and an expiry that strands a tier-only account |
| **Q-15** — "no row" is not success | Pre-check returns 409 `plan_row_missing`; `writePatch` returns the same when `updatePlan` reports no match | two cases, both asserting no write was attempted |
| **Q-13** — an unknown value is refused at the route | `assign_tier` enumerates `TIER_ORDER`; `add_override` enumerates the catalog | two cases, each with a valid counterpart |
| **Q-6 / M-3** — a database constraint is never the message | `set_expiry` refuses `no_cohort_to_expire` / `no_tier_to_expire` explicitly | two cases + the clearing case that must still work |
| **M-4** — a blank cohort is refused | The schema has no blank option; the RPC's own check is the backstop | schema cases |
| **C3-2** — an override may not grant a `not_built` capability | `add_override` refuses 409 `capability_not_built` before writing | refusal, **plus** `revoke` allowed (not a grant) and an `available` capability allowed |
| **RC-1** — no tiers configured | `assign_tier` returns 400 `no_tiers_configured` on the production config | one case on `readCodeConfig()` |
| **RC-10** — is this even a tenant? | Profile **or** any onboarding message; otherwise 404 | one case |
| **A-3** — reset guards | Echoed id must match; `confirmTierLoss` required when a tier is assigned; trial needs a clock; deleted overrides ride in the outcome | five cases |
| **WC-7** — audit flushed before responding | `await auditTrail.flush()` after the log | a case asserting the **order** (`['log','flush']`), not just that both happened |
| **R2-1** — launch | Dry run reports and writes nothing; a real run is 409 while no tier is configured, and 501 until Slice 2 owns its execution | three cases |

#### 4.30.3 What a person can actually SEE once this deploys

Nothing changes for a customer. Nothing is enforced, `BOS_ENTITLEMENTS_MODE` is unset, and no UI is added. What appears is **four admin-only JSON endpoints**, reachable by a signed-in user who is in the `admin_users` table (everyone else gets 401/403):

| URL | Returns |
|---|---|
| `GET /api/admin/business-os/entitlements/accounts/<uuid>` | That account's state (`champion` today), its lifecycle dates, **every capability with the layer that decided it**, its overrides **with reasons** (the admin-only view), and `effectiveWithinSeconds: 30` |
| `POST /api/admin/business-os/entitlements/accounts/<uuid>` | Applies one op. `{"op":"set_cohort","cohort":"champion","expiresAt":null,"reason":"design partner"}` |
| `GET /api/admin/business-os/entitlements/shadow-report` | The static section immediately — accounts by state and cohort, **who has free access with no end date**, and the trim list. With `?from=&to=` it adds observed usage; with `&asTier=` the replay; with `&includeSetupAi=true` the setup-AI measurement |
| `POST /api/admin/business-os/entitlements/launch` | `{"confirm":"launch_champion_existing","reason":"planning"}` → how many accounts the switch-on would touch. Writes nothing |

**To hit them you need:** the migration applied (the runbook), a signed-in session, and a row in `admin_users`. The most useful one on day one is the shadow report with no parameters — it answers "who is getting the product free, for ever, and which of them never even finished setting up" from real data. **The observed and replay sections stay empty until `BOS_ENTITLEMENTS_MODE=shadow` is set**, because nothing records until then.

#### 4.30.4 For SA

1. **`lib/audit/events.ts` and `lib/audit/types.ts` are shared files.** Eight `BOS_ENTITLEMENT_*` events and one entity type (`business_os_account_plan`) were added. Additive only; no existing value changed.
2. **The RC-15 guard gained a fifth category, `admin_ops`.** `adminOps.ts` holds the write calls but is not a route, so smuggling it in as a "reader" would have made `NO_STATE_WRITE_REFERRERS` a lie. The partition now also decides `isTest` **first**, so a test file under `app/api/admin/**` counts once.
3. **The account-seam guard classified `adminOps.ts` as EXEMPT**, with the reason: it works in account ids throughout, and the `userId` the guard sees is the repository's input field name (`user_id` is the column).
4. **A test-quality note worth recording:** the first draft of `adminOps.test.ts` used `reason: 'x'` everywhere, which is INVALID (minimum 3 characters) — so "refuses an unknown tier" was really asserting "refuses a one-character reason". Its counterpart assertion caught it. The suite now has an explicit reason-length case with both sides.
5. **`launch` returns 501 for a real run.** Slice 2 owns its execution (idempotency marker, per-account audit, summary), and half of it now would be worse than none.
6. **The CLAUDE.md Key Documentation row is APPLIED.** It was proposed here, the user approved it, and TL added the single line; QA checked it claim by claim (both linked paths exist, the config-edit and one-line/`removals` claims match the code, the `not_built` rule matches the loader, and "nothing is enforced yet" matches `mode.ts` + `TIER_ORDER = []`). §4.30.5 keeps the wording for the record.

#### 4.30.5 The CLAUDE.md row — ✅ APPLIED (2026-09-22)

**Proposed here, approved by the user, added by TL.** Kept for the record; the live wording is in CLAUDE.md § Key Documentation. QA verified each claim rather than the prose (§14.12).

> `[BUSINESS_OS_ENTITLEMENTS.md](/docs/architecture/BUSINESS_OS_ENTITLEMENTS.md)` — **What an account can do in Business OS, and why.** Capability catalog, tier matrix, cohorts, the resolver and the admin surface. **Changing what a plan includes is a config edit** (`config/tierMatrix.ts`), and adding a capability to a tier is one line while removing one needs a `removals` entry and a version bump. **Nothing is enforced yet** — `BOS_ENTITLEMENTS_MODE` is unset in production. Read before touching `lib/business-os/entitlements/**` or `app/api/admin/business-os/entitlements/**`; the apply runbook is `BUSINESS_OS_ENTITLEMENTS_APPLY_RUNBOOK.md`. | 2026-09-22

---

### 4.31 QA fixes applied (2026-09-22) — QA-1, QA-2, QA-3 and the audit nit

The three Low findings from §14.12, plus the free one. Implementation stays uncommitted.

| # | What changed |
|---|---|
| **QA-3** (SA C5-1) | **The `not_built` rule is now step 3 of the add-a-tier procedure**, not only a remark two sections away. It is followed by a boxed subsection that gives: the table of **what counts as granting on every shape** (including the two that catch people — `'purchasable'` on an add-on, and `purchasable: true` on a quantity with `included: 0`), the **verbatim error message** an offender gets, and the two fixes in the order they are usually right — withhold it in the row keeping the intended value in a comment, or correct the catalog entry if the feature really does exist, on the bar *"a customer gets the outcome"*. It names the three entries that were corrected downwards on exactly that test. The rule rejected **eight** capabilities in Eyal's draft the first time it ran, which is why it belongs where someone following the steps will hit it. |
| **QA-1** | **`deletedOverrides` → `endedOverrides`**, and both comments plus the audit-event description now say what the code does. Since **M-2** the reset RPC **ends** every override (`ended_at`, `ended_by_admin_id`, `ended_reason`) and contains no `DELETE` — the Jest migration guard asserts that. The field's comment now says the rows survive and that this is the before-state in one place *next to the actor and the reason*, rather than the last copy of it; `lib/audit/events.ts` says the same; and the test's name and body follow. Three artefacts described the opposite of the code in the record someone reads during an incident. |
| **QA-2** (SA C5-2) | **Two tests make the `AUDIT_EVENTS[…] ?? action` fallback unreachable.** A **source sweep** over `adminOps.ts` collects every `'BOS_ENTITLEMENT_*'` literal — matching the plain string form too, because three of them reach `writePatch` as its last argument and a sweep that missed those would have been the vacuous version — and asserts each is a registered key, with `new Set(actions).size === 7` as the non-vacuity leg. An **executed** test then runs all seven op variants and asserts the action each returns is registered. The fallback stays, with a comment saying why: an unregistered action should still write a row rather than `undefined`, and a row named oddly beats no row at all. |
| **Nit** | The 403 and throwing-admin-check cases now assert `state.audit` is empty as well, like the 401 case. A refusal writes no audit row, because the gate precedes everything. |
| **Stale line** | §4.30.5 is retitled **✅ APPLIED** — the user approved the CLAUDE.md row and TL added it; QA verified each claim. The task line and the SA note above it say the same. |

**Re-verified**

| Check | Result |
|---|---|
| Scoped suites + routes + audit + admin guards | **55 suites, 1,054 tests, 0 failures** (was 1,052; +2 for QA-2) |
| `npm run test:authz-guard` | **74 passed**, no new exemption |
| Typecheck, the verified method | **2,030 — unchanged baseline**, 0 × TS2688, **0** in entitlements and **0** under `app/api/admin/business-os` |
| `npm run lint:hooks` / `console.*` | clean / 0 |
...
#### The finding: "Essentials has no chat" was not expressible - now it is, as config

Turning the eight per-operation `chat.*` capabilities off **does not close the chat surface**. Under the configured read rule (`domain_group`), an Essentials owner's chat question about their own contacts maps to `crm.core` - which Essentials has - so it resolves to `allowed`. SA widened this on review: the same mapping is the default for **every** operation, so it is not only reads - an Essentials owner could **write** through chat too.

SA's diagnosis of why the config could not say it: `chat.*` was never one thing. It is four feature areas reachable through chat, three modalities and one marketing capability - and **nothing meant "may use the chat surface at all"**.

**Resolved as FR-46** (requirement updated 2026-09-24): a new capability `chat.access`, off for Essentials and on for Autopilot, shipped **now** in the catalog and both tier rows. **Nothing reads it until Slice 2**, which gates the chat entry point on it once per turn, before any per-capability check, and refuses as `not_entitled` with a specified customer-visible message rather than an improvised one. See §5 for the addendum entry, and §4.34 for what shipped here.

The two tests in `productionConfig.test.ts` still pin the gap in both directions - they are what proves the gate is still needed, because `chat.access: false` alone changes nothing until something reads it.
...
| # | What changed |
|---|---|
| **T-1** (required) | **The chat-surface gap is now a requirement, not a test comment.** `FR-46` in the requirement, with SA's five points spelled out: (a) one capability, `chat.access`; (b) gated **once per turn at the entry point**, before any per-capability check, so the refusal never depends on which entity the planner picked; (c) refused as `not_entitled` naming Autopilot, with the **customer-visible wording written out** rather than left to be improvised - and both plan names read from the `presentation` config so a rename carries; (d) the per-operation map kept, because it still decides what an Autopilot account may do inside chat and it feeds the report and metering; (e) the surface decision **recorded in shadow**, because without it the report cannot count *accounts that would lose chat entirely*, which is the number the price difference rests on. Plus `AC-38`, a §5.3 catalog row, and a Slice 2 addendum row in §5. |
| **`chat.access` in the catalog NOW** | **Chosen: add it now.** The catalog is a claim about what the CODE delivers, and the chat surface exists - so `available` is the honest lifecycle, and the `not_built` rule has nothing to object to. Putting it in the catalog and both tier rows today means Slice 2 is a **wiring** change rather than a pricing one, and it means the commercial difference is stated exactly once, in `tierMatrix.ts`, where a reviewer can see it. The alternative - hold it until Slice 2 - would have left the config saying Essentials has chat for as long as the gate took to build. Shape is `boolean`, not `group`: a group is "a named set of fine-grained operations on one surface", and this is the surface. Catalog is now **38** capabilities (`catalog.invariant` count updated deliberately), the paid tiers differ by **ten** keys (nine `chat.*` + `ai.actions`), and the drift snapshot was refreshed - an ADDITION, so no `removals` entry and no version bump (B-10). |
| **Row 55** | Names the benign cause first: a tenant is counted from the union of business profiles and onboarding messages, so **someone who legitimately reset or deleted their onboarding transcript, with no business profile yet, stops being counted while their plan row correctly stays**. The message says to check that before looking for an admin `ensure_plan_row` on the wrong id, and says it is only worth investigating if the gap is large or the rows are recent. |
| **Guard extended to the migrations** | The two applied migrations are now in the guard: they carried **20 and 5** comment apostrophes and **2** string semicolons between them, which is one careless line from the same failure - and the next migration will be written by copying one of them. Cleaning them up is **comments only, plus two `COMMENT ON` strings** where a semicolon became "and". ⚠️ Those two table comments now differ in production by one punctuation mark, because the migrations are already applied and are not being re-run. Harmless, recorded here rather than silently fixed. Guard is now **27 tests over six files**, with an explicit case asserting both migrations are still in the list. |
| **`cohorts.ts:33`** | `export interface CohortConfig extends CohortConfigShape<...> {}` is now a **type alias**, clearing the standing `@typescript-eslint/no-empty-object-type` error (SA's call). Nothing merges into that name, so the alias loses nothing. |
| **Untranslated names** | Both config files now say plainly that the plan names are **deliberately untranslated for now** - all three locales carry the English string, the user will supply Hebrew and Spanish, and they are present rather than optional so a missing translation is a visible duplicate rather than an `undefined`, and so the day they arrive is a data change. |
| **`ungated` branch** | **Kept**, per SA. It has no production instance now that the agent-platform entities have left the chat catalog, and it is covered by the mocked-classification test in `planCapabilities.test.ts`. |

**Typecheck baseline is now 2,029**, refreshed as SA asked. The previous standing number was 2,030; nothing in this change set accounts for the -1 and it is in the safe direction, so it is adopted as the baseline rather than explained away. Future runs on this branch are measured against **2,029**.

#### Re-verified

| Check | Result |
|---|---|
| `npm run test:bos-entitlements` scope | **49 suites, 934 tests, 0 failures** (was 930 before this round) |
| That scope plus the admin entitlement routes and `scripts/__tests__` | **53 suites, 1,085 tests, 0 failures** |
| `npm run test:authz-guard` | **74 passed**, no new exemption |
| Typecheck, the verified method | **2,029** - the refreshed baseline, matched exactly - 0 x TS2688, **0** in `lib/business-os/entitlements` and 0 under `app/api/admin/business-os` |
| ESLint over `lib/business-os/entitlements`, `app/api/admin/business-os` and `scripts/__tests__` | **clean, 0 problems** |
| `console.*` in the touch set | 0 |
...
| **Chat SURFACE (FR-46, added 2026-09-24)** | **The FIRST chat check, before the one below.** `chat.access` is resolved **once per turn at the entry point**, before a plan is built and before any per-capability check. Refusal is `not_entitled` with `lowestTier` from `lowestTierFor('chat.access')`, rendered as a **normal assistant message** (FR-46c wording, both plan names read from the tier `presentation` config, never hard-coded), so the conversation stays usable and says what the owner can still do on the screens. The turn records its surface decision in shadow, and `asTier` gains a **distinct-accounts-that-would-lose-chat-entirely** count (FR-46e) — the number the price difference rests on. Tasks + tests in the addendum; AC-38. **Why it cannot wait for the per-capability layer:** every chat operation maps to the capability of the DOMAIN it touches, so an Essentials account reading *or writing* its own contacts through chat resolves to `crm.core`, which it has. |
...
### 14.12 QA — component 5: admin operations and docs, and Slice 1 exit (2026-09-22)

**Test mode:** full, and the last QA round of Slice 1.
**Strategy:** **A + B + E** — suites run independently, the authorization surface walked by hand rather than read from the guard's report, and the docs read as the person who will follow them in six months.
**Reviewed against:** workplan `4590000e` + §13.8 (C5-1, C5-2); components 1–4 committed at `2bbbb2ec`, `e5f5d163`, `d0995daa`, `af9748a6`.

**Verdict: ✅ PASS — component 5 is ready for the user's code review and for RM to commit. Slice 1 is code-complete.**
- **Authorization holds under adversarial reading**, and the census claim checks out on my own walk.
- **Dev's self-reported vacuous-refusal class is genuinely fixed** — 16 refusals, every one naming its code, several also asserting the write did not happen.
- **Three findings, all Low**: SA's two low fixes (**C5-1, C5-2**) are still open in the tree, plus one of my own — an audit field that says `deleted` about rows the design deliberately *ends*.

#### 14.12.1 What QA ran

| Command | Result |
|---|---|
| `npm run test:bos-entitlements` (the CI command) | ✅ **48 suites, 912 tests** — matches SA exactly |
| Wider scope: the above **+ the admin route suites + `lib/audit` + `lib/admin`** | ✅ **56 suites, 1,077 tests, 0 failures** |
| `npm run test:authz-guard` | ✅ **74 passed**, no new exemption |
| Hooks ESLint over `lib/business-os/entitlements`, `app/api/admin/business-os`, `lib/audit` | ✅ exit 0 |
| `console.*` / `: any` in `adminOps.ts` and the three routes | ✅ **0 / 0** |
| Typecheck, the verified method | ✅ **2,030 — the unchanged baseline — 0 × TS2688, 0 in any entitlements file, 0 under `app/api/admin/business-os`**; control 3 elsewhere |
| Scope | ✅ Exactly the named files. `CLAUDE.md` is **one line** added, nothing else |

#### 14.12.2 Part A — authorization, adversarially

**The gate, verified by my own walk rather than from the guard's output:**

| Check | Evidence |
|---|---|
| Four handlers, four gates | `grep -c 'requireAdmin('` → accounts route **2** (GET + POST), launch **1**, shadow-report **1**. Each handler contains its own literal call, which is what guard R1 reads. |
| Nothing before it on any path | The `try` opens **after** the gate in all four. The only expression evaluated earlier is the `logger.child({...})` argument — object construction, no I/O, no request body, no param read. The dynamic `[accountId]` is touched at `route.ts:60` and `:120`, both inside the `try`, both after the gate. |
| The census claim | ✅ **Checked independently:** `find app/api/admin -name route.ts` → **47**; of those, **40** contain `requireAdmin(`. The 7 that do not are the hand-rolled handlers CLAUDE.md already documents as parked (slice 4) — none is new. All three entitlement routes are in the 40, and the guard's own assertion is written as a **floor**, not an equality (`admin-authz-surface.guard.test.ts:842`), so the new files are counted rather than quietly excluded. |
| No `AdminAccessService` in a `route.ts` | ✅ Only the *test* mocks it, which is correct and outside R2's scan. |
| Do the 401/403/throws tests assert **absence of side effects**? | ✅ **Yes** — `expect(repositoryCalls).toEqual([])` on all three, and the 401 case also asserts `state.audit` is empty. That is the assertion that makes the status codes worth anything. *Nit:* the 403 and admin-check-throws cases do not also assert the audit is empty; it is free and would make the three consistent. |
| Can a non-admin learn anything? | **No.** Because the gate precedes every read, the interesting distinctions — 404 `not_a_business_os_account` vs 409 `plan_row_missing` vs 200 — are only ever reachable *after* an admin check passes. A non-admin gets the same 401/403 for any `accountId`, having caused no database access, so there is no timing or shape oracle. Zod detail is `NODE_ENV === 'development'` only; the 500 body is generic. |
| Is the audit actor genuinely persisted? | ✅ **Verified independently:** `AuditTrailService.ts:123,128` maps `input.actorId` → the `actor_id` column, and `AuditTrailRepository.ts:30,36` selects and types it. The route passes `userId: accountId` (who it is about) and `actorId: gate.user.id` (who did it), which is the better shape. |

**A previous finding of mine is now closed.** `lib/business-os/entitlements/__tests__/accountSeam.guard.test.ts` is the assertion R4-2 asked for and my §14.11 **A-2** recorded as missing: every entry point must resolve its id through `resolveAccountId` or sit on an exempt list that says why, with a "scans a plausible number of files" meta-guard and a check that the exempt entries exist. The account route uses the seam at `:65` and `:125`.

#### 14.12.3 Part B — the write ops

**The vacuous-refusal class Dev self-reported is genuinely fixed, and I checked it by counting rather than reading.** `adminOps.test.ts` contains **16** refusal assertions and **16** `error: '<code>'` strings; the one `ok: false` that greps as bare (`:469`) names `tier_assigned` on the next line. Fourteen distinct codes are asserted, and several refusals *also* assert the write never happened (`expect(calls.resetPlanState).toEqual([])`). **Hunting the same class elsewhere:** the "unknown tier / unknown capability" cases now live in Zod — `adminOpSchema` builds both enums from the config — and are covered by a route test that asserts the repository was not touched. One honest residual, already stated in the code: with `TIER_ORDER` empty (today's production) `tierEnum` degrades to `z.string()`, so "an unknown tier is refused by the schema" only becomes true once a tier exists; until then the op is refused by `no_tiers_configured` instead.

| Condition | Verdict |
|---|---|
| `wouldLeaveNoBasis` vs real patch behaviour | ✅ Correct in the subtle place. It projects `{...plan, ...patch}`, **replicates `updatePlan`'s paired-expiry clearing** (`adminOps.ts:200-201`) so the projection cannot disagree with what the repository will do, and tests with `tierInForce`, so an **expired** tier does not count as a basis. The one divergence from `updatePlan`'s `hasOwnProperty` test — an explicitly-`undefined` key — cannot be expressed in a JSON body. |
| C3-2 (no `not_built` grant, revoke still allowed) | ✅ `adminOps.ts:397` with the right asymmetry. *Nit:* the second check at `:414-419` is unreachable — `:397` already returned for every non-`revoke` op — and its comment describes a `beta` case that it does not actually test. |
| Reset guards | ✅ All four: confirm literal (Zod), echoed id vs path (`:480`), `tier_assigned` unless `confirmTierLoss` (`:486`), and the before-state carried into the audit. |
| WC-7 | ✅ `log` then `flush` then respond, with the route test asserting the **order** (`expect(state.events).toEqual(['log','flush'])`), not merely the calls. |
| Tenant isolation on a service-role write keyed by a caller-supplied id | ✅ Full M1/G3 pattern in the right order: gate → uuid parse → tenant pre-check (404) → plan-row check (409) → invariant pre-check (409) → allow-listed `PATCH_FIELDS` patch → `.eq('user_id', accountId)`. The id comes from a validated path parameter and is never spread into a payload. |

| # | Sev | Finding |
|---|---|---|
| **QA-1** | Low | **The audit says `deleted` about rows that are deliberately not deleted.** `adminOps.ts:516` names the field `deletedOverrides`, its comment at `:514-515` says *"those override rows are GONE after this"*, and `lib/audit/events.ts` repeats it (*"carries the deleted overrides… the only remaining trace"*). Since **M-2**, `business_os_reset_plan_state` contains no `DELETE`: it **ends** the overrides, and the migration's own comment is *"Ended, never deleted"* — a property this module argued for and the Jest guard enforces. Three artefacts now describe the opposite of what the code does, in the record someone reads during an incident. Rename to `endedOverrides` and correct both comments. (The list is also the full override history, including rows ended earlier — correct for a before-state, worth saying.) |
| **QA-2** | Low | **SA's C5-2 is still open, and nothing tests the fallback.** `route.ts:171` — `AUDIT_EVENTS[outcome.action] ?? outcome.action` would silently write an unregistered action string. Neither `adminOps.test.ts` nor `routes.test.ts` mentions `AUDIT_EVENTS`, so no test asserts that every `action` the ops emit is a registered key. The op union is closed; either drop the fallback or add the one-line assertion. |
| **QA-3** | Low | **SA's C5-1 is still open.** See Part C. |

#### 14.12.4 Part C — the docs

**`BUSINESS_OS_ENTITLEMENTS.md` is workable for the six-months-later reader.** I read it as that person: it says who owns each layer and when it changes, gives the four-step add-a-tier procedure, states the deliberate one-line/two-line asymmetry with the ledger entry and version bump, explains why `'renewal'` is rejected before billing, covers histories, the staleness bound, the mode flag, the pre-enforcement gates, the admin operations **with their refusal codes**, and the ops checks — and it points at the apply runbook instead of duplicating it. ToC and change history per the doc standard.

**C5-1 has NOT landed (QA-3).** `docs/architecture/BUSINESS_OS_ENTITLEMENTS.md:42-45` — the add-a-tier steps still say only that a missing value fails to compile and fails Zod. The rule most likely to stop the person following them — **a tier may not grant a `not_built` capability** — appears at `:36` and in the refusal-code list at `:114`, but not in the procedure. It rejected Eyal's draft matrix on eight capabilities; it will be the first thing a real matrix hits. Still one line in step 2.

**The CLAUDE.md row is accurate.** I checked each claim rather than the prose: both linked paths exist (`docs/architecture/BUSINESS_OS_ENTITLEMENTS.md`, `docs/BUSINESS_OS_ENTITLEMENTS_APPLY_RUNBOOK.md`); "changing what a plan includes is a config edit" matches `config/tierMatrix.ts`; the one-line/`removals` asymmetry matches the snapshot rule; "a tier may never grant a `not_built` capability — the config refuses to load" matches the loader; and "nothing is enforced yet — `BOS_ENTITLEMENTS_MODE` is unset and no commercial tier is configured" matches `mode.ts` and `TIER_ORDER = []`. Exactly one line was added. **The workplan's §4.30.5 "awaiting the user" line is stale** — the row is on disk and approved; mark it applied.

#### 14.12.5 Part D — Slice 1 exit criteria (§4.16), QA's own verdict

| # | Criterion | QA verdict |
|---|---|---|
| 1 | AC-1/3/5/6/36/37 on production config; AC-2/4/7 on the fixture | ✅ **Met.** 912 tests in the CI scope; AC-4 proven end to end through the resolver, AC-6 proven by a test that a self-set `profiles.role` is still 403. |
| 2 | Flag off ⇒ no entitlement reads or writes; chat-v4 never loads config | ✅ **Met**, and verified by me in §14.11 with a harness where every collaborator throws — not from the test names. |
| 3 | The shadow path cannot throw, including a throwing config loader | ✅ **Met.** |
| 4 | The verification script proves the triggers never raise, a trial cannot restart, no user policies, revokes in effect, reset atomicity | 🔄 **Replaced, knowingly.** The probe suite is prohibited on production (§4.20.2). Trigger binding, privileges, RLS, constraint definitions and the backfill's shape are proven by the read-only checker at apply time; **S-8(i) — a failing plan write cannot fail a product write — remains argued from the code, not demonstrated**, with runbook steps 6b and 8 as the observation. This is the one exit criterion that is genuinely weaker than written, and it is recorded as such in three places. |
| 5 | A-1 fallbacks covered; the no-end-date list covers both kinds | ✅ **Met.** |
| 6 | Shadow on for a week; the report shows real usage; tenants-without-plan-row = 0 | ⛔ **Not met, and cannot be** before the production apply plus a week of traffic. Note that the report's setup-AI number also carries my §14.11 B-1/B-2 findings, so when this criterion is finally assessed the numbers should not be read as measured until those land. |
| 7 | Every CI job green on the PR | ⛔ **Pending the PR's first run.** Everything is green locally; the scoped typecheck cannot be run faithfully from this worktree, so the PR's `Type check (Business OS LLM attribution)` run is the real evidence. |

**Assessment: Slice 1 is code-complete. Every criterion that does not require a database or a CI run is met.**

**What is unproven, and why — for the PR description, in one place:**

| Unproven | Why | Who unblocks it |
|---|---|---|
| The migration and backfill actually apply | No database has ever seen them. The pre-flight, the read-only checker and the runbook exist precisely because the apply is the first execution | **The user**, via `BUSINESS_OS_ENTITLEMENTS_APPLY_RUNBOOK.md` |
| S-8(i): a failing plan write cannot fail a product write | The only proof needs a table-level lock on production | Observed instead via steps 6b and 8; provable if a staging database ever exists |
| A trial cannot be restarted by replaying onboarding | Needs writes to `auth.users` | Same — structurally asserted from the migration source today |
| The PostgREST round trip (`resetPlanState`, `recordEvents` through supabase-js) | Needs the service-role key and a live schema cache | The first admin op or shadow turn after the apply |
| The report means anything | Needs a week of shadow data | The apply, then `BOS_ENTITLEMENTS_MODE=shadow` |
| The setup-AI number | §14.11 B-1/B-2 — the sample is not what it claims and the read can truncate | Dev, before Slice 3 reads it |
| Cross-instance cache invalidation | Bounded by the 30 s TTL, carried on every decision as `effectiveWithinSeconds` | Accepted by design |
| **G-1: the service-role key rotated, the old key revoked, verified** | **Every guarantee in this module — no user policies, service-role-only RPCs, the `admin_users` gate — assumes the key is private. The repo was public with the live key committed.** Slice 1 adds no new exposure and may merge and run in shadow; `enforce` and all of Slice 4 must not proceed until this is done | **The user** |
| G-2: the three CI checks made required on `main` | A repository setting | The user or a repo admin |

#### 14.12.6 Needs the user

1. **C5-1 and C5-2 are still open** (QA-2, QA-3), plus **QA-1**. All three are one-line fixes and none blocks the commit — but C5-1 is the line that stops the next person's config from being rejected for a reason the procedure never mentioned.
2. **G-1 is the one that matters.** It is unchanged since the first review and it is the only item on the unproven list that is a *security* dependency rather than a sequencing one.
3. **The production apply is yours**, and the runbook is written for it. Nothing in Slice 1 changes what a customer sees, before or after.

#### 14.12.7 Final Status

- [x] **Component 5 passes — ready for the user's code review and for RM to commit.** 48 suites / 912 tests in the CI scope, 56 / 1,077 in the wider one, authz guard 74 with no new exemption, lint clean, 0 `console.*`/`any`, typecheck at the unchanged 2,030 baseline with 0 in this module and 0 under `app/api/admin/business-os`.
- [x] **Authorization verified independently**, including the route-file census (47 files, 40 gated, the 7 ungated ones pre-existing and documented) and the persistence of the audit actor.
- [x] **Three Low findings: QA-1 (the `deletedOverrides` misnomer), QA-2 (C5-2), QA-3 (C5-1).**
- [x] **Slice 1 is code-complete.** Four exit criteria met, one knowingly replaced, two pending the apply and the PR.
- [x] **Still true: no script, migration or route in this slice has been run against any database.**
...
**Verdict: ✅ PASS — ready for the user's code review and for RM to commit, with one open verification question that costs one paste to settle.**
- **The tier configuration is sound**, and the user's "a feature that does not exist cannot be allocated" rule **re-verified by me against the real two-tier matrix**, not the old empty one.
- **The SQL guard is well built and its parser is correct** — but I could not reconcile its stated mechanism with the evidence, and under that mechanism **the fixed scripts would still break**. See **A-1**; it is the one thing in this round I would not sign off from a desk.
- **The chat-surface gap is confirmed in both directions**, and T-1 is now a requirement (FR-46 + AC-38) and a Slice 2 row rather than a test comment.

#### 14.13.1 What QA ran (final state)

| Command | Result |
|---|---|
| `npm run test:bos-entitlements` | ✅ **49 suites, 934 tests** (SA saw 930 before the T-fixes) |
| The SQL guard + the admin routes + the authz guard | ✅ **5 suites, 157 tests**; the guard now covers **six** files and says so |
| Hooks ESLint over the module and the admin routes | ✅ exit 0 |
| Typecheck, the verified method | ✅ **2,029**, 0 × TS2688, **0 in any entitlements file**. ⚠️ The control also moved, **3 → 2 errors in `lib/repositories`**, so the −1 is not obviously the `never` diagnostic §13.9 attributes it to. Worth one look before the baseline is refreshed. |
| Scope | ✅ The named files, plus **both migrations** — comment rewording for T-3, see A-5 |

#### 14.13.2 Part A — the four SQL scripts and the guard

**The guard's parser is correct.** I checked the three things that would make it lie: a `--` inside a string is not treated as a comment (`splitComment` consumes quotes before testing for `--`); `''` is handled twice over — explicitly in `stringLiterals`, and in `splitComment` by toggling twice, which lands in the right state; and the negative control uses the two **real** offending lines plus a legitimate `RAISE EXCEPTION '…';` that must not be flagged. The meta-test now asserts six non-trivial files and, separately, that both migrations are still in the list.

**The rules do fail on the pre-fix content — reconstructed, not replayed.** I re-derived the offenders from `git show HEAD:` myself: preflight **7** comment-apostrophe lines and **3** string-semicolons; checker **2** and **8**. All nine apostrophe offenders were **full-line** comments, none trailing.

**Meaning is preserved.** I filtered the diff to non-comment lines and inspected every one: `;` → `,` inside display strings, three additive sub-selects in `counts`, and the new row 55. **No predicate, threshold, count or status changed.** Row 30's extra sentence is text only. **Row 55 discriminates**: `plan_rows` and `tenants` are counted independently, it fires on the real hazard (an admin `ensure_plan_row` against a wrong id), and it is a `WARN` not a `FAIL`. **T-2 landed** — its `what_to_do` names the benign cause first.

| # | Sev | Finding |
|---|---|---|
| **A-1** | **High (verification gap, not a code defect)** | **The stated mechanism and the observed evidence cannot both be true, so the fix is not provably the fix.** I implemented the splitter the guard's header describes (tracks `'`, knows nothing about `--`) and ran it over the files. It says: the **fixed** `preflight` still splits into **4** fragments, **3** of them beginning with English prose — `read \`what_to_do\``, **`a missing role aborts it.`**, `the insert adds the write on top. SET …`; the fixed checker into **3** (2 prose-leading); the rollback into **26**. The middle one is striking: a fragment whose first token is the bare word **`a`** is exactly what produces `relation "a" does not exist`, the error the user saw. **But the same model says the two migrations would split into 63 and 6 fragments — and they applied to production cleanly.** So either (i) the model is right and the scripts are still broken, or (ii) the model is wrong and we do not actually know which ingredient mattered. I could not find a third model that fits both: there is **no `--` inside any string literal** in the pre-fix files (I checked — 0 in both), so a regex comment-stripper cannot explain the inversion either. **What settles it costs nothing: paste the fixed pre-flight into the editor and report what happens.** The user has to run it again before the week-later checker anyway. Until then, "the scripts are paste-safe" is a hypothesis with a test in front of it, not a verified property. |
| **A-2** | **Medium** | **Twelve semicolons sit inside `--` comments across the three pasted scripts** (preflight 6, checker 3, rollback 3), eight of them with prose after the semicolon. Under the guard's own model each is an independent trigger of the same failure, and **the fix made them live rather than dormant**: before, the inverted quote state masked them (the pre-fix preflight cut into 2 fragments); now the quote state is honest everywhere, so every one of them is a cut point (4 fragments). If A-1 resolves toward "the model is right", this is the remaining half of the fix: a third rule — no `;` inside a `--` comment — and twelve rephrasings. |
| **A-3** | Low | `entitlementSqlScripts.guard.test.ts:53-93` parses **per line**, so a string literal spanning two lines would be misread in both directions. None exists today, and rule 4's file-level quote parity is a partial backstop — but a *pair* of odd lines passes it. One sentence in the header, or a per-line parity assertion. |
| **A-4** | Low | **No rule covers `$$` dollar-quoting**, which the rollback's `DO $$ … $$` block (26 fragments under the naive model) and both migrations depend on entirely. The only evidence the editor handles it is that the migrations applied — the same single observation A-1 turns on. |
| **A-5** | Note | **T-3 landed, and it was not a one-liner:** covering the migrations required rewording **25** comment lines and two `COMMENT ON … IS '…'` strings in files that are **already applied to production**. The rewording is safe (nobody re-applies them) but the two table comments in the database now differ from the file. Worth one line in the workplan so a future reader diffing `pg_description` against the file is not surprised. |

#### 14.13.3 Part B — the four plans

**The user's rule, re-verified against the REAL matrix.** §13.9 and the existing suite prove it on the *fixture*; the question was whether it still holds now that production has tiers. I wrote a temporary probe (deleted immediately; the tree is unchanged) that mutates the **shipped** config: granting `marketing.posts`, `website.custom_domain: 'included'`, `addon.mobile: 'purchasable'`, `sms.messages: {perMonth:100}` or `payments.reminders: true` to the **real `basic` or `pro` row** is refused every time, withholding the same capability is accepted, and the shipped config validates as-is. **Nine of my ten probes passed;** the tenth failed only because I asserted the wrong error *message* — a presentation entry for an unconfigured tier **is** refused, by the record's enum key (`Invalid enum value. Expected 'basic' | 'pro', received 'enterprise' at presentation.enterprise`), which means the `superRefine` branch for that direction is belt-and-braces rather than the live path. The other direction — a tier with no presentation entry — is refused by the `superRefine` with its own message.

| Check | Verdict |
|---|---|
| "Exactly N keys differ" | ✅ Non-vacuous: it `toEqual`s a **named, sorted list**, so a tenth difference *and* a missing one both fail. Note it is now **ten**, not nine — `chat.access` joined the eight `chat.*` plus `ai.actions`. |
| The nine-chat sweep | ✅ Derives the list from the `pro` row, asserts `toHaveLength(9)` **and** `toContain('chat.access')`, then checks both directions per capability — so a tenth chat capability cannot be silently skipped. |
| Drift snapshot honest? | ✅ The non-vacuity check was upgraded with the matrix: it now asserts the snapshot's tier keys equal the live matrix's **and** that every row carries every capability. The new **"a lowered value in the SHIPPED matrix is caught"** test flips `crm.core` to `false` on a live tier and asserts the finding names that tier and mentions `removals` — it fails when it should. |
| `mode.ts` UD-2 | ✅ The change is asserted as the new expectation **and** the old mechanism is kept alive under `jest.isolateModules` with an empty `TIER_ORDER`, so the gate cannot rot. The added `expect(logged.error).not.toHaveBeenCalled()` is a real assertion. |
| `decide.test.ts` cohort retarget | ✅ **Not papering over anything.** The fixture's `basic` is deliberately thin, so a cohort pointed at it would be refused at step (a) and the state overlay — the subject of that suite — would never run. It is retargeted to the fixture's `growth` with the reason written down, and the **production** pointing is asserted in `productionConfig.test.ts`. The only cost is that this suite no longer exercises the production cohort shape, which is covered elsewhere. |
| `adminOps.assign_tier`, `report.asTier`, `lowestTierFor` | ✅ Follow mechanically; `lowestTier: 'pro'` is asserted in the chat-gap test, which is the one that matters commercially. |

| # | Sev | Finding |
|---|---|---|
| **B-1** | **For the user — a change in risk posture, not a defect** | **UD-2 no longer downgrades `enforce`.** That is correct (its precondition was "no tier configured", and there are now two), and the gate is still tested. But it was the *code-level* reason an accidental `BOS_ENTITLEMENTS_MODE=enforce` was harmless, and that reason is gone. What stands between production and live enforcement today is the env var being unset — G-1 (key rotation) and G-2 (required checks) are process gates, not code. Worth stating plainly in the PR rather than discovering later. |
| **B-2** | Low | **The write direction of the chat-surface gap is asserted nowhere.** `productionConfig.test.ts:283-340` pins the read (`contacts.find` → `crm.core` → allowed, both readings, with the upsell). SA's widening — that an Essentials owner can **change** contacts, tasks, pages and insights through chat — lives only in review prose and the §5 row. I verified it independently (`ENTITY_DOMAIN` is the default for *every* op, and `capabilityForOp('contacts','create')` → `crm.core`, which `basic` has). One more case in the same block would pin the half that is actually worse. |

**The chat-surface gap, verified in both directions, and T-1.** Reads: confirmed by the existing test and by me. Writes: confirmed by me from the mapping, as SA describes. **T-1 landed in full during this review** — `chat.access` is in the catalog (38 capabilities) and in both tier rows; **FR-46** with its five parts and **AC-38** are in the requirement with a change-history entry; and §5 has a new **"Chat SURFACE"** row placed *before* the per-capability row, specifying the once-per-turn entry-point check, the refusal shape, the `presentation`-sourced wording, the shadow recording and the `asTier` count. That is more precise than the six points SA asked for. **T-2, T-4 and T-5 are also applied** (benign cause named; `cohorts.ts` type alias with its reasoning; the brand-name note).

#### 14.13.4 Needs the user

1. **A-1 — one paste settles it.** Before the week-later checker run, paste `scripts/preflight-bos-entitlements-migration.sql` into the SQL editor and report what comes back. If it runs, the model behind the guard is wrong but harmless and we say so; if it fails again — most likely at `a missing role aborts it.` — then A-2 is the rest of the fix. **Nothing else in this branch depends on the answer.**
2. **B-1 — the `enforce` safety net is now off.** No customer is affected (the flag is unset), but the belt-and-braces refusal is gone while **G-1, the service-role key rotation, is still open**. That ordering is worth a decision rather than a default.
3. Everything else here is configuration that changes nothing until enforcement is switched on.

#### 14.13.5 Final Status

- [x] **PASS — ready for the user's code review and for RM to commit.** 49 suites / 934 tests, guard + routes + authz 157, lint clean, typecheck 2,029 with 0 in this module.
- [x] **All five SA items (T-1 to T-5) verified applied**, T-1 in both halves.
...
### 14.15 QA — the Business OS Tiers admin page, both rounds (2026-09-24)

The page lives on its own branch and has its own workplan, so both QA rounds are written there in full: **[business-os-tiers-admin-page.md](/docs/workplans/business-os-tiers-admin-page.md) § QA Testing Report**. This is the pointer, per the single-source rule.

| Round | Verdict | What it found |
|---|---|---|
| **1** (§ QA Testing Report) | **NOT ready** — three High | The page **did not render at all**: `types.ts` and the components consumed `gateBuilt` / `withheldWithoutGate` while the server produced neither, and a **green 764-test run** hid it. The account lookup threw a second way — the resolver's `basis` **object** rendered as a React child — and counted "capabilities in force" with `value !== false`, saying **26 of 38** where the resolver says **19**, next to a card on the same screen saying 19. Plus: an agent-platform-only id returned 200 with a confident panel instead of the documented 404 |
| **2** (§ QA Testing Report — round 2) | **Ready for the user's review; two small fixes before commit** | All three High and the Medium **closed and attacked** — eight mutations, every one caught, three matching Dev's claimed failure counts exactly (6 / 5 / 2). Dev's compile-time claim independently verified: forcing `basis` back to `string` yields `TS2352`. Four new findings, the sharpest being **NEW-1** — the new "no gate yet" marker fires on the **ten `not_built` capabilities**, telling a reader the product currently allows features that **do not exist** — and **NEW-3**, the error vocabulary, which is the same "nothing compares the two sides" gap one layer down, and which a code added *in this very fix* (`tenant_check_failed`) already fell through |

**The lesson worth carrying out of this module**, and the reason round 1's finding matters more than any individual bug: *a fixture is a statement of what we believe the server sends; something must compare it to what the server does send.* Two suites can both be green, thorough and well-written and still never meet. Round 2 confirms the fix generalises — the plans contract is closed at runtime, the accounts contract at compile time **and** runtime, and both fail in both directions when probed.
...
| 2026-09-24 | QA re-verification of the Business OS Tiers admin page: three High closed, four new findings (QA) | Added §14.15 as a pointer to the page workplan, where both rounds live in full. Round 2 attacked every fix by mutation: eight mutations, all caught, three matching Dev's claimed counts exactly. Verified independently that forcing `AccountPayload.basis` back to `string` produces TS2352, so the recorded-body assignment really is a compile-time check; and that both payload contracts fail in both directions. New: NEW-1 the "no gate yet" marker fires on the ten `not_built` capabilities and reads as "the product allows this"; NEW-2 the tenancy rule is a second copy that diverges under a repository error; NEW-3 the error vocabulary has no contract and `tenant_check_failed` has no copy; NEW-4..7 Lows including two negative controls that cannot fail. 30 suites / 789 tests, authz guard 74, lint clean, typecheck 2,077 with zero in this branch. |
