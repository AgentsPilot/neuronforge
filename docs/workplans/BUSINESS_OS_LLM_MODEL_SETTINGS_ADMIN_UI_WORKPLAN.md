...
**Status:** **Slice 1 MERGED (PR #98). Slice 2 code complete; SA review (Fix Required) and QA report (46/7/2/1, no High) both closed — ALL SA AND QA FIXES APPLIED — F-1, F-3, F-4, F-5, F-6 plus F-7…F-11; awaiting SA/QA re-check → the user's view → RM** (uncommitted). Slice 2's gates, mutations and deviations are in §5.3 / §5.3b / §5.3c / §5.4; RC-D and DEF-6 are both closed there. **F-2 is deliberately NOT fixed here** — it changes a required gate and is the user's call (§5.3b). → **F-2, and both of §5.3d's "recorded, not fixed" items, are now implemented on branch `fix/admin-guard-hardening-and-lint`** — see [ADMIN_GUARD_HARDENING_AND_LINT_WORKPLAN.md](/docs/workplans/ADMIN_GUARD_HARDENING_AND_LINT_WORKPLAN.md).
...
| **S2-T3** | ~~Provenance badges render from the payload~~ **— first half superseded 2026-09-24 by […ADMIN_SCREEN_REFINEMENTS…](/docs/requirements/BUSINESS_OS_LLM_ADMIN_SCREEN_REFINEMENTS_REQUIREMENT.md), FR-6** (old text left visible): the three-way badge is replaced by exception-only markers on a database-vs-code axis, asserted by R-T8…R-T11 in `page.render.test.tsx`. **Second half UNAFFECTED and still biting:** `temperature` "not set" renders as **"not set — the provider default applies"** | AC-4 |
...
| **S2-T6** | Locked controls are disabled with their reason **as text** (not tooltip-only). **2026-09-24 — narrowed, not withdrawn:** the `enabled` control and its lock reason left the page with the switch surface (refinements FR-1), so what survives is the pair of TEMPERATURE lock texts — and under FR-6 those are now the **only** carrier of "code-owned by policy", which makes this row more load-bearing than it was. Re-homed as **R-T18** | AC-12 |
...
| **S2-T8** | **No string on the page** asserts an area is off (a rendered-output scan outside the `Configured: off` chip) — **kept verbatim 2026-09-24 and extended over the new call-level chip**. ~~`FailOpenNotice` renders beside **every** switch with no dismiss affordance~~ **superseded by refinements FR-5**: the component is deleted and the obligation is vacuous while no switch is on the page (see the open obligation under FR-17 of the approved requirement) | AC-16 |
...
| **S2-T11** | After a (mocked) successful write the page states the ~60 s propagation and that the ledger check only starts counting after it. **2026-09-24 — re-pointed, not deleted:** the ledger panel is PARKED (refinements FR-3), so the page half now asserts `PAGE_STANDING_NOTE` in the header (R-T3), and the panel's own ledger-specific clause gained a **new** assertion inside `ledgerPanel.render.test.tsx` (**R-T19**) so it stays under test while unmounted. Both compose one private `PROPAGATION_CLAUSE` | FR-20 |
...
> ✅ **F-2 FIXED 2026-09-24, on its own branch** (`fix/admin-guard-hardening-and-lint`, with OI-21 and the lint entry point). It was not the one-line `toMatch` proposed above: F-1's own history shows a shape match is defeated by a `try/catch` or an early return, so R6 now asserts the **first-statement** property, and the rule was **extracted to `tests/helpers/admin-page-guard.ts`** so this suite and the required gate share one implementation. Mutation-proved eight ways against the real layout. See the other workplan for the evidence table.
...
> **Update 2026-09-24 — all three are now CAUGHT, and SA's ruling still stands.** On the F-2 branch the first statement must **be** the guard call rather than contain it (an anchored match, with an optional `const … =` so the legitimate `const admin = await requireAdminPage();` still passes), which rejects the `&&` and ternary shapes; and the shadowed no-op is caught by requiring the identifier to be imported from `@/lib/admin/requireAdminPage` with no local declaration of the name. That is the fourth regex round SA named, taken deliberately: F-2 needed the **required** gate raised to the level this suite had already reached, and adding a rendering test to a pure static-scan suite that gates every merge in the repo is a bigger change than the fix it delivers. **The behavioural test remains the durable answer** and is recorded in the shared module's header and with OI-20.
...
> ✅ **Property 1 is now ASSERTED (2026-09-24, same branch as F-2)** — rule **R8** of the required guard: every render entry point under `app/admin/**` is a client component with no `async` default export, nothing exempted, mutation-proved four ways (including a **client** page beside a **server nested layout**, which a `page.tsx`-only rule would miss). **Slice 3 will now fail CI if it introduces a Server Component admin page**, which is what this entry asked for. ⚠️ The header bypass **itself** is still open — the `curl` still returns 200 with the guard never running; what is closed is that the condition making it harmless can no longer break silently.
...
| Date | Change | Details |
|------|--------|---------|
| 2026-09-24 | **Slice 2's screen refined (separate cycle) — four rows superseded or re-pointed** | Implemented on `feature/business-os-llm-admin-screen-refinements` against […ADMIN_SCREEN_REFINEMENTS…](/docs/requirements/BUSINESS_OS_LLM_ADMIN_SCREEN_REFINEMENTS_REQUIREMENT.md). **Deleted:** `components/FailOpenNotice.tsx` and seven copy strings (`FAIL_OPEN_*` ×4, `LOCK_AREA_FAIL_OPEN`, `LOCK_AREA_NOT_SWITCHABLE`, `LOCK_CALL_NOT_SWITCHABLE`) plus `PROVENANCE_LABEL` / `PROVENANCE_TITLE` and the local `ProvenanceChip`. **Parked, NOT deleted:** `components/LedgerCheckPanel.tsx` — unmounted from `AreaCard` with a dated comment; the component, `ledgerCheckCopy.ts`, the `…/ledger` route and every one of their tests stay and stay green, and the route's own header now records that it is not dead code. **Added:** `markers.ts`, `components/Marker.tsx`, `components/ExcludedCallRow.tsx`. **Superseded rows:** S2-T3 (first half), S2-T6 (narrowed), S2-T8 (second half), S2-T11 (re-pointed) — old text left visible in each |
