...
| `lib/business-os/llm/aiUnavailableMessages.ts` | created. `AI_UNAVAILABLE_CHAT` **and** `AI_UNAVAILABLE_WEBSITE_WRITING`, both `Record<'en'\|'he'\|'es', string>`, plus `aiUnavailableLanguage()` / `chatUnavailableMessage()` / `websiteWritingUnavailableMessage()`. **Not** `respondChatUnavailable(...)`: the three chat routes have three different success shapes (v4's turn, v2's `data.message`, v1's `data.response`), so a shared responder would have been a switch on the caller wearing a helper's clothes. Each route builds its own shape and takes only the sentence. Imports nothing. Note (RC-W9): its callers (chat v1/v2) join the `typecheck:bos-llm` scope anyway, because `lib/business-os/llm/` is a CORE dir |
...
| `app/business-os/website/page.tsx` | `LABELS.ai_unavailable` added in en/es/he; shown in the generate-from-profile notice (as a **warning**, not an error — nothing failed and nothing was overwritten) and **once at the top of the block being edited**, scoped to that block, rather than beside each of the eleven AI-writing controls: every one of them is off for the same reason at the same moment, and eleven copies of one sentence is noise. The 2 `console.error` converted (D-46) |
...
| tests | **`callParams.boundary.step3.test.ts`** (new — T3-S/T3-M for chat + images, and the four blind spots for those three sites), **`modelSettings.off.chat.test.ts`** (new — T3-G/T3-O, including the v1/v2/v4 gates in all three languages, the parked-write and budget placements, and images-off-with-reuse), **`aiUnavailableMessages.test.ts`** (new — T3-L plus the server/client parity checks), the four blind-spot suites appended to **`callParams.boundary.step2.test.ts`**, and the inverted assertions in `modelSettings.off.nonchat.test.ts` / `modelSettings.test.ts` / `modelSettingsPolicy.test.ts` / `scripts/__tests__/bos-llm-settings.test.ts`. `GeneratedImageService.attribution.test.ts`, the chat attribution suite and the chat-v4 audit suite keep their assertions with the resolver mocked (D-51) |

### 7.2 Tasks

- ⬜ **T3.0** (Q-8) **NOT DONE — owed, and deliberately not faked.** One image call with an unknown model on a non-production OpenAI key, to record the status/code. It needs a live key and a live call; no non-production key or test environment exists (§7.5). (The worktree's gates now run — D-49 — but that changes nothing here: this check is a real call to OpenAI, not a local one.) The classifier was not widened, so nothing in this step depends on the answer: an unclassified image refusal simply is not retried, which is today's behaviour. **Carried to QA / the user.**
- ✅ **T3.1** Planner (+ FR-13, loop model = `modelUsed`, RC-W6) · ✅ **T3.2** Analysis · ✅ **T3.3** Message module · ✅ **T3.4** v4/v2/v1 gates (RC-W2 placement) · ✅ **T3.5** Images (resolved model in request and price, price built inside the attempt, off after reuse: RC-W4) · ✅ **T3.6** `full_site` `'fail'` callers (route + mutate; mutate still creates the page, RC-W3) and the three website locks flipped · ✅ **T3.7** regenerate/testimonial off + the page notice (+ the 2 `console.error` converted — D-46) · ✅ **T3.8** Tests (T3-S captured-unwired-then-replayed, T3-G, T3-O, T3-M, T3-L, plus the four blind-spot suites, all mutation-proved — D-55); gates in §7.4, all green · ⬜ **T3.9** SA → QA → user → RM.
...
**Run 2026-09-21 (Dev), verbatim, after the SA Step 3 review fixes:**

```
> npm run typecheck:bos-llm          (run 1)
typecheck-bos-llm: 177 files in scope, 31 errors, 0 new (83.9s)
typecheck-bos-llm: passed

> npm run typecheck:bos-llm          (run 2)
typecheck-bos-llm: 177 files in scope, 31 errors, 0 new (89.0s)
typecheck-bos-llm: passed

> npm run typecheck:bos-llm          (run 3)
typecheck-bos-llm: 177 files in scope, 31 errors, 0 new (83.5s)
typecheck-bos-llm: passed
```

**What is stable here is the VERDICT, not the file count (QA D3-3).** Three runs in a row return `31 errors, 0 new` and `passed`, and that is the claim: the gate reaches the same answer every time. The **scope count is NOT stable and is not offered as evidence** — QA measured `177` then `167` on the same tree, because the walk includes generated `.next/types/**/route.ts` shims and therefore depends on whether a build has been run in the checkout. That is **FU-2**, a known local-reporting artefact; CI runs checkout → `npm ci` → the gate with no build, so `.next/` never exists there and the count is deterministic. Read "files in scope" with that caveat and compare only `errors` and `new` between rounds.

The instability that mattered was real and is fixed. SA saw `32 errors, 1 new` twice where an earlier round recorded `0 new`: the baseline key added for `chat-v2` embedded a **truncated** union (`… 11 more …`) whose member order is not stable across program shapes, so the same error matched the baseline on one run and not the next — reported as both "fixed" and "new". The key is deleted and the underlying bug fixed instead (see below), and the error count drops 32 → **31**.

```
> npm run build
 ✓ Compiled successfully
 ✓ Generating static pages (293/293)
(exit code 0)
```

```
> npx jest lib/business-os app/api/business-os app/api/website app/api/intake app/api/onboarding app/api/cron/insight-detect lib/services lib/ai scripts --ci
Test Suites: 134 passed, 134 total
Tests:       28 skipped, 2272 passed, 2300 total
Snapshots:   25 passed, 25 total
```

`--ci`, so no snapshot could be written to make itself pass. The count has moved twice since the first Step 3 round, both times because tests were added rather than changed: 2,260 → 2,264 for the SA round (the confirm branch ×2, the D-54 and S1-7 policy guards), and 2,264 → **2,272** for the QA round (the eight FR-13 reporting sites, D-60).

```
> NODE_OPTIONS=--max-old-space-size=8192 npx tsc --noEmit | grep -c "error TS"
2034          (2,030 excluding the 4 in generated .next/types)
```

**QA's numbers, and they are the right ones (QA D3-4).** An earlier round of this section recorded `2035 / 2031`; that measurement was taken **before** the `chat-v2` `entityType` fix, which removes exactly one error. Re-measured on the finished tree, the Dev gets `2034 / 2030` — identical to QA. The count that matters is unchanged either way: **Step 3 adds none.** Step 2 recorded `2,031` on a tree that still contained the `chat-v2` bug, so `2,030` is the same number with one pre-existing error fixed.

Nine of those errors are in files this step touches, and every one was verified to pre-date it by reading `HEAD`: `chat/route.ts:275` (`fetchExistingServices`), `regenerate/route.ts:83` (`profile || undefined`), `WebsiteSectionService.ts:532` (the KI-1 request cast), and six in `app/business-os/website/page.tsx` at lines 1337-6140, none within 3,000 lines of this step's edits.

**`--list` diff (RC-W9) — the prediction held.** Two **production** files join the scope, exactly as RC-W9 said they would, along with the new CORE module and the three new test files:

```
caller            app/api/business-os/chat-v2/route.ts
caller            app/api/business-os/chat/route.ts
core              lib/business-os/llm/aiUnavailableMessages.ts
core              lib/business-os/llm/__tests__/aiUnavailableMessages.test.ts
```

Bringing chat v1 and v2 into scope surfaced 2 pre-existing errors in them. **One was fixed, one is baselined:**

| File | Error | Disposition |
|---|---|---|
| `app/api/business-os/chat-v2/route.ts:151` | `TS2322` — `entityType: 'chat'` is not a member of `EntityType` | **FIXED** (SA finding 1). It is now `'system'`, which is what chat-v4 already writes for its own turn and write entries, so the two chat versions agree. This was never a valid value; it was simply never type-checked, because this route was outside the gate's scope until Step 3. The baseline entry is **deleted** — baselining it was the wrong call, and the truncated-union instability is what made that visible |
| `app/api/business-os/chat/route.ts:275` | `TS2322` — `duration: number \| null` vs the helper's `number \| undefined` | **Baselined**, 1 occurrence. A `?? undefined` would change what the intent parser is told about a service with no duration — a product decision for whoever owns v1, not a Layer 2 one. The gate now holds it at exactly one occurrence, so it cannot grow |

### 7.5 Rollout notes

New messages appear only when a row switches something off. After deploy, QA: AC-13 item 3 for chat and images.

**The website kill switch is now whole (S1-7 closed).** `set website --enabled false` stops all eight website calls. `full_site`, `field_regenerate` and `testimonial_enhance` are `switchable: true` in the policy, their ★ "AI writing is unavailable" paths shipped in this step, and the change script's "PARTIAL SWITCH" warning is **gone** (D-45): it would otherwise have self-cleared for `website` and then started naming `chat/planner`, which is not a call that keeps spending — the chat area gate stops it at route entry. What guards the underlying risk now is a test, `modelSettingsPolicy.test.ts`'s "leaves no call that keeps spending when its area is switched off", which asserts the set of non-switchable calls in switchable areas is exactly `['chat/planner']`. The runbook (§8, FR-18) must describe the switch as complete, with that one documented exception.
...
### 7.6 Implementation notes and deviations (Dev, 2026-09-21)

**Branch:** `feature/business-os-llm-layer2-step3`, cut off `feature/business-os-llm-layer2-step2` and **merged up to `origin/main` `cddc1916`** once PR #85 landed — a fast-forward, no conflicts, because that commit *is* Step 2's merge.

**What was wired: 3 call sites, 3 entry gates, 5 owner-facing off paths.** The three remaining catalogued calls (`chat/planner`, `chat/analysis`, `images/image_generation`) now take `{ provider, model, temperature }` from `resolveBosLlmSettings` and build their request inside a `withModelFallback` attempt. **Leg (A) is now empty** — all 22 calls are wired, and `callParams.snapshot.test.ts` asserts the empty set rather than merely happening to have one. No attribution, grouping id, `runAiAction` scope, `callWithTracking` call or audit entry changed anywhere.

| # | Note |
|---|---|
| **D-41** | **The planner's loop model is a `let`, reassigned from `modelUsed` after every attempt (RC-W6).** `withModelFallback` is called with `{ ...settings, model }` rather than `settings`, so after a fallback the next iteration's `model` already equals `defaultModel` and the wrapper passes straight through instead of re-trying a model it knows is refused. Every `fail(...)`, every `diagnostics({ model })` and the plan-cache `store({ model })` therefore name the model that RAN (FR-13). The 3-attempt repair loop is the reason this matters: without it, attempt 2 would re-send the model attempt 1 was refused for, and a configuration mistake would cost three failed calls per question instead of one. |
| **D-42** | **The planner keeps no off branch, deliberately (D-27).** `chat/planner` is `switchable: false`, so `settings.enabled` is always `true` there; the chat area's switch is enforced at the entry of all three routes. A branch for `false` inside the planner would be dead code implying a second mechanism. |
| **D-43** | **The chat v1 gate sits after Zod, not merely "after auth" as §7.1 says.** §3.8 places it "after auth" for v1 and "after auth, the feature flag and Zod" for v2. Putting it before validation would answer a malformed body with a sentence about the assistant instead of its 400, and SA's Step 2 finding 12 explicitly credited the off branches for sitting *after* auth and validation. Both v1 and v2 now gate after validation, and T3-G asserts that a malformed body still gets 400 and a stranger still gets 401. |
| **D-44** | **The chat mutate path reports through a new `notice` field, following the existing `url` convention.** §7.1 asked for "the preview link plus the translated sentence". `executeMutate` already appends a handler-returned `url` to the confirmation line (the `:804-805` precedent); `notice` is the same shape for a sentence, and `preview` is now `[preview, url, notice].filter(Boolean).join(' — ')`. Generic, not landing-page-specific: any handler that succeeds while part of what the owner expected did not happen can use it. The page is still CREATED (Q-7 reversed), nothing is written into it, and no provider call is made. |
| **D-45** | **The change script's "PARTIAL SWITCH" warning was DELETED, not left to self-clear.** It listed the calls of an area that `isSwitchableBosLlmCall` says cannot be switched off. With the three website calls flipped, that list is empty for `website` — but the predicate would then have started naming `chat/planner` on `set chat --enabled false`, and the planner is **not** a call that keeps spending: it has no switch of its own precisely because the area gate stops it. The warning would have told an operator their kill switch was partial exactly when it had become complete. The underlying S1-7 risk is now guarded by a test instead of a runtime line nobody may read: `modelSettingsPolicy.test.ts` asserts the set of non-switchable calls inside switchable areas is exactly `['chat/planner']`, so a future call added with `switchable: false` and no off path fails the suite. `scripts/__tests__/bos-llm-settings.test.ts`'s S1-7 case was inverted to demand the warning's ABSENCE for both `website` and `insights`. |
| **D-46** | **`console.*` in touched files: 2, and both converted.** `app/business-os/website/page.tsx` had two `console.error` calls (services fetch, services refresh) and already imported `logger`; both are now `logger.error({ err: error }, '…')`. **The user approved the conversion on 2026-09-20**; §12's "pending user OK" note was stale and has been corrected. SA re-grepped all 16 touched files: **0 `console.*`**, so §12 needs nothing further. |
| **D-47** | **The four T2-S blind spots are closed for all 19 Step 2 sites and all 3 Step 3 sites — except one half of SA-2.** (a) SA F-1: the `getProvider` spy now RECORDS its argument, and every site asserts `['openai']`. (b) QA D2-1: `resolveBosLlmSettings` is wrapped (not stubbed — the real resolver still runs) and each site asserts it asked for its OWN `(area, call)` key, exactly once. (c) QA D2-2: a new `it.each` over all 19 sites overrides that call's model to `gpt-4o-2026-override` and asserts the boundary carries it. (d) QA D2-3: the same loop refuses the override with a 404 `model_not_found` and asserts the boundary shows `[override, default]`. **SA-2 is closed only at the site SA named** — `generateWebsite`'s public entry must still reach `website/full_site` exactly once. Driving all nineteen public entry points is a larger piece of work than this step; the residual limit is that a public caller which stops calling one of the other sixteen privately-driven sites is still invisible to T2-S. Recorded rather than assumed away. **None of this touches `boundaryCalls()` or the committed Step 2 snapshot**, whose whole value is that it was captured unwired and has not moved. |
| **D-48** | **T3-S uses Step 2's method: captured unwired, replayed wired. Done, not owed.** The three production files were reverted to `HEAD` (the wiring diff saved to a patch first), the snapshot file deleted, and the AC-2 tests run against the **unwired** sources: *3 snapshots written*. The wiring was then re-applied and the whole suite re-run with `--ci`, which never writes: **3 snapshots passed, unchanged.** That is the AC-2 before/after comparison made on the same code path rather than on two descriptions of it. What is recorded is the **whole argument list of every provider call plus the call count**, with strings over 200 characters digested as `sha256:<hex> (len N)` — byte-exact, since one changed character changes the digest — and every value AC-2 is about (model, temperature, `frequency_penalty`, `max_tokens`, roles, size, quality, the whole attribution context) recorded literally. **Two things had to be added for the capture to be meaningful, and both are stated in the file:** (a) a `SystemConfigService` mock serving every key its documented default; (b) the SA F-1 / QA D2-1 assertions are kept in their own `describe`, deliberately OUT of the snapshot, because unwired sources call no resolver and an assertion that failed before `toMatchSnapshot()` would have stopped the capture being written. **SA reviewed this and rated the equivalence STRONGER than the Dev argued it:** the seed migration `COALESCE`s each legacy key into the area row (`20261003_…:106-111`), so a mock returning the documented fallback is exactly what the real service does when a key is absent — the equivalence is structural, not contingent on P-2's "nothing was stored". |
| **D-49** | **The gates were blocked, then run. What the block cost is worth recording.** The worktree initially had no `node_modules` and no `.env.local`, so `typecheck:bos-llm`, `build` and `jest` all failed with `'next' is not recognized`; creating the junction Step 2 used (D-40) and `npm ci` were both refused by the environment, and **nothing was worked around** — the step was reported as unreviewable instead. The user restored both. **What the tests found on their first ever run: 8 failures across 2 of the 6 new/changed suites, every one a fault in the TEST harness, none in the production code.** (1) `GeneratedImageService` uploads the PNG before returning, and the suite's `supabaseServer` mock had no `storage` — the service's own catch turned the throw into `{ ok: false, reason: 'failed' }`, so three image cases failed looking exactly like a wiring bug. (2) The analysis fixture was a `compute` result with no `agg`; `buildAnalysisPayload` describes each step as "<fn> of <entity>.<field>", so it threw inside `analyse`'s own try, which returns null and makes **no provider call** — three more, again looking like an unwired site. (3) `modelSettings.off.chat.test.ts` replaced `PendingFillStore` and `ConfirmationStore` wholesale, leaving `isCancelMessage` and `readConfirmationReply` undefined, so the parked-write case got a 500 — a TypeError that looked precisely like a missing gate. (4) The same file mocked `AnalysisService` to a throwing spy and then imported `analyse` from that mock to drive the analysis-off case; it now reaches the real module through `requireActual`. (5) On the replay, the planner's provider assertion had to become a SET rather than a list: a turn legitimately asks the factory twice, because the plan cache's lookup embedding is a separate DEC-3-excluded call on the same provider. **The moral is the one Step 1 already recorded: tests written while the toolchain is down are drafts.** Four of the five would have been caught by nothing except running them. |
| **D-54** | **`settings.provider` is honoured at the two chat sites and ignored at the other twenty — SA ruled: LEAVE IT, and guard it.** SA F-1 ended with a fork: pass `settings.provider` at the sites, or declare the hardcoded `'openai'` deliberate. Step 3 took the first half for the two calls it wired (`ProviderFactory.getProvider(settings.provider)` in the planner and the analysis service); the nineteen Step 2 sites still pass the literal, and `images/image_generation` cannot take a provider at all because it goes through `getOpenAI()` (a second image provider needs a capability interface, not a provider key — F-15). **SA's ruling: do not retrofit.** The divergence is inert, because `allowedProviders` is `OPENAI_ONLY` for all 22 calls, so the resolver can never hand the two reading sites anything else. But the ruling depends on that, and the boundary suites' `getProviderArgs` assertion cannot guard it — it sees `'openai'` either way and cannot tell a literal from a resolved value. So the guard is a **policy test**: `modelSettingsPolicy.test.ts` asserts `ALLOWED_PROVIDERS_LAYER2 === ['openai']` and that every one of the 22 calls has `allowedProviders: ['openai']`. The moment any call is allowed a second provider, the nineteen literal sites would silently keep using OpenAI while the row said otherwise — and that test fails first. |
| **D-52** | **SA F-9 taken the second way SA offered: the dependency is RECORDED, not the seven resolves moved.** SA's finding was that `BriefingNarrator`, four `WebsiteAIContentService` block methods, `IntakeGenerationService` and the infer-question route call the resolver OUTSIDE the `try` that owns their template fallback, so their safety rests on "the resolver never throws" holding for ever — and SA allowed either moving the calls or writing the dependency down. `modelSettings.ts`'s header now names all seven sites, says which test keeps the promise true (T1-5), and tells the next author to move them before adding any throwing path. **Why not move them:** at every one of the seven the resolved `enabled` is read BEFORE the `try`, to choose the off branch, so "move the resolve" is really "move the off branch inside the `try` too" — a restructure of seven fallback paths. **SA reviewed and accepted this: no move.** |
| **D-53** | **F-11 fixed at the source as well as the callers.** `generateWebsite`'s `ai_unavailable` refusal now carries `error: AI_UNAVAILABLE_WEBSITE_WRITING.en` alongside `code`, so a caller that logs or shows `result.error` — and several do — no longer shows `undefined`. The two Step 3 callers switch on `code` and use their own translation; the `error` string is for everyone else. |
| **D-55** | **The four blind-spot closures are mutation-proved, not asserted.** Each mutation was introduced against a REAL call site, the suite run, the mutation reverted, and the suite re-run green. **F-1 (provider identity):** `Planner.ts` switched to `ProviderFactory.getProvider('anthropic')` → **1 failure**, `T3-S: each site asks for its own settings key, from openai › chat/planner`. This is the escape SA demonstrated on `BriefingNarrator` with 20/20 boundary tests staying green; it now bites. **D2-1 (a site resolving a neighbour's key), QA's exact M7:** `hero_content`'s site changed to resolve `'faq_content'` → **3 failures**, led by `website/hero_content resolves its own key`. Worth noting precisely: **the boundary SNAPSHOT still passed**, because those two calls resolve identical values — which is the blind spot, and why the key has to be recorded separately from the request. **D2-2 (the resolved model reaches the boundary), QA's M1:** `hero_content` re-hardcoded to `model: 'gpt-4o-mini'` inside the attempt → **2 failures**. 123/123 suites stayed green on this in Step 2. **D2-3 (the request is built inside the attempt), QA's M8:** `hero_content` pinned to `model: settings.model` inside the attempt → **exactly 1 failure**, and D2-2 still passes — the right discrimination, because the first attempt does send the resolved model and only the retry is wrong. |
| **D-50** | **`config.model` is now dead at its only reader.** `SystemConfigRepository.getImageGenerationConfig()` still returns a `model` field (from the `image_generation_model` key), and `GeneratedImageService` no longer uses it for the request or the price — both come from the `images` area row. The field was left in place: it is part of a shared config shape, the admin image-configuration surface may still show it, and removing it belongs with whatever owns that surface, not with this step. Recorded as a follow-up (FU-4) rather than a silent orphan. |
| **D-51** | **Five existing suites needed the resolver mock or an inverted assertion.** New resolver mocks (the D-38 shape, pinned to the code defaults): `lib/business-os/bizql/__tests__/llm-attribution.test.ts` (planner + analysis), `lib/services/__tests__/GeneratedImageService.attribution.test.ts`, and an `isBosLlmAreaEnabled` mock in `app/api/business-os/chat-v4/__tests__/route.audit.test.ts`. Assertion changes: `modelSettings.off.nonchat.test.ts`'s D-35 block now asserts the OPPOSITE (all eight website calls go off), `modelSettings.test.ts` gained the same for the resolver, `modelSettingsPolicy.test.ts`'s lock list lost the three website calls and gained the S1-7 guard, and `aiAudit.routes.test.ts`'s two service mocks return the new typed outcome. The image suite's two "unpriced model" cases now override the RESOLVED model rather than `config.model`, because that is where the model comes from now. |

| **D-56** | **The chat switch does not stop a CONFIRMED landing-page write, and the comment that said it did is fixed (SA High, finding 2).** `chat-v4/route.ts`'s gate comment claimed the confirm/cancel branches "make no model call at all". False for confirm: `applyFrozenWrites` replays the frozen steps, and `pages.create` with `page_type: 'landing'` calls `generateWebsite` — a full `website/full_site` call, made while chat is off. The **behaviour** is right and stays: it is website spend, the WEBSITE switch owns it (`full_site` is switchable since Step 3 and the mutate path passes `onAiDisabled: 'fail'`), and refusing a write the owner already approved would be worse. Only the comment was wrong, and a comment like that is what an operator later relies on. It now states exactly what happens, and two tests pin it instead of describing it (D-57). |
| **D-57** | **The confirm branch is now covered on both sides (SA Medium, finding 3).** Only *cancel* was tested with chat off — the easy direction, because it throws work away. Added: (a) a parked write can still be **confirmed** with chat off (`applyFrozenWrites` is reached, the answer is the write's, not the unavailable sentence, and the planner and analysis are still never called); (b) the spending path, driven through the real `executeMutate` at the joint SA identified — with chat off, `pages.create` for a landing page **does** reach `generateWebsite`, and it asks for `onAiDisabled: 'fail'`; with the **website** area also off, the page is still created, no copy is written, and the reply carries the preview link plus the translated sentence. |
| **D-58** | **The `chat-v2` baseline entry was the wrong fix, and the gate said so (SA High, finding 1).** SA reported `177 files, 32 errors, 1 new` twice where this step had recorded `167 / 0 new`. The cause was the baseline key itself: it embedded a **truncated** union (`… 11 more …`) whose member order is not stable across program shapes, so the same error matched the baseline on one run and not the next, and was reported as both "fixed" and "new". Baselining a pre-existing error is legitimate; baselining one whose *text* is unstable is not, because the key stops identifying the error. Fixed at the root — `entityType: 'chat'` is not a member of `EntityType` and never was, so it is now `'system'`, matching what chat-v4 already writes — and the baseline entry deleted. Error count 32 → **31**, and the gate now reads identically on three consecutive runs (§7.4). The remaining `chat/route.ts` entry is stable (no truncation) and stays. |
| **D-59** | **The workplan's line endings were normalised (SA Medium, finding 4).** SA saw 4,839 changed lines for ~103 real insertions. The working copy was CRLF while the blob is LF, so any diff taken without `core.autocrlf` normalisation showed the whole file. It is now LF, and `git diff --stat` reports **~200 insertions / 12 deletions with autocrlf either on or off** (the exact count moves as this section is edited; the point is that it is the same under both settings) — reviewable under any reader's settings. Other source files are left alone: they are CRLF because that is how this repo checks out, and the repo's own `git diff` normalises them. |

| **D-60** | **FR-13's model-reporting paths are now pinned at all nine planner sites, not one (QA D3-1).** QA found that a sentinel model at `Planner.ts:558, :592, :605, :620, :709, :781, :792, :802` left 891/891 and 2176/2176 green: only the repair case was covered, so eight of the nine places the planner NAMES a model were defended by nothing. Two of them were pointed: **`:605` is SA finding 5's entire fix from the previous round**, and **`:709` is the plan-cache `store({ model })`** — a cache entry keyed to a model that never ran would later be replayed as though it had been produced by one. Seven new cases in `callParams.boundary.step3.test.ts` drive each reachable path with the configured model REFUSED, so the fallback runs on the code default, and assert the path reports the default. **All eight mutation-proved, one at a time with a revert between each: every sentinel failed exactly one test, and exactly the intended one**; `Planner.ts` was then proved byte-identical to its starting state. **`:802` is unreachable** — the malformed-JSON branch only `continue`s on `attempt === 0` and the validation branch only on `attempt < 2`, so the last iteration always falls through to `:792` — and its test says so instead of pretending to drive it, checking by reading that the statement uses the shared `model` shorthand rather than a literal. |
| **D-61** | **The kill switch FAILS OPEN, by design, and that was undocumented until now (QA D3-5).** `isBosLlmAreaEnabled` returns `true` when the settings read throws or the cache is unreadable (`modelSettings.ts`), so **a database problem silently re-enables an area an operator switched off**. That is FR-6 working as specified — a configuration fault must never be worse than today's behaviour, and the alternative (fail closed) turns a Supabase blip into a Business OS outage — and it is the right trade for a platform whose switch is used mostly to change a model, not to stop a fire. But it is exactly the wrong thing to discover later: an operator who switches an area off **to stop a cost runaway** and walks away must know the switch can lift itself if the read fails, and that the only signal is the resolver's own error log. **Documented, not changed** (changing it needs its own decision, and it is not a Step 3 one). §7.7 carries it as a Step 4 runbook item, where FR-18 owns the operator-facing text. |

### 7.7 Follow-ups recorded out of Step 3 (do not widen this step)
...
| website | `full_site` | `complete()` JSON | openai → openai | `gpt-4o` | `gpt-4o` (C) | 0.7 | 0.7 (A) | on → on | yes (since 3) | 2 (params), 3 (off outside build) |
| website | `landing_page` | `chatCompletion` JSON | openai → openai | `gpt-4o` | `gpt-4o` (C) | 0.7 | 0.7 (A) | on → on | yes | 2 |
| website | `field_regenerate` | `complete()` | openai → openai | `gpt-4o-mini` | `gpt-4o-mini` (A) | 0.7 | 0.7 (A) | on → on | yes (since 3) | 2 (params), 3 (off) |
| website | `testimonial_enhance` | `complete()` | openai → openai | `gpt-4o-mini` | `gpt-4o-mini` (A) | 0.5 | 0.5 (C) | on → on | yes (since 3) | 2 (params), 3 (off) |
...
| After Step 2 deploy: one action per Step 2 area; ledger `model_name` matches "after" | QA | **⚠️ STILL OWED (user).** Step 2 merged as PR #85 on 2026-09-21, but QA2-8's six owner actions and two SQL queries have **not** been run against production. Nothing in Step 3 assumes they were: Step 2 is merged, not verified |
| After Step 3 deploy: chat and images | QA | *(pending)* |
| Step 3 wiring: request at the provider boundary, before vs after | Dev | **2026-09-21: 3 snapshots captured on the UNWIRED call sites, then re-run against the wired ones with `--ci` — 3 passed, nothing moved** (`callParams.boundary.step3.test.ts`; §7.6 D-48). Four mutations prove the four blind-spot closures bite (D-55), including QA's own M7 — which the snapshot alone still does not catch, and the new settings-key assertion does |
...
| `app/business-os/website/page.tsx` | **2** → **0** | 3 | **Converted** (D-46): both `console.error` calls are now `logger.error({ err: error }, '…')` through the `logger` the file already imported. **The user approved this on 2026-09-20** ("2. agree", converting them during Step 3), so it is settled, not pending. Not a separate commit — this step commits nothing |
...
## SA Code Review — Step 3

**Reviewed by SA — 2026-09-21**
**Status:** 🔄 Fix Required — 2 High, 2 Medium, 1 Low. The implementation is sound; the **gate is red**, and one load-bearing code comment states a guarantee the code does not make.

**Verified against the code, not the summary.** Re-ran `tsx scripts/typecheck-bos-llm.ts` twice and the six Layer 2 Jest suites (18 suites / 411 tests / 23 snapshots — **green**). Read every production hunk. Traced the confirm branch, the `pages.create` handler and the seed migration by hand.

### Code Review Comments

1. **`scripts/typecheck-bos-llm.baseline.json:2` + `app/api/business-os/chat-v2/route.ts:151` — the gate is RED, and the new baseline key is unstable — Priority: High.**
   My run, twice, deterministic: `177 files in scope, 32 errors, 1 new (85s) … FAILED`. Two problems, one cause.
   *The key is a truncated union whose member order is not stable across program shapes.* The Dev recorded `Type '"chat"' is not assignable to type '"agent" | "ai_pricing" | … | 11 more …'` (alphabetical). The same error now prints `'"user" | "proposal" | "profile" | … | 10 more …'` (declaration order). The gate therefore reports the baseline entry **"fixed"** *and* the error **"new"**, and exits 1. Any baseline key containing `… N more …` is a coin flip on program size.
   *The reported run is also stale:* §7 records `167 files / 32 errors / 0 new`; the branch as it stands is `177 files`. The gate numbers in the workplan were not taken from the final tree.
   **Fix:** do not baseline this one. `entityType: 'chat'` is not a member of `EntityType` (`lib/audit/types.ts:145`) — it is a genuine pre-existing bug that writes an out-of-set value into `audit_trail.entity_type`. Chat-v4's own write already uses `entityType: 'system'` (`app/api/business-os/chat-v4/route.ts:700`); use that, delete the baseline entry, and re-run and re-record the gate. The chat-v1 `duration` entry is stable (no truncated union) and is fine to keep baselined.

2. **`app/api/business-os/chat-v4/route.ts:731-746` — the placement rationale is factually wrong for the confirm branch — Priority: High.**
   The comment states: *"Those paths make no model call at all, so letting them run costs nothing"* and *"this is a spend switch, so it must stop the spend."* The **cancel** branch is indeed free. The **confirm** branch is not:
   `applyFrozenWrites` → `executeMutate` → `HANDLERS.pages.create` (`lib/business-os/bizql/mutate/MutateExecutor.ts:719`) → `new WebsiteGenerationService().generateWebsite(...)` (`:822`) is a full `website/full_site` LLM call — made **while chat is off**.
   This is not a defect in the design: the spend lands in the `website` area, is attributed there, and *is* stopped by the website switch and by the `onAiDisabled: 'fail'` wiring this step added. The defect is the claim. A future reader will take this comment as a proof that the chat gate stops all chat-initiated spend, and it does not.
   **Fix:** rewrite the comment to say what is true — the confirm branch may complete a write that spends in **another** area, governed by that area's own switch — and carry the same sentence into §7.1 and §10.1. This is also a decision the user must hear in business terms (below).

3. **`lib/business-os/llm/__tests__/modelSettings.off.chat.test.ts:332` — the confirm path is untested with chat off — Priority: Medium.**
   The only parked-write case drives `readConfirmationReply('cancel')`. The user-facing promise Q-10 was approved on is that a parked write stays **confirmable**, and the confirm branch is also the one path that can still spend (finding 2). Both are uncovered.
   **Fix:** add a T3-G case — a frozen `pages.create` confirmed with chat off — asserting it applies, returns the `notice`, and makes no *chat*-area call. T3-G's row in §7.3 already promises "confirmed and cancelled"; today it delivers one of the two.

4. **`docs/workplans/BUSINESS_OS_LLM_MODEL_SETTINGS_LAYER2_WORKPLAN.md` — the file was rewritten LF → CRLF — Priority: Medium.**
   `git diff --stat` reports **4,839 changed lines**; `git diff --stat --ignore-all-space` reports **103 insertions, 12 deletions**. The real edit is 2 % of the diff. In a PR this is unreviewable and it will collide with every other branch touching the workplan. `.gitattributes` pins only `*.sh`, so nothing prevents a recurrence.
   **Fix:** normalise the file back to LF before committing. Consider `*.md text eol=lf` in `.gitattributes` as a Step 4 chip.

5. **`lib/business-os/bizql/planner/Planner.ts:571-580` — `fail(...)` in the `catch` can name a model that was not the last one attempted — Priority: Low.**
   `model = modelUsed` (`:527`) runs only after `withModelFallback` resolves. If the **fallback** attempt throws, `withModelFallback` rethrows and `model` is still `settings.model`, while the last request on the wire carried `defaultModel`. Every success path, every `diagnostics(...)`, the cache store and both mid-loop `fail(...)` calls are correct, and re-entry is correct (on `continue`, `model` is already the one that ran, and because `withModelFallback` no-ops when `model === defaultModel` a second fallback cannot occur). Only the terminal `catch` is imprecise.
   **Fix (optional):** have `withModelFallback` attach `modelUsed` to the rethrown error, or write the limit down beside the `let`.

### What I checked and found correct

- **The three gates.** Placement is right in all three. chat-v4 is before the budget refusal and before the planner, analysis, plan-cache embedding (`PlanCache.ts:221`, `:341`) and data layer — all of which are downstream of §4b. chat-v2 and chat-v1 are after auth **and** Zod, so 401/400 still win (D-43); §7.1's "after auth" for v1 is superseded by the stricter placement, correctly. `runAiAction` writes **no** entry when the gate fires, because `aiActionAudit.ts:304` returns early on `calls.length === 0` (FR-7) — the "no ledger row, no audit entry" claim holds. The chat area contains only `planner` and `analysis` (`modelSettingsPolicy.ts:167-177`), and in production both are reachable only from chat-v4; the `scripts/bizql-*` harnesses bypass the gate, which is correct for developer tooling but worth one line in §10.
- **D-48, the capture — the reasoning is sound, and stronger than the Dev claims.** The proof does not actually rest on P-2. `supabase/migrations/20261003_seed_bos_llm_area_settings.sql:106-111` **COALESCEs each legacy key into the area row**, so wired and unwired resolve the *same* value whether or not a legacy key was ever stored. P-2 only fixes which literal the snapshot records; the equivalence is structural. And mocking `SystemConfigService` to return each key's `fallback` argument is not an approximation of "key absent" — it is exactly what `SystemConfigService.getString/getBoolean` does when the key is absent, so the unwired capture ran the real code path. **Ruled: accepted.** One residual worth a line in §7.6: the seed has already run on prod (2026-09-21), so a `bizchat_*` key written *after* it would be silently ignored; the migration's "— superseded by bos_llm_area_*" description marking (`:230`) is the only thing warning an operator off.
- **The four blind-spot closures generalise.** D2-1, D2-2 and D2-3 are `it.each(SITES)` over all **19** Step 2 sites, plus the 3 in `callParams.boundary.step3.test.ts` = 22. Not just the mutated ones. D2-1's assertion is exact equality (`expect(mockResolveArgs).toEqual([[area, callName]])`), so it also catches a site that resolves twice or resolves nothing — a strictly stronger guard than "not the neighbour's key". The `hero_content` → `faq_content` mutation really is invisible to the boundary snapshot (the two rows resolve identical values) and really is caught here; keeping the snapshot untouched rather than widening it was the right call, because its evidentiary value is that it has not moved since the unwired capture.
- **The images change.** `GeneratedImageService.ts:397-408` builds `imagePriceResolver(config.pricesUsd, model, size, quality)` **inside** the attempt from the same `model` the request carries, and captures `priced` inside. A retry can neither price against the refused model nor fall to $0 on a missing table key. The `!settings.enabled` check at `:334` sits after the reuse lookup and before the daily count, so an owner keeps their existing library — correct, and matches N-4. The step3 fixture's per-model prices (0.25 / 0.50) make the price key observable from the log line, which is the only way to test this from outside; and priming the full size × quality matrix for both models avoids `checkImageModel` rejecting the override and passing for the wrong reason.
- **The planner's `let`.** Correct on every path except finding 5.
- **The switch completion.** `modelSettingsPolicy.test.ts`'s new S1-7 guard asserts the non-switchable set inside switchable areas is exactly `['chat/planner']`, which is a stronger and more durable statement than the runtime warning it replaced — the warning would have fired on `chat/planner` and told an operator their kill switch was partial when it is complete. Deleting it was right.
- **Leg (A) is genuinely empty** (`callParams.snapshot.test.ts:302`), and the `it`-with-a-loop rather than `it.each([])` is the right mechanic.
- **Baseline honesty: verified.** `git diff --numstat` = **2 insertions, 0 deletions**; 23 → 25 keys; no existing entry altered. Both added entries exist verbatim at `HEAD` (`chat-v2/route.ts:117`, `chat/route.ts:234-242`) and are untouched by this diff. Nothing was slipped in. Leaving the chat-v1 `duration` one unfixed is right; leaving the chat-v2 one is not — see finding 1.
- **Standards.** Zod on every gated route before the gate. Pino throughout: **0 `console.*` across all 16 touched files**. Every new refusal returns HTTP 200 with a code, which is the correct semantics for "an operator turned this off". `notice` follows the existing `url` convention rather than inventing a shape. No secrets, no RLS bypass, no new direct Supabase access.

### Rulings on the four open questions

| # | Ruling |
|---|---|
| **D-54** — `settings.provider` honoured at 2 sites, ignored at 19 | **Declare the site-level `'openai'` deliberate and document it. Do not retrofit in Step 4.** `allowedProviders` is `OPENAI_ONLY` for every one of the 22 calls (`modelSettingsPolicy.ts:140`, `:222`), so `settings.provider` is provably `'openai'` everywhere and the divergence is **inert** — no operator can produce it. A 19-site retrofit is churn against a value that cannot vary. **But the drift must fail loudly the moment it becomes real:** add to `modelSettingsPolicy.test.ts` an assertion that every call's `allowedProviders` is `OPENAI_ONLY`, with a comment naming the 19 literal sites that must be retrofitted *before* a second provider is allowed. Note that `callParams.boundary.step2.test.ts`'s `expect([...new Set(getProviderArgs)]).toEqual(['openai'])` is **not** that guard — it cannot distinguish a hardcoded literal from `settings.provider`, because the fixture resolves to `'openai'` either way. |
| **D-52** — the seven F-9 resolves outside the `try`, named rather than moved | **Accept. No move.** Both `resolveBosLlmSettings` (`modelSettings.ts:841-851`) and `isBosLlmAreaEnabled` (`:861-867`) wrap everything in a catch-all that returns a value. "Never throws" is therefore a property of the module, not an obligation on seven callers, and running gates does not change that. The header note is correct belt-and-braces and the right shape: it tells a future author who *adds* a throwing path what they owe. |
| **D-46** — the two `console.error` in `app/business-os/website/page.tsx` | **Confirmed.** Both are now `logger.error({ err: error }, …)` (`:2661`, `:2693`), 0 `console.*` remain in the file, and the user approved the conversion on 2026-09-20. §12's correction is accepted. |
| **FU-5 / SA-2** — closed only at `generateWebsite`'s public entry | **Enough for Step 3.** The shape SA named — a public entry short-cutting above the call site — was *introduced by this step* at exactly that site, and is covered there. The complementary direction is covered too: `modelSettings.off.nonchat.test.ts` and `modelSettings.off.chat.test.ts` drive routes and public entries for every other area and assert no provider call. Driving 19 public entry points is real work with a poor ratio. **Track it as a Step 4 chip** and keep the honest comment at `callParams.boundary.step2.test.ts`'s SA-2 block — a written-down instrument limit is worth more than a silent one. Do not block. |

### Optimisation Suggestions

- `MutateExecutor.ts:2162` — `[preview, url, notice].filter(Boolean).join(' — ')` is behaviour-identical to the old ternary when `notice` is absent, and reads better. Good.
- `aiUnavailableMessages.ts` — the "imports nothing, so a client module could import it" note is a real constraint worth an import-guard test eventually, not now.
- Consider `*.md text eol=lf` in `.gitattributes` (finding 4).

### Is Step 3 safe to deploy?

**The code is safe to deploy. Step 3 is not ready to land.**

Nothing in this diff can lose an owner's data, leak across tenants, bypass RLS or Zod, or spend money the operator did not authorise in the area that owns the spend. Every refusal is a 200 with a code, every off path leaves the owner's text untouched, and the fail-open posture of the resolver means a settings fault degrades to today's behaviour rather than to an outage. If it shipped as-is it would work.

It must not land, because **finding 1 leaves `npm run typecheck:bos-llm` red on `main`**, with a baseline key that will keep flapping as the scope grows — which is precisely the gate Step 4 is about to build on. And **finding 2** leaves a comment that will be read as a proof. Fix 1 and 2, add 3, normalise 4, and Step 3 is approved.

### Is the user's standard #1 met when this lands?

**For the 22 catalogued calls: yes.** Leg (A) is empty, verified at `callParams.snapshot.test.ts:302`, and the per-site key/model/retry assertions make that a dataflow statement rather than a text one. Every catalogued Business OS LLM call takes its model from its area row, and every area's kill switch is whole.

**Platform-wide: not literally, and the gap is by prior decision, not by omission.** Two live Business OS chat paths still choose their own model:

| Path | Model | Ledger | Status after Step 3 |
|---|---|---|---|
| chat-v2 `AIDataLayerService.ts:896`, `:1244` | `process.env.OPENAI_CHAT_MODEL` or `'gpt-4o'`, **raw OpenAI SDK** | **Not tracked at all** | Entry gated by the chat switch |
| chat-v1 `IntentParser.ts:116` | `'gpt-4o-mini'` | Catalogued as a known non-catalog component | Entry gated by the chat switch |

Both are documented out of scope (requirement F-13 / V-4, §10.1 "gate only" rows), and Step 3 is what makes "chat off" actually stop them. So the honest statement is: **the kill switch is complete; model resolution is complete across the catalogue.** Neither file will fight the Step 4 CI gate — T4.1 already scopes it to catalog importers, and neither imports the catalog.

### Code Approved for QA: **No** — fix findings 1 and 2, add 3, normalise 4, then yes.
...
**Reviewed by SA — 2026-09-21**
**Status:** ✅ Code Approved for QA — all five findings fixed and all four rulings correctly applied. One Low doc defect, non-blocking.

Scope: the delta since the Step 3 review only. Everything below was verified against the tree.

### The five findings

| # | Verdict |
|---|---|
| **1 — the red gate** | **Fixed, and I reproduced it.** `npx tsx scripts/typecheck-bos-llm.ts` × 3, byte-identical output every run: `177 files in scope, 31 errors, 0 new` / `passed`. Matches the Dev's five runs and my own earlier scope count. The right fix was taken: `entityType: 'system'` at `chat-v2/route.ts:158` (with a comment that says why `'chat'` was never valid and that chat-v4 already writes `'system'`), and the unstable truncated-union key **deleted** rather than re-recorded. Baseline diff against `HEAD` is now **1 insertion, 0 deletions** — only the stable chat-v1 `duration` entry — and 23 → 24 keys. No existing entry altered. |
| **2 — the false comment** | **Fixed, and it does not overclaim in the other direction.** `chat-v4/route.ts:730-764` now states plainly that `applyFrozenWrites` replays frozen steps, that `pages.create` with `page_type: 'landing'` calls `generateWebsite` — a full `website/full_site` call made while chat is off — why that is defensible (website spend, stopped by the website switch, `onAiDisabled: 'fail'`), and where it is pinned. Crucially the closing sentence is scoped: *"Past this point, no LLM call is made"* — true, and not the absolute the old comment asserted. Verified against the code: the `pages.create` handler really is gated on `pageType === 'landing'` (`MutateExecutor.ts:808`). |
| **3 — the two tests** | **Fixed. They pin behaviour, with one caveat.** `modelSettings.off.chat.test.ts:406` drives the real route with a real frozen confirmation and asserts the write ran, the answer is about the write and not the chat-off sentence, and that the planner and analysis were never reached — the confirmable promise, not a restatement of a mock. `:450` is the strong one: the **real** `executeMutate` → real `HANDLERS.pages.create` → the service boundary, asserting one call and `onAiDisabled: 'fail'` with chat off. That is exactly finding 2, pinned. **Caveat (Low, non-blocking):** `:462` ("is stopped by the WEBSITE switch") forces the refusal with `mockGenerateWebsite.mockResolvedValueOnce({ code: 'ai_unavailable' })`, so `rows({ website: false })` is decorative in that case — the website row does not produce the outcome the test name claims. What it *does* pin is new and worth having: that `executeMutate` turns an `ai_unavailable` result into `applied: true` plus a preview carrying the link **and** the translated sentence — the `notice` plumbing, which had no test. The half the name claims is genuinely proved next door (`modelSettings.off.nonchat.test.ts`, "onAiDisabled: 'fail' refuses with ai_unavailable and writes nothing"). Rename, or derive the refusal from the row; do not add coverage. |
| **4 — line endings** | **Fixed.** `file` reports no CRLF terminators; `git diff --stat` is now **201 insertions / 12 deletions**. |
| **5 — the planner** | **Fixed, minimally and in the right place.** `lastModelOnWire` is declared at `:358`, set to `attemptModel` **inside** the attempt callback at `:486` (before the provider call), and read at exactly one site — the terminal `catch` (`:605`), with a comment saying it is the code default after a fallback. `grep` confirms three occurrences and no others; every success, `diagnostics` and mid-loop `fail` path still uses `model = modelUsed`. No behaviour changed on any path that was already correct. |

### D-54, as ruled

**Correctly applied, and it does close the gap I named.** `modelSettingsPolicy.test.ts:215` asserts `ALLOWED_PROVIDERS_LAYER2` equals `['openai']` *and* walks all 22 calls asserting `allowedProviders: ['openai']`. Two reasons this is the right guard rather than a weaker one:

- `BosLlmProvider` is **derived** from `ALLOWED_PROVIDERS_LAYER2` (`modelSettingsPolicy.ts:30-31`) and every policy's `allowedProviders` is `OPENAI_ONLY = ALLOWED_PROVIDERS_LAYER2` (`:129`), so there is no way to widen the allowed set without failing this test first. The ruling's premise — the divergence is inert because the resolver cannot produce a second provider — is now enforced rather than asserted in prose.
- The test's own comment states the thing the boundary suites cannot do: `getProviderArgs` sees `'openai'` either way and cannot distinguish a literal from a resolved value. That was the gap; it is named at the guard, which is where the next author will find it.

### Workplan integrity after the §7.6/§7.7 rebuild

**Nothing was lost.** Checked mechanically against `HEAD`:

- **Deviation IDs:** 40 at `HEAD`, 59 now. **Zero missing**; the 19 added are exactly D-41…D-59, contiguous.
- **Every other ID family** — `FU-`, `RC-`/`RC-W`, `S1-`, `Q-`, `T*.N`, `AC-`, `FR-`, `DEC-`, `D2-`, `D-Q` — **zero missing** in all ten.
- **Headings:** no heading present at `HEAD` is absent now. Top-level `## N.` section list is byte-identical.
- **All 12 deleted lines read individually:** every one is a stale row or task line replaced by updated content in the same place (the three `switchable: no` website rows in §10.1, the Step 3 file/task/test rows, the S1-7 "partial switch" paragraph, the §12 `console.error` row, the RM placeholder). No deviation, no evidence, no decision was dropped.

**One Low defect from the rebuild — `### 7.5 Rollout notes` is now out of order.** At `HEAD` §7.5 was the last subsection of §7; the rebuild re-inserted §7.6 (`:866`) and §7.7 (`:895`) **above** it, leaving the order 7.1, 7.2, 7.3, 7.4, **7.6, 7.7, 7.5** (`:903`). There is also a `---` directly against the `### 7.5` heading with no blank line. Content is intact — this is ordering only — but a numbered section out of sequence is exactly what a reader skims past. Move §7.5 back above §7.6 before commit.

### Gates, re-run by SA on this tree

| Gate | SA result |
|---|---|
| `tsx scripts/typecheck-bos-llm.ts` | **177 files / 31 errors / 0 new — `passed`, three consecutive runs, identical output** (129.4s, 97.1s, 90.6s) |
| Layer 2 jest selection (18 suites) | **18 suites / 415 tests / 23 snapshots pass** (411 → 415: the four new cases) |
| Baseline diff vs `HEAD` | 1 insertion, 0 deletions, 23 → 24 keys, nothing altered |
| Workplan | LF, 201 insertions / 12 deletions, no ID or heading lost |

Not re-run by SA: `npm run build` (~8 min, unchanged surface since the Dev's clean 293/293) and the full 134-suite selection.

### Must-fix

1. **`### 7.5 Rollout notes` ordering** (Low, doc-only) — move it back above §7.6 and restore the blank line after the `---`.

### Optional, non-blocking

2. `modelSettings.off.chat.test.ts:462` — rename the case, or derive the refusal from `rows({ website: false })` instead of `mockResolvedValueOnce`, so the name matches what the assertion proves.

### Is Step 3 safe to deploy?

**Yes.** The gate is green and *provably stable* — three runs, identical, on the key whose instability was the original blocker. The comment that would have misled an operator now says what the code does, and both halves of it are pinned by tests. The planner fix is one variable read at one site. Nothing in the delta changes runtime behaviour except the `entityType` correction, which replaces an out-of-set audit value with the one chat-v4 already writes.

**Standard #1 is met when this lands, on the terms recorded in the Step 3 review:** complete for the 22 catalogued calls, with chat-v2 (`AIDataLayerService`) and chat-v1 (`IntentParser`) still choosing their own model by prior documented decision (F-13 / V-4) — both now stopped by the chat gate, so the kill switch is whole even where model resolution is not.

### Code Approved for QA: **Yes** — with must-fix 1 (doc ordering) applied before commit.
...
## QA Test Report — Step 3

**QA — 2026-09-21** (worktree `neuronforge-llm-layer2-step3`, branch `feature/business-os-llm-layer2-step3`, **uncommitted**, merged up to `origin/main` `cddc1916`; tested against the code, not against the Dev or SA summaries. Nothing was committed, no migration was applied, no production data was read or written, no live provider was called. Every mutation below was applied to real production source, run, and reverted; the tree was proved byte-identical to its starting state by a 34-file SHA-256 manifest taken before and after the session.)

**Test mode:** full · **Strategy:** B (integration, Jest + mocked boundaries) + A (unit) + **mutation testing of the test suite itself** (33 mutations) + one throwaway runtime harness through the real `runAiAction` · **Focus:** api, pipeline, spend control, schema, cost/attribution · **Input source:** prompt keywords + §7.3 / §7.4 + the four blind spots recorded in QA's Step 2 report.

**Counts: 47 checks — 43 PASS, 0 FAIL, 4 BLOCKED (live / post-deploy).** **33 mutations of real production source — 25 caught, 8 escaped** (the eight are one finding: D3-1, the planner's model-reporting paths). 5 defects: **0 High, 1 Medium, 4 Low.** **Ship recommendation: SHIP.**

---

### QA3-1. Gates, run by QA verbatim

| Gate | QA result | §7.4 claim | Verdict |
|---|---|---|---|
| `npm run typecheck:bos-llm` — **run 1** | `177 files in scope, 31 errors, 0 new (85.3s)` · `passed` · exit 0 | 177 / 31 / 0 new | ✅ reproduced |
| `npm run typecheck:bos-llm` — **run 2** | `167 files in scope, 31 errors, 0 new (91.2s)` · `passed` · exit 0 | 177 / 31 / 0 new | ✅ **the gate's verdict is stable** (31 errors, 0 new, both runs); the **scope count** is not — **D3-3** |
| `npm run build` (`NODE_OPTIONS=--max-old-space-size=8192`) | `✓ Compiled successfully` · `✓ Generating static pages (293/293)` · exit 0 | `293/293`, exit 0 | ✅ reproduced exactly |
| `npx jest lib/business-os app/api/business-os app/api/website app/api/intake app/api/onboarding app/api/cron/insight-detect lib/services lib/ai scripts --ci` | `134 passed, 134 total` · `28 skipped, 2264 passed, 2292 total` · `25 passed, 25 total` snapshots · exit 0 | identical | ✅ reproduced exactly |
| `NODE_OPTIONS=--max-old-space-size=8192 npx tsc --noEmit \| grep -c "error TS"` | **2034** (**2030** excluding the 4 in generated `.next/types`) | 2035 / 2031 | ✅ **the conclusion holds, and is one safer than claimed** — 2030 is one *fewer* than Step 2's recorded 2031, so Step 3 adds none; but the recorded number does not reproduce — **D3-4** |
| `tsc` errors inside the 29 files this step touches | **9**, at exactly the lines §7.4 names: `chat/route.ts(275,5)`, `regenerate/route.ts(83,11)`, `WebsiteSectionService.ts(532,33)` and six in `app/business-os/website/page.tsx` (1337, 1339, 1347, 1370, 1456, 6140) | same 9, pre-existing | ✅ **pre-existing**, none new. The six in `page.tsx` are 1,300–4,700 lines away from this step's edits |
| `scripts/typecheck-bos-llm.baseline.json` | **24 keys** — SA finding 1's fix (the unstable truncated-union `chat-v2` key **deleted**, not re-recorded) is present in the tree | 24 | ✅ |

The full `--ci` selection was re-run after the last mutation was reverted: **134 suites / 2,264 tests / 25 snapshots, unchanged.** Neither committed snapshot file moved.

**SA's must-fix 1 (§7.5 ordering) is applied:** §7.1–§7.7 are now in sequence. **SA's optional item 2 is also applied — and QA proved it load-bearing rather than merely renamed** (QA3-3, M24).

---

### QA3-2. Attack on the four closures — QA's own Step 2 mutations, re-run against Step 3

Each of the four escapes from QA's Step 2 report was re-introduced at **each of the three newly wired Step 3 sites** — deliberately not at the sites Dev used in D-55, so that the closures are tested rather than the demonstration. The run was `npx jest lib/business-os/llm lib/business-os/bizql/__tests__ lib/services/__tests__/GeneratedImageService app/api/business-os/chat-v4 app/api/website scripts/__tests__ --ci` (53 suites / 891 tests / 23 snapshots, ~18 s).

| # | Step 2 escape | Mutation (real production source) | Caught? | Failures |
|---|---|---|---|---|
| M1 | **D2-2** — nothing proves the resolved model is used | `Planner.ts` request `model: attemptModel` → a hardcoded model | ✅ **yes** | 2 tests + **1 snapshot** |
| M1a | same | `AnalysisService.ts` request `model` → a hardcoded model | ✅ yes | 2 tests + 1 snapshot |
| M1b | same | `GeneratedImageService.ts` request `model` → `config.model` | ✅ yes | 3 tests (**snapshots green** — the key and override assertions are what bite) |
| M7 | **D2-1** — a site resolving a NEIGHBOUR's key | `Planner.ts` resolves `('chat','analysis')` instead of `('chat','planner')` | ✅ **yes** | 4 tests, **23/23 snapshots still green** — precisely the blind spot, now closed by the key assertion and by nothing else |
| M7a | same | `AnalysisService.ts` resolves `('chat','planner')` | ✅ yes | 4 tests, snapshots green |
| M7b | same | `GeneratedImageService.ts` resolves `('website','full_site')` | ✅ yes | 16 tests + 1 snapshot |
| M8 | **D2-3** — the request not built inside the retry | `Planner.ts` attempt pinned to `model: settings.model` | ✅ **yes** | **exactly 1** — the right discrimination: attempt 1 is still correct, only the retry is wrong |
| M8a | same | `AnalysisService.ts` attempt pinned to `settings.model` | ✅ yes | exactly 1 |
| M8b | same | `GeneratedImageService.ts` attempt pinned to `settings.model` | ✅ yes | exactly 1 |
| M9 | **D2-4 / SA F-1** — provider identity invisible | `Planner.ts` `getProvider(settings.provider)` → `getProvider('anthropic')` | ✅ **yes** | 1 (`T3-S: each site asks for its own settings key, from openai › chat/planner`) |
| M9a | same | `AnalysisService.ts` → `getProvider('anthropic')` | ✅ yes | 1 |

**11 / 11 caught. All four Step 2 blind spots are closed, independently reproduced by QA.** D-55's claim is verified, and QA confirms the detail that matters most: **M7 leaves the boundary snapshot entirely green**, so the settings-key assertion is the only instrument that sees it — which is why it had to be recorded separately from the request.

`images/image_generation` takes no provider argument (it goes through `getOpenAI()`), so M9 does not apply there. D-54's policy guard — `ALLOWED_PROVIDERS_LAYER2` asserted to be OpenAI-only across all 22 calls — is present, and is the right instrument for the 19 literal sites, since the boundary suites cannot tell a literal provider key from a resolved one.

---

### QA3-3. The hunt for a fifth blind spot — 14 further mutations

| # | Mutation | Caught? | Failures |
|---|---|---|---|
| **M10** | **The RC-W4 hazard itself:** the images price resolver keyed on `config.model` while the request carries the resolved model — cost recorded against the wrong model, or at zero | ✅ yes | 3 |
| **M14** | **An off-gate that returns the right shape but still spent:** a provider call inserted immediately before the images `unavailable` return | ✅ yes | 1 |
| **M15** | **A detached call** (SA finding 3's shape, untested in Step 2): a non-awaited `provider.chatCompletion(...)` added to the planner before the real call | ✅ yes | 3 |
| **M16** | **Right key, wrong result:** images resolves `images/image_generation` correctly, then passes `{ ...settings, model: config.model }` into `withModelFallback` | ✅ yes | 4 |
| **M11** | **A gate reading the wrong area:** chat-v4 gates on `isBosLlmAreaEnabled('insights')` | ✅ yes | 4 |
| **M12** | **A gate that loses its `await`** — a Promise is truthy, so the negation is always false and the gate silently never fires | ✅ yes | 4 |
| **M13** | **A gate that answers in the wrong language:** the resolved language replaced by a hardcoded English | ✅ yes | 2 |
| **M17** | **chat-v2's gate removed** — a direct POST reaches `AIDataLayerService` | ✅ yes | 3 |
| **M18** | **chat v1's gate removed** — a direct POST reaches `IntentParser` | ✅ yes | 3 |
| **M19** | `WebsiteAIContentService` `field_regenerate` off-check removed | ✅ yes | 1 |
| **M24** | **Is `rows({ website: false })` load-bearing** in "a confirmed landing page is stopped by the WEBSITE switch" (SA re-check's optional item 2)? Flipped back to `rows({ chat: false })` | ✅ yes — **1 failure** | the stub derives its outcome from the **real** resolver, so the row genuinely drives the refusal. SA's caveat is fixed properly, not renamed away |
| **M25** | The Hebrew website sentence drifts from the page's `LABELS.he.ai_unavailable` | ✅ yes | 2 (the server/client parity check) |
| **M26** | `aiUnavailableLanguage()` always returns English | ✅ yes | 11 |
| **M20 / M21 / M23** (+ the full nine-site sweep) | **The planner's model-REPORTING paths** — see below | ❌ **8 of 9 ESCAPED** | **0** |

#### The fifth blind spot, found — **D3-1**

The four closures are **per call site**: "does this site resolve, and then use, *its own* model?" They say nothing about the **reporting** paths — the places that write down *which model ran*. That is FR-13, and it is what this step's own scope statement promises: *"the planner's repair loop, **plan-cache store** and **every fail/diagnostics path** report the model that actually ran after a fallback retry."*

QA replaced the reported value with a sentinel literal at each of the planner's nine model-reporting sites, one at a time:

| Line | What it reports | Suite result |
|---|---|---|
| `:558` | mid-loop `fail(...)` | 790/790 **green** — escaped |
| `:592` | mid-loop `fail(...)` | 790/790 **green** — escaped |
| `:605` | the terminal `catch`'s `model: lastModelOnWire` — **SA finding 5's entire fix, added this round** | 790/790 **green** — escaped (re-confirmed against the wide 125-suite selection: **2,176/2,176 green**) |
| `:620` | the clarification path's `diagnostics` | 790/790 **green** — escaped |
| `:709` | **the plan-cache `store({ ..., model })`** | 790/790 **green** — escaped (re-confirmed on the 53-suite selection: 891/891) |
| `:718` | the success path's `diagnostics` | **1 failure** — caught |
| `:781` | `diagnostics` | 790/790 **green** — escaped |
| `:792` | `fail(...)` | 790/790 **green** — escaped |
| `:802` | `fail(...)` | 790/790 **green** — escaped |

**1 of 9 covered.** T3-M is a single assertion — `expect(outcome.diagnostics.model).toBe('gpt-4o-mini')` at `callParams.boundary.step3.test.ts:545` — and it exercises only the success path.

**The behaviour is correct.** QA read every one of the nine: `model = modelUsed` at `:550` and `lastModelOnWire` at `:605` are right on every path, exactly as SA verified by reading. This is a **test-coverage** defect of the same family as Step 2's D2-1/D2-2/D2-3, one level down — what shipped is correct, and the suite would not notice if it stopped being. It is **Medium** rather than Low because two of the eight are load-bearing beyond the general case: `:605` is a fix added *in this round, in response to an SA finding*, and is defended by nothing; and `:709` is the plan-cache store, whose wrong value is **persisted** and read back by later turns.

---

### QA3-4. The three chat gates, adversarially

| Question | Evidence | Verdict |
|---|---|---|
| Does the v4 gate sit **before** the budget refusal? | the gate is at `route.ts:767`; the budget refusal at `:793`. Behaviourally, "chat off beats an exhausted budget" asserts the answer is the chat-off sentence **and** that `budget.blocked` is still `true`, so the right sentence wins and the budget state is still reported | ✅ PASS |
| ...and before **every** spending path in the route? | conversation memory `:823`, planner `:860`, plan-cache lookup `:1351` / `:1689`, analysis `:1405` — all below `:767`. QA grepped the entire pre-gate region (`:356` to `:767`) for `plan(`, `analyse`, `chatCompletion`, `getProvider`, `getOpenAI`, `embed`, `PlanCache`, `Embedding`: the **only** hits are the documented `executeMutate` / `applyFrozenWrites` branch | ✅ PASS |
| **Can anything else reach a provider through chat with chat off?** A background job, a cron, the plan-cache embedding, a retry, a detached call, chat-v2 or v1 by direct POST | `getBizQLPlanner()` and `analyse(` have **exactly two** production callers, both in `chat-v4/route.ts` below the gate (`:860`, `:1405`); every other caller is a `scripts/bizql-*` developer harness, which bypasses the gate by design (worth the one line SA asked §10 for). `createAIDataLayerService` has one caller (`chat-v2:138`, below its gate); `parseIntent` from `IntentParser` has one (`chat/route.ts:194`, below its gate) — `WebsiteActionHandler.parseIntent` is a regex parser, not an LLM call. No cron reaches chat; `memory-consolidation` makes no provider call. Direct POST proved by M17/M18; a detached call by M15; the plan-cache embedding is below the gate | ✅ PASS |
| Is a **parked write still confirmable** with chat off? | `modelSettings.off.chat.test.ts:421` drives the real route with a real frozen confirmation: `applyFrozenWrites` called once, the answer is the write's own and **not** the unavailable sentence, planner and analysis never reached. Cancel is covered next door | ✅ PASS |
| **The documented exception**: does confirming a parked "create a landing page" still write the page **with AI** while chat is off? | Traced by hand: the confirm branch (`:686`) reaches `applyFrozenWrites`, then `executeMutate`, then `HANDLERS.pages.create`, which on `pageType === 'landing'` (`MutateExecutor.ts:808`) opens a `website`-area `runAiAction` and calls `generateWebsite(..., { onAiDisabled: 'fail' })`. Pinned at `modelSettings.off.chat.test.ts:466`: with `rows({ chat: false })`, `generateWebsite` is called **once**, with `onAiDisabled: 'fail'` | ✅ PASS — **the behaviour is right, and the comment now says so instead of the opposite** |
| ...and does switching **website** off as well stop it? | `rows({ chat: false, website: false })`: the page is still **created** (`applied: true`, Q-7 reversed), no copy is written, and `preview` carries **both** the `website-preview` link **and** the translated sentence. **M24 proves the row drives this** | ✅ PASS |
| Zod and auth still win over the gate | `[401, 401, 401]` for v4/v2/v1 unauthenticated; `400` for v2/v1 on a malformed body (D-43) | ✅ PASS |
| No ledger row, no audit entry when a gate fires | `aiActionAudit.ts:304` returns on `calls.length === 0`; pinned by the chat-v4 audit suite's "a turn that made no LLM call writes nothing (FR-7)" | ✅ PASS |
| **The kill switch fails OPEN** | `isBosLlmAreaEnabled` (`modelSettings.ts:861-867`) catches everything and returns `true`, and `getSnapshot()` degrades to the code defaults, which are `enabled: true`. A settings-read failure, a dead database or a deleted row therefore **re-enables** chat. This is FR-6 by design and the right posture for a *parameter* resolver — but it is not what an operator reaching for a kill switch during an incident will assume | ⚠️ **by design, undocumented — D3-5** |

---

### QA3-5. The ★ messages, in all three languages

| Surface | Shape | Languages | Owner text | Verdict |
|---|---|---|---|---|
| chat-v4 | HTTP **200**, the route's normal turn shape: `answer.text` is the sentence, `rows: []`, plus `budget` | `it.each(['en','he','es'])`, driven through the real route | n/a | ✅ PASS |
| chat-v2 | HTTP **200**, `data.message` plus `actions: []` | all three; language from `businessProfileRepository`, read **only** on the off path (N-7, verified in the diff) | n/a | ✅ PASS |
| chat v1 | HTTP **200**, `data.response` plus an `unknown` intent | all three (the pending context's language, else the profile's) | n/a | ✅ PASS |
| website `generate-from-profile` | HTTP **200**, `success: false`, `code: 'ai_unavailable'`, plus the English sentence for callers with no label map | the client renders `labels.ai_unavailable` (en/es/he at `page.tsx:303/482/661`), as a **warning**, not an error | **not overwritten** — `WebsiteGenerationService.ts:286` refuses after three *reads* and before the first write, with the comment saying exactly that | ✅ PASS |
| `blocks/[blockId]/regenerate` | HTTP **200**, `success: false`, `code`, and the field name | page label | **not overwritten** — nothing is written | ✅ PASS |
| `enhance-testimonial` | HTTP **200**, `success: false`, `code`, and `originalQuote` | page label | **not overwritten** — the original quote is echoed back | ✅ PASS |
| **chat mutate** (the one server-rendered sentence) | `applied: true`, and `preview` is the link, the url and the notice joined — **the preview link AND the sentence** | server-side `websiteWritingUnavailableMessage()`, en/he/es | the page is created, no copy is written, no provider call | ✅ PASS |
| images | the **existing** `media.generate.unavailable`, already in all three languages (`LanguageContext.tsx:3044/6387/8465`); `LanguageContext` untouched (Q-6) | all three | an existing library picture is still returned — the off check sits **after** the reuse lookup | ✅ PASS |
| server/client parity | `aiUnavailableMessages.test.ts` compares the server sentences against the page's `LABELS`. **M25 proves it bites** (2 failures on a one-language drift); **M26** (language picker pinned to English) fails 11 | — | — | ✅ PASS |

---

### QA3-6. Images — the price and the request can never disagree

| Check | Evidence | Verdict |
|---|---|---|
| The request model and the price key are the **same resolved model** | `GeneratedImageService.ts:397-408`: `imagePriceResolver(config.pricesUsd, model, size, quality)` is built **inside** the `withModelFallback` attempt, from the same `model` the request carries. **M10** (key on `config.model`) and **M8b** (request pinned to `settings.model`) both fail | ✅ PASS |
| ...**after a fallback retry** | `callParams.boundary.step3.test.ts:574` — the override is refused, the models on the wire are `[override, default]`, and the logged price is the **default** model's (0.25), not the override's (0.50). Per-model prices are what make the key observable from outside | ✅ PASS |
| ...**observed at runtime**, not only asserted | QA throwaway harness (written, run, **deleted**): images driven through the **real** `runAiAction`, real `withModelFallback` and real `OpenAIProvider`, with the `images` row configured to `gpt-image-1-mini` and the first call refused 404 `model_not_found`. Models on the wire `["gpt-image-1-mini","gpt-image-1"]`; ledger `[["gpt-image-1-mini",false,0,0,0,G],["gpt-image-1",true,0,0,0.25,G]]` — **0.25 is the default model's price**, so the cost followed the model that ran, and the failed attempt cost nothing | ✅ PASS — **RC-W4 confirmed at runtime, not just in a spy** |
| The price-coverage guardrail is live | observed incidentally while building the harness: with an empty price table the `gpt-image-1-mini` override is **rejected by `checkImageModel`** and the resolver hands back the code default, so no override reaches the wire at all | ✅ PASS |
| Off **after** the reuse check | `:334` sits after `findBySourceRef` and before the daily count — an owner keeps their existing library with images off (N-4 / RC-W4) | ✅ PASS |
| Edge case, recorded not raised | `GeneratedImageService.attribution.test.ts`'s two "unpriced model" cases now set the **resolved** model to an unpriced one through a resolver mock — a state the real resolver cannot produce, because the guardrail above rejects it. The assertions are still the right ones; the fixture simply reaches them by a route production does not have | ℹ️ Low, no action |

---

### QA3-7. Attribution, cost and audit — unchanged, and proved at a **Step 3** site

Step 2's QA closed the "one audit entry on a retry" question at a Step 2 site and recorded that as the honest limit of its evidence. QA closed it at a Step 3 site here, through the real `runAiAction`, the real `withModelFallback` and the real `OpenAIProvider`.

| Check | Evidence | Verdict |
|---|---|---|
| A retry writes **exactly one** audit entry | `area: images`, `actionType: website_image`, `callCount: 2`, `failedCallCount: 1`, `callNames: ["image_generation"]`, `models: ["gpt-image-1-mini","gpt-image-1"]`, `outcome: "succeeded"` — **one** `AuditTrail.log` call | ✅ PASS |
| The **failed** attempt is a 0-token row | `gpt-image-1-mini`, `success: false`, input 0, output 0, cost 0 | ✅ PASS |
| Grouping ids intact | both ledger rows carry the group as `session_id`; the audit entry's `entityId` and `groupId` are the same group | ✅ PASS |
| Tokens cannot be double-counted | `inputTokens 0, outputTokens 0, totalTokens 0, estimatedCostUsd 0.25` — one image's cost, not two | ✅ PASS |
| No Step 3 site opens an audit scope **inside** the attempt | read at all three: chat-v4 opens `runAiAction` at `:344` around the whole turn; images' owners are `media/generate` and `MutateExecutor`'s `pages.create`, both above `generateImage`. Structurally, a retry cannot produce two entries | ✅ PASS |
| No `buildBosCallContext` argument, grouping id, `runAiAction` scope or `callWithTracking` call changed anywhere | full diff read; the three Step 3 boundary snapshots record the whole attribution object and did not move | ✅ PASS |
| `entityType: 'chat'` → `'system'` (SA finding 1) | the only runtime behaviour change in the SA delta. `'chat'` was never a member of `EntityType`, so this stops an out-of-set value reaching `audit_trail.entity_type`; `'system'` is what chat-v4 already writes | ✅ PASS — an improvement, correctly scoped |

---

### QA3-8. Standards and the rest of the step

| Check | Result |
|---|---|
| `console.*` in the 29 touched files | **0** — QA re-grepped every one. D-46 and §12 hold |
| New direct Supabase access | **none** in the diff; the two new reads go through `businessProfileRepository` |
| Zod before the gate on all three chat routes | verified in the source and proved by the 400 case |
| A `'use client'` path importing the server-only resolver | none — `app/business-os/website/page.tsx` only reads `code` and renders its own label; `npm run build` passes |
| **Leg (A) is genuinely empty** | `callParams.snapshot.test.ts:302` asserts the unwired set is empty, with the reasoning written down beside it. All 22 calls resolve from area rows |
| **The website kill switch is whole** | `full_site`, `field_regenerate` and `testimonial_enhance` are `switchable: true`; the PARTIAL SWITCH warning is **deleted** from `scripts/bos-llm-settings.ts` (only a comment at `:306` explains why), and `scripts/__tests__/bos-llm-settings.test.ts:232` asserts its **absence** for both `website` and `insights`. The S1-7 risk is now guarded by `modelSettingsPolicy.test.ts`'s assertion that the non-switchable calls inside switchable areas are exactly `chat/planner` |
| SA re-check must-fix 1 (§7.5 ordering) | ✅ applied |
| SA re-check optional 2 (the decorative `rows({ website: false })`) | ✅ applied **and** proved load-bearing by M24 — stronger than the rename SA offered |

---

### QA3-9. Defects

**0 High. Nothing blocks the deploy.** D3-1 is a hole in the *test suite*, not in the shipped code; QA read every affected line and the behaviour is correct.

| # | Defect | Severity | Where / fix |
|---|---|---|---|
| **D3-1** | **The fifth blind spot: FR-13's model-REPORTING paths are asserted at 1 of 9 sites in the planner.** The four closures are per call site; they say nothing about the places that write down *which model ran*. Replacing the value with a sentinel at `Planner.ts` `:558, :592, :605, :620, :709, :781, :792, :802` leaves the whole suite green (891/891, and 2,176/2,176 on the wide selection). Only `:718` is covered, by T3-M's single `expect(outcome.diagnostics.model)`. **Two of the eight are load-bearing beyond the general case:** `:605` (`model: lastModelOnWire`) is **SA finding 5's entire fix, added this round, and defended by nothing**; `:709` is the **plan-cache `store({ model })`**, named in this step's own scope statement, whose wrong value is *persisted* and read back by later turns. Behaviour verified correct by reading — this is Step 2's D2-1/2/3 family, one level down | **Medium** | `callParams.boundary.step3.test.ts`. ~25 lines: the existing "retries on the default, and the REPAIR call uses the model that ran" case already sets everything up — add (a) the clarification outcome's `diagnostics.model`, (b) a `fail(...)` outcome's `diagnostics.model`, (c) a `PlanCache.store` spy's `model` argument |
| **D3-2** | **The Change History row records the gate numbers SA REJECTED.** The 2026-09-21 "Step 3 implemented" row still says `167 files in scope, 32 errors, 0 new`, `2,260 passed`, `23 → 25 keys` and "ten errors in touched files". The tree says 31 errors, 2,264 passed, **24** baseline keys and nine errors in touched files, and §7.4 records those correctly. A reader who reads only the Change History gets the state SA sent back | Low (doc) | update the row, or append a line pointing at §7.4 |
| **D3-3** | **`typecheck:bos-llm`'s SCOPE count is not stable locally.** QA run 1 = `177 files`, run 2 = `167 files`, both `31 errors / 0 new / passed`. FU-2 explains it (`.next/` presence) and the gate's **verdict** is what matters and is stable — but §7.4 presents three identical `177`-file runs as the proof that "the gate now reads identically on three consecutive runs", and it does not read identically here. Since a scope-dependent instability is exactly what caused SA finding 1, the record should name which number is invariant and which is an artefact | Low (doc / evidence) | one sentence in §7.4 |
| **D3-4** | **The `tsc` baseline does not reproduce.** §7.4 records `2035` / `2031` excluding `.next/types`; QA measures **2034 / 2030** on the identical tree. The conclusion is *safer* than claimed (2030 is below Step 2's recorded 2031, so Step 3 adds none), but this is the same class as Step 2's D2-8 — a recorded gate number a second machine cannot reproduce weakens the record even when the gate passes | Low (doc / evidence) | §7.4; record it as approximately 2030–2031, plus or minus one by environment, rather than an exact figure |
| **D3-5** | **The kill switch fails OPEN, and nothing says so.** `isBosLlmAreaEnabled` returns `true` on any error, and `getSnapshot()` degrades to the code defaults, which are `enabled: true`. So a settings-read failure, a dead database or a deleted row silently **re-enables** every switched-off area within the retry window. This is FR-6 by design and correct for a *parameter* resolver — but "off" is the thing an operator reaches for in an incident, and neither §7.5 nor the Step 4 runbook outline (§8.1) mentions it | Low (doc / operational) | the Step 4 runbook (FR-18), emergency section: switching an area off is **not** durable against the settings read failing — it is a spend control, not a circuit breaker. No code change |

---

### QA3-10. What can only be proven after this deploys — and Step 2's is **still owed**

> ⚠️ **Read this first: Step 2 has not been verified in production.** Step 2 merged as PR #85 on 2026-09-21, but QA2-8's six owner actions and two SQL queries have never been run against the live system, and §10.2's "After Step 2 deploy" row is still empty. **Nothing in this Step 3 report implies otherwise.** The Step 2 checks should be run *before or alongside* the Step 3 ones — if the seeded rows are not being read as expected in production, Step 3 inherits that fault at three more call sites, and the Step 3 checks below would not distinguish the two causes.

| AC / item | Why it cannot be proven here | Status |
|---|---|---|
| AC-13 item 3 for chat and images — the live ledger `model_name` matching §10.1's "after" column | needs the deployed code reading the **live** rows; every test here uses the `SEEDED_ROWS` fixture | **BLOCKED — post-deploy** |
| AC-2 against the live rows | same. T3-S proves "wired code + fixture rows == unwired code"; once an operator edits a row, AC-2 stops applying by design | **BLOCKED — post-deploy** |
| **T3.0** — one image call with an unknown model on a **non-production** OpenAI key, to record the status and code | needs a live key and a live call; no non-production key and no test environment exist. Correctly **not faked** by Dev. Nothing in the step depends on the answer: the classifier was not widened, so an unclassified image refusal is simply not retried — today's behaviour | **BLOCKED — owed, user** |
| AC-13's live off/on cycle for chat (and for leads, from Step 2) | no test environment; it would run on the live system, where real users would see "the assistant is unavailable" for up to ~60 s each way. **Pending the user's decision.** The automated coverage (T3-G, T3-O, QA3-4) stands meanwhile | **BLOCKED — user decision** |

#### The exact post-deploy checks for Step 3

Run **after** the Step 2 checks (QA2-8). Wait **at least 60 s** after any settings write before reading. **Change no row** — the point is that the deploy changed nothing.

| # | Area | The one action to perform | Expected |
|---|---|---|---|
| 1 | chat | ask the Business OS assistant one ordinary question ("how many contacts do I have") in **chat-v4** | ledger `feature = business-os-chat`, `component = planner`, `model_name = gpt-4o-mini`. If the answer was a number worth explaining, a second row with `component = analysis`, same model |
| 2 | chat | ask **the same question again**, so the plan cache answers | **no** new `planner` row (the cache hit is the pass), and the answer is unchanged |
| 3 | images | generate one website image from the website page's image picker | `feature = business-os-images`, `component = image_generation`, `model_name = gpt-image-1`, and **`cost_usd` greater than 0** — a zero-cost row means the price key and the request model disagree, which is exactly what RC-W4 exists to prevent |
| 4 | website | regenerate one field, and enhance one testimonial, on the website page | `component = field_regenerate` and `testimonial_enhance`, both `gpt-4o-mini`; the buttons behave exactly as before |
| 5 | chat v1 / v2 | if either is reachable in your build, send one message to each | they answer normally — the gates are **on**, so nothing should change |

```sql
-- Run once, after all five actions. Read-only.
SELECT feature, component, model_name, count(*) AS calls,
       min(cost_usd) AS min_cost, max(created_at) AS latest
FROM token_usage
WHERE created_at > now() - interval '2 hours'
  AND feature IN ('business-os-chat', 'business-os-images', 'business-os-website')
GROUP BY 1, 2, 3
ORDER BY 1, 2, 3;
```

**Pass:** `planner` and `analysis` show `gpt-4o-mini`; `image_generation` shows `gpt-image-1` with `min_cost` above 0; `field_regenerate` and `testimonial_enhance` show `gpt-4o-mini`; and **no `component` shows two different `model_name` values**.
**Fail → stop and escalate:** a second `model_name` for one `component` means an FR-11 retry fired in production and the key is refusing the configured model — look for the `'Provider rejected the configured model; retrying once with the code default'` line from `BosLlmModelFallback`. A `min_cost` of 0 on `image_generation` means the price key is not the model that ran.
...
**Then fill §10.2's "After Step 3 deploy: chat and images" row — and §10.2's "After Step 2 deploy" row, which is still blank.**

---

### QA3-11. Final status

- [x] **Behaviour: PASS.** Every acceptance criterion testable without a deploy passes. The request at the provider boundary is unchanged for all three Step 3 sites (3 snapshots captured unwired, replayed wired under `--ci`, unmoved — QA re-ran them). All three chat routes stop at the gate with zero provider calls, zero ledger rows and zero audit entries, in three languages, with 401 and 400 still winning. A parked write is still confirmable, and the one path that still spends with chat off is website spend that the website switch stops — both halves pinned by tests, and QA proved the row drives the second half. Images price the model that ran, before and after a fallback, observed at runtime. One audit entry per action on a retry, the failed attempt as a 0-token row, grouping ids intact — proved at a Step 3 site.
- [x] **SHIP.** No High is open. The four Step 2 blind spots are genuinely closed: QA reproduced all four escapes at sites Dev and SA did not use, and all eleven were caught. Fourteen further mutations found **one** more gap, and it is in the suite, not the code.
- [ ] **D3-1 should be closed before Step 4** (~25 lines in `callParams.boundary.step3.test.ts`). Not a blocker, but `:605` is an SA finding's fix that no test defends, and `:709` persists its value.
- [ ] **D3-2, D3-3 and D3-4** are workplan edits of a few minutes. **D3-5** is one sentence owed to the Step 4 runbook.
- [ ] **T3.0 remains owed** (a live unknown-model image call on a non-production key). Correctly not faked.
- [ ] **Post-deploy: run QA2-8's six actions (Step 2 — still owed) and then QA3-10's five actions and two queries**, and fill **both** §10.2 rows. AC-13's live chat off/on cycle remains **pending the user's decision**.
...
|---|---|---|---|---|
| 0 — settings and pricing routes | — | **#61** | 2026-09-19 | The RLS migration `20260920a_lock_system_settings_and_pricing_rls.sql` was applied to production by the user on 2026-09-20, **ahead of** the code deploy (§4.4 records why that was harmless) |
| 1 — policy, resolver, retry, script, seed (inert) | — | **#79** | 2026-09-21 | The seed migration `20261003_seed_bos_llm_area_settings.sql` was **applied to production 2026-09-21** (§10.2 P-4/P-5). The eight rows exist and hold today's values |
| 2 — the six non-chat areas | `32cbeadf` … `4944b11a` | **#85** | 2026-09-21 | 19 call sites in 10 files. ⚠️ **Not yet verified in production:** §10.2's "After Step 2 deploy" row and QA's §QA2-8 checks (six owner actions + two SQL queries) are still owed by the user |
| 3 — chat, the chat gates, images, the ★ messages | *(RM populates)* | *(RM populates)* | *(RM populates)* | Branch `feature/business-os-llm-layer2-step3`, stacked on Step 2 and merged up to `origin/main` `cddc1916` after #85 landed |
| 4 — literal check and docs | *(RM populates)* | *(RM populates)* | *(RM populates)* | — |
...
| 2026-09-21 | **Step 3 implemented — chat, the three chat gates, images and the ★ messages (code complete, uncommitted); all gates green** | Branch `feature/business-os-llm-layer2-step3`, stacked on Step 2 and fast-forwarded to `origin/main` `cddc1916` once PR #85 landed. **The last 3 of 22 calls are wired**, so **leg (A) is now EMPTY** and `callParams.snapshot.test.ts` asserts the empty set: `chat/planner` (the `SystemConfigService` read and the `temperature: 0` literal both gone; the 3-attempt repair loop's `model` is a `let` reassigned from `modelUsed`, so repairs and every `fail`/`diagnostics` path report the model that RAN — RC-W6, FR-13), `chat/analysis` (both `bizchat_analysis_*` keys gone; off -> the planner's own sentence), and `images/image_generation` (the model now comes from the `images` area row and is used in **both** the request and the price resolver, which is built **inside** the attempt — RC-W4 — so a retry can never be priced as the refused model or at $0). **The chat area switch gates all three chat routes**: chat-v4 immediately before the budget refusal (so a parked write can still be confirmed or cancelled, and chat-off beats 'out of allowance'), chat-v2 and chat v1 after auth AND validation (D-43). No planner, analysis, embedding, data-layer or intent-parser call runs; no ledger row, no audit entry. **The ★ messages** ship in `lib/business-os/llm/aiUnavailableMessages.ts` (chat and website copy, en/he/es) plus `LABELS.ai_unavailable` on the website page; **images reuse the existing `media.generate.unavailable`**, which already had all three languages (Q-6), and `LanguageContext.tsx` was not touched. **The website kill switch is now whole (S1-7 closed):** `full_site`, `field_regenerate` and `testimonial_enhance` are `switchable: true`; the onboarding build still finishes on starter copy (`onAiDisabled: 'fallback'`) while generate-from-profile and the chat mutate path refuse (`'fail'`) with nothing written — the mutate path still CREATES the page and replies with the preview link plus the sentence, through a new generic `notice` field (D-44). The change script's PARTIAL SWITCH warning was **deleted**, not left to self-clear, because it would then have started naming `chat/planner` (D-45); a policy test now asserts the non-switchable set is exactly `['chat/planner']`. **T3-S uses Step 2's method (D-48):** 3 snapshots captured against the UNWIRED sources with the wiring reverted to `HEAD`, then replayed with `--ci` against the wired ones — **3 passed, unchanged**. **The four T2-S blind spots are closed and MUTATION-PROVED (D-55)** for all 19 Step 2 sites and all 3 Step 3 sites: a provider switch on the planner fails 1 test (SA F-1, previously green); QA's own M7 (`hero_content` resolving `'faq_content'`) fails 3 — while the boundary snapshot still passes it, which is exactly the blind spot; QA's M1 (re-hardcoding a model) fails 2; QA's M8 (pinning `settings.model` inside the attempt) fails exactly 1, leaving D2-2 green, which is the right discrimination. All reverted, suites green. SA-2 is closed only at `generateWebsite`'s public entry (FU-5). SA F-9 (D-52) and F-11 (D-53) both addressed. **Gates (§7.4) — superseded by the SA and QA rounds below; the numbers on this row were corrected per QA D3-2 and now read as the finished tree does:** `typecheck-bos-llm: 31 errors, 0 new`, **passed** — RC-W9's prediction held, chat v1 and v2 JOINED the scope as callers and surfaced 2 pre-existing errors of their own, so the first run FAILED; one was fixed at the root (`chat-v2`'s `entityType`) and one baselined, leaving **24** baseline keys. `npm run build` ✅ `Compiled successfully`, `293/293`, exit 0. Jest **134 suites / 2,264 passed / 25 snapshots** (`--ci`). `tsc` **2,030** excluding generated `.next/types` — Step 2's 2,031 with one pre-existing error now fixed, so this step adds none. **First-ever test run found 8 failures, all in the harness, none in production code** (D-49): a `supabaseServer` mock with no `storage`, an analysis fixture with no `agg`, two store mocks that dropped the named exports the route also imports, and a suite that drove `analyse` through its own throwing mock. Deviations D-41 … D-55 and follow-ups FU-4/FU-5 in §7.6/§7.7. **Still owed: T3.0** (a live unknown-model image call) and Step 2's own post-deploy verification (§10.2) |
