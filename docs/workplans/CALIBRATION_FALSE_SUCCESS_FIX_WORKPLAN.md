...
- [x] **P3** — schema-driven scatter item-ref repair with **high-confidence auto-apply**. New `lib/pilot/shadow/ScatterItemFieldValidator.ts` resolves the iterated collection's element fields from the **plugin definition** (source of truth — beats the mutated stored schema, the WP-56 trap; falls back to the stored schema for non-plugin sources) and flags `{{item.badField}}` refs, proposing the correct field (identifier-aware → Levenshtein). Wired into `batch/route.ts` **before the dry-run**: confidence ≥ 0.9 (identifier-matched, plugin-def-verified) → `applyFix()` rewrites the exact token in the DSL + persists `pilot_steps` + counts as an auto-fix, so the dry-run executes the *fixed* workflow and calibration converges to success. Lower-confidence / unapplyable matches are surfaced as critical before/after proposals (shaped as a `CollectedIssue` with `id`/`category`/`affectedSteps` so `IssueGrouper` keeps them distinct — a live test caught that without these they collapsed into `unique:undefined` and were merged away). `userFacing.ts` gets a dedicated `scatter_item_field` translator. 11 unit tests (incl. the mutated-schema/plugin-def case and `applyFix` round-trip). **Deferred:** a committed regression scenario.
- [x] **P4** — filed **WP-56** in WEAK_POINTS (summary row + Change History) linked to the `contracts-googledocs-v2ui-pipeline-a` scenario; OPEN_ITEMS pointer added. Root-cause **prompt steering**: FIELD FIDELITY (WP-56) rule in § 6.9 LOOP of `intent-system-prompt-v2.ts` — `item_ref.<field>` names must come from the iterated collection's element schema, never reused from a container/other step (plugin-agnostic). Committed `4724e67`. Still open in WP-56: deterministic binder/compiler reconciliation for iteration-variable refs, and pinning the `id`→`folder_id` data_schema mutation.
...
| Date | Change | Details |
|------|--------|---------|
| 2026-06-08 | P3 auto-apply + live-test fixes | First live test on `8c7caa01`: detector fired correctly but the proposal was (a) swallowed by `IssueGrouper` (`unique:undefined` merge — missing `category`/`id`/`affectedSteps`) and (b) only surfaced, not applied, so the agent stayed broken. Fixes: shaped the issue as a `CollectedIssue` so grouping keeps it distinct; moved detection BEFORE the dry-run and **auto-apply** confidence-≥0.9 fixes (rewrite token + persist `pilot_steps` + count as auto-fix) so the dry-run runs the fixed workflow; low-confidence stays surfaced. Added `applyFix()` + 2 tests (11 total). |
| 2026-06-08 | P3 + P4 implemented | **P4** (committed `4724e67`): WP-56 filed + Phase 1 FIELD FIDELITY prompt steering in § 6.9 LOOP. **P3**: new `ScatterItemFieldValidator` (schema-driven, plugin-agnostic) detects `{{item.badField}}` scatter/loop refs and surfaces an identifier-aware before/after proposal; wired into `batch/route.ts` as a critical, non-blocking, never-auto-applied issue; dedicated `userFacing` translator; 8 unit tests. Deferred: one-click apply + regression scenario. |
