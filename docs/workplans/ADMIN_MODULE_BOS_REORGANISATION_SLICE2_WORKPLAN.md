...
| 2026-09-26 | CI fix + docs (PR #112) | CI `Business OS entitlements invariants` failed: `enforcementPoints.test.ts:288` found the new summary route importing `isBusinessOsTenant` without a register entry. Added it to `KNOWN_NON_GATE_IMPORTERS` (read-only, refuses no capability). Local runs had not included `lib/business-os/entitlements`, which is the gap; future slices touching entitlements imports run that folder too. CLAUDE.md admin counts refreshed (80 / 74 / 6); requirement change history records U-1 to U-5 |
