...
- 🚧 **1.2.b PARTIALLY RESOLVED → spun off** (task_dda5f400). The public website routes cannot be fully cleanly remediated: `forms/intake` writes **phantom** `crm_contacts.name`/`status` + `crm_activities.type` columns that don't exist in either migration (schema is `first_name`/`last_name`/`stage`, `activity_type`) — **this remains open** and needs a **BA/SA data-model decision**. **UPDATE 2026-08-10 (M6, Scheduling pilot):** the **booking-route T2 double-log** portion is now **CLOSED** — the [Scheduling internal-plugin pilot](/docs/workplans/BUSINESS_OS_SCHEDULING_INTERNAL_PLUGIN_WORKPLAN.md) removed the explicit `crm_activities` inserts in `booking/{create,confirm,finalize}` (T2 owns them); the booking routes were confirmed to write **correct** columns (no `name`/`status` drift). Only the `forms/intake` phantom columns (+ duplicate `20260721`/`20260722` migrations) remain open under `task_dda5f400`.
