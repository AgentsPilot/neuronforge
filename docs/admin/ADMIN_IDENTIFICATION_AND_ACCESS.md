...
Re-derived **2026-09-26** (Admin Archiving slice 1): **81 handlers across 52
...
> register now lists all 80. Admin Archiving slice 1 (2026-09-26) added
> `archiving#GET`, gated from birth: **81 / 75 + 6 / 52 files**.
...
| **Every one of the 81 `/api/admin/*` handlers requires an admin.** 75 via `requireAdmin`, 6 via their own equivalent check. **Zero open.** | ✅ **True as of 2026-09-26** (slices 2 + 3 of the unification; re-counted in admin reorganisation slice 2) |
...
| ~~"The guard proves every gate actually runs"~~ | R1 proves a gate is **present**, not that it runs **first** or runs at all. 75 handlers are gated (2026-09-26): the 65 counted on 2026-09-21 were verified by hand and by the oracle, the 10 added since only by their own route tests — measured, not enforced. See [Known gaps in the guard itself](#known-gaps-in-the-guard-itself) and Open Item 8. |
...
**75 `requireAdmin` · 6 correct-but-inline · 0 open.** (Rows 73–80 were added 2026-09-25 and row 81 on 2026-09-26; the numbering of rows 1–72 is kept so older references still resolve.)
...
| 81 | `archiving` | `GET` | ✅ gated | `requireAdmin` first statement, new in Admin Archiving slice 1 (read-only overview; position pinned by its route test I-10) |
