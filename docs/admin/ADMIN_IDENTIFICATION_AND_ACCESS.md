...
Re-derived **2026-09-26** (admin reorganisation slice 4): **81 handlers across 52
route files = 75 `requireAdmin` + 6 inline + 0 open.** The split is measured, not
...
>
> **80 → 81 (slice 4, 2026-09-26):** `health-summary#GET` (the Health landing),
> gated from birth; no cap moved.
...
| **Every one of the 81 `/api/admin/*` handlers requires an admin.** 75 via `requireAdmin`, 6 via their own equivalent check. **Zero open.** | ✅ **True as of 2026-09-26** (slices 2 + 3 of the unification; re-counted in admin reorganisation slice 4) |
| **All 24 `/admin` pages are protected on the server**, by inheritance from `app/admin/layout.tsx` | ✅ **True as of 2026-09-21** (slice 5), for the 21 pages then; the 3 added since (`business-os-tiers`, and in admin reorganisation slice 4 the moved `platform-dashboard`, alongside the rewritten `/admin` Health page) inherited it without an edit. Re-counted 2026-09-26: 24 `page.tsx` files under `app/admin/` |
...
| ~~"The guard proves every gate actually runs"~~ | R1 proves a gate is **present**, not that it runs **first** or runs at all. 74 handlers are gated (2026-09-25): the 65 counted on 2026-09-21 were verified by hand and by the oracle, the 9 added since only by their own route tests — measured, not enforced. See [Known gaps in the guard itself](#known-gaps-in-the-guard-itself) and Open Item 8. |
...
### Every admin handler and its state (81)
...
**74 `requireAdmin` · 6 correct-but-inline · 0 open.** (Rows 73–80 were added 2026-09-25; the numbering of rows 1–72 is kept so older references still resolve.)
...
| 8 | **Guard precedence gap.** | ⬜ Todo | R1 proves a gate is **present**, not that it runs **first** or runs at all. Applies to all 74 gated handlers (80 in total, 2026-09-25). Closing it needs the oracle's instrumentation extended to cover the **body parse** — `mockTablesTouched` records DB/RPC/auth-API calls, not `request.json()` — otherwise a precedence check is only half a check. |
