...
Re-derived **2026-09-25** (admin reorganisation slice 2): **80 handlers across 51
route files = 74 `requireAdmin` + 6 inline + 0 open.** The split is measured, not
...
| **Every one of the 80 `/api/admin/*` handlers requires an admin.** 74 via `requireAdmin`, 6 via their own equivalent check. **Zero open.** | ✅ **True as of 2026-09-25** (slices 2 + 3 of the unification; re-counted in admin reorganisation slice 2) |
...
### Every admin handler and its state (80)
