...
**Created by:** BA
**Date:** 2026-09-18
**Status:** Implemented in code, pending review, QA and deploy. Steps 0, 1 (the owner-policy migration, applied by the user) and 2 are merged; steps 3–5 (every area wired) are code-complete as of 2026-09-19. See the [workplan](/docs/workplans/BUSINESS_OS_LLM_AUDIT_TRAIL_WORKPLAN.md). SA approved; RC-1 to RC-12 applied; user decisions D-1 to D-6.
...
7. **FR-7 — No LLM call, no entry.** An action that made no LLM call writes no AI audit entry: a chat answer served from an **exact** plan-cache hit (no call at all), an image served from the reuse cache, a request refused by the daily image cap, a briefing served from its cache. (The chat cache-hit ledger row is not an LLM call — Layer 1 FR-15.) A **semantic** plan-cache hit still makes one billed embedding call, so it **does** write a one-call entry (SA ruling, step 3–5 code review §21, deviation c).
...
- [ ] **AC-7** (FR-7) — `log()` is not called for: a chat turn served from an exact plan-cache hit, an image served from the reuse cache, an image refused by the daily cap, a cached briefing, and an action that failed before any LLM call. A semantic plan-cache hit (one embedding call) writes exactly one one-call entry.
...
| **OI-D** | **Recommendation: change it later for AI entries.** Write AI audit entries **immediately** (not through the batch queue) and **await confirmation with a short time limit — about 1–2 seconds** — so an owner's action is never held up by more than that, and a slow or failed write is logged rather than silently dropped. Background jobs are unaffected (they already run for seconds per business). This closes KI-B for AI entries without touching any other product | **Parked by the user (2026-09-19).** Production measurement (workplan §23): 0 of 6 entries lost, but every non-flushed entry was delayed 7 s – 3.5 min until a later request woke the server; logout (FR-29) saved in < 1.1 s. Loss risk is highest for overnight jobs and lead replies; billing is unaffected (`token_usage` writes immediately). QA recommends doing OI-D **before launch**; the user chose to park it for now. Revisit together with OI-A |
...
| **OI-F** | **The Stripe code queries the database directly instead of going through repositories** (CLAUDE.md mandatory rule 1). Direct `.from(` / `.rpc(` calls on main 7646760a: `app/api/stripe/webhook/route.ts` (72), `sync-subscription` (8), `update-subscription` (3), `cancel-subscription` (2), `invoices` (2), `reactivate-subscription` (2), `create-checkout` (1), `create-portal` (1), plus `lib/stripe/StripeService.ts` (6). Step 0 only changed how four of these routes write audit entries and deliberately left their data access alone (payment-flow scope). Move the reads and writes into repositories (e.g. subscription / billing repositories, with `user_id` scoping and the `tenant-isolation-guard` review for the service-role paths), webhook first by volume | **Open** — separate fix, raised by the user 2026-09-18; not part of Layer 3 |
| **KI-F** | **KI-B observed in production (2026-09-19 post-deploy check).** The user performed four actions at ~10:40 UTC on production: `/v2/settings` Profile save, `/v2/settings` Notifications save, logout, and the `/v2/settings` Billing "Update Payment" (Stripe portal). Only `SETTINGS_PROFILE_UPDATED` reached `audit_trail`; `SETTINGS_NOTIFICATIONS_UPDATED`, `USER_LOGOUT` and `CUSTOMER_PORTAL_ACCESSED` did not. Each missing one was the last request on its serverless instance (the user left, or was redirected to Stripe), so the queued entry was never flushed. The Business OS settings change (`BUSINESS_BRANDING_UPDATED`, 10:54, written in-process) arrived. **The `/v2/settings` misses are recorded as a known bug and not fixed now**: `/v2` is not part of the Business OS app (user decision 2026-09-19). The logout miss is handled by FR-29 | **Known** — `/v2` part left as is; logout fixed by FR-29 |
| **FR-29** (added 2026-09-19) | **Flush the audit queue on logout.** When a client audit write carries `USER_LOGOUT`, the write route (`lib/audit/clientAuditWrite.ts`) enqueues the entry as today and then **awaits `AuditTrail.flush()`** (the existing public method; `AuditTrailService` itself is unchanged, consistent with D-4) with a time limit of about 2 s, before responding. The browser already awaits this response before signing out (`app/business-os/settings/page.tsx` logout path), so the logout entry is saved, and anything else queued on that instance is saved with it. A flush error or timeout is logged and never blocks the logout. Test: a logout write calls flush once and still returns success if flush fails or times out; other events don't call flush | **Code-complete 2026-09-19** (workplan §20.2): `lib/audit/clientAuditWrite.ts`, with 5 tests; a separate commit in the Layer 3 step 3–5 PR. User decision 2026-09-19 |
...
| 2026-09-18 | Open item OI-F added | The Stripe routes and `StripeService` query the database directly (97 calls across 8 routes and the service) instead of through repositories (CLAUDE.md rule 1). Raised by the user; recorded as a separate fix outside Layer 3. No FR/AC change |
| 2026-09-19 | Steps 3–5 code-complete | Every area in the per-area table writes one AI audit entry per action through `runAiAction`: chat (and the nested chat website operation), insights per business, briefing (with a required trigger), website, intake, the onboarding build (one entry, both areas), onboarding turns, leads and images. Docs updated (FR-20); the `bos-llm-call-standards` skill's Standard 6 finalised. No FR or AC change |
| 2026-09-19 | KI-F + FR-29 (post-deploy finding) | Post-deploy check showed KI-B in production: of four user actions only the profile save reached audit_trail. /v2/settings misses recorded as a known bug (not Business OS; user decision). New FR-29: flush the audit queue on USER_LOGOUT in the client write route, awaited with a ~2 s limit, AuditTrailService unchanged (user decision 2026-09-19) |
| 2026-09-19 | FR-7 / AC-7 reworded (SA §21 CR-2) | Only an exact plan-cache hit makes no LLM call and writes no entry; a semantic plan-cache hit makes one billed embedding call and writes a one-call entry. No scope change |
| 2026-09-19 | OI-D parked (user decision) | Production KI-B measurement (workplan §23): nothing lost, AI entries delayed up to 3.5 min. QA recommends OI-D before launch; the user parked it for now, to revisit with OI-A |
