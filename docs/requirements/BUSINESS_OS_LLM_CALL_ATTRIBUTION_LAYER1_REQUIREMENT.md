...
| **2** | Model, provider, temperature and on/off per call, stored as one JSON configuration per area, keyed by the Layer 1 call names; no hardcoded models — including the onboarding conversation's `'gpt-4o'` literals (Layer 1.5 KI-C / F-9). **Delivered 2026-09-21** in four steps: gated settings routes, the resolver + policy + change script, the six non-chat areas, then chat, the chat entry gates and images. All 22 catalogued calls resolve their model, temperature and on/off switch from eight `system_settings_config` rows within ~60 s, with a per-area kill switch; `npm run check:bos-llm-literals` keeps it that way. KI-C closed. Requirement: [BUSINESS_OS_LLM_MODEL_SETTINGS_LAYER2_REQUIREMENT.md](/docs/requirements/BUSINESS_OS_LLM_MODEL_SETTINGS_LAYER2_REQUIREMENT.md); operator runbook: [BUSINESS_OS_LLM_MODEL_SETTINGS_RUNBOOK.md](/docs/runbooks/BUSINESS_OS_LLM_MODEL_SETTINGS_RUNBOOK.md). **Not included:** the admin screen (a later layer), and chat-v2 / chat-v1 still choose their own model (F-13 / V-4) — both stopped by the chat switch | Layer 1 call names |
...
| 2026-09-21 | Roadmap: Layer 2 delivered | The Layer 2 row now records what was built, in four steps: the settings and pricing routes gated (PR #61), the policy / schema / resolver / retry / change script (PR #79), the six non-chat areas wired (PR #85), then chat, the three chat entry gates and images (PR #87), with Step 4 (the `check:bos-llm-literals` CI gate, the skill and the operator runbook) in review. All 22 catalogued calls take provider, model, temperature and on/off from eight `system_settings_config` area rows within ~60 s; Layer 1.5 **KI-C is closed**. Links the [Layer 2 requirement](/docs/requirements/BUSINESS_OS_LLM_MODEL_SETTINGS_LAYER2_REQUIREMENT.md) and the new [operator runbook](/docs/runbooks/BUSINESS_OS_LLM_MODEL_SETTINGS_RUNBOOK.md). Not included: the admin screen, and chat-v2 / chat-v1's own model choice (F-13 / V-4), which the chat switch stops rather than resolves |
