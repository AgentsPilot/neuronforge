...
| AI & chat | **Access to the chat assistant (`chat.access`)** — the surface itself. Added 2026-09-24 (FR-46): the per-operation rows below cannot express "this plan has no chat", because every chat operation resolves to the capability of the domain it touches. | bool | off | on | on |
...
46. **FR-46** [Slice 2] (added 2026-09-24, on SA review of the first tier configuration) **Access to the chat surface is its own capability, `chat.access`, and the chat entry point is gated on it once per turn.**

    The eight per-operation `chat.*` capabilities cannot express "this plan has no chat". Every chat operation is mapped to the capability of the **domain it touches** (FR-6), so an account that is sold without chat but does have CRM resolves a chat read *or write* of its contacts to `crm.core`, which it has — and chat answers, and acts. Withholding the eight groups leaves the assistant open; the plan says one thing and the product does another. Five parts:

    a. **One capability.** `chat.access` (boolean, `available`, owner audience) is added to the catalog (§5.3). It is **off for Essentials and on for Autopilot**, and it is the whole commercial difference between them, stated once, in config. Adding it to the catalog is done now, with the tier configuration; **nothing reads it until Slice 2.**

    b. **Gated once per turn, at the entry point.** The chat route checks `chat.access` **before** any plan is built and before any per-capability check. The refusal must not depend on which entity the planner happened to choose — an account either may use chat or may not, and the answer must be the same for "show me my contacts" and "email every client".

    c. **Refused as `not_entitled`, and chat EXPLAINS.** The decision is `not_entitled` with `lowestTier` = the lowest tier that includes `chat.access` (Autopilot). The owner sees a **normal assistant message**, not an error state, not a toast, not a stack trace, and the conversation stays usable. The wording (English; Hebrew and Spanish follow the plan-name translations):

       > *"The AI assistant is part of **Autopilot**. Your plan is **Essentials**, so I can't do this for you here — but nothing is locked: you can do all of it yourself from the Contacts, Invoices, Calendar and Website screens. Upgrading to Autopilot turns the assistant back on."*

       Both plan names are read from the tier `presentation` config (never hard-coded, FR-12), so renaming a plan changes this sentence with it. The message says what the owner **can** still do, because everything in Essentials remains fully usable without chat; it must not read as a dead end.

    d. **The per-operation map stays.** `chat.marketing`, `chat.invoice_control`, `chat.email`, `chat.search`, `chat.scheduling`, `chat.quotes`, `chat.reporting` and `chat.bulk` still decide what an account **that has chat** may do inside it, and they remain the basis of the shadow report and of metering. `chat.access` is a gate in front of them, not a replacement for them.

    e. **The surface decision is recorded in shadow.** Every turn records its `chat.access` decision, so the report can answer *"how many accounts would lose chat entirely?"* — the number that justifies the price difference. Without it the report can only count operations, and an account that used chat once a day and an account that used it hourly would look the same size of loss. The `asTier` replay counts **distinct accounts** whose `chat.access` would be refused, alongside the existing per-capability counts.
...
- [ ] **AC-38** (FR-46, Slice 2) For an account on a plan without `chat.access`: opening chat and asking anything — a read, a write, a question about a domain the plan *does* include — produces the assistant message in FR-46c naming Autopilot and Essentials from config, and **no** plan is executed. The same account performs the same work successfully through the normal screens. For an account on a plan with `chat.access`, chat behaves exactly as before, and the per-capability refusals inside chat are unchanged. In shadow mode, one `chat.access` decision is recorded per turn, and the report reports the number of distinct accounts that would lose chat entirely.
...
| 2026-09-24 | **FR-46 added: `chat.access`, the chat surface as a capability** | Raised by Dev while configuring the first real tier matrix and confirmed and widened by SA: withholding the eight per-operation `chat.*` capabilities does **not** close the chat surface, for reads **or writes**, because every chat operation maps to the capability of the domain it touches. Adds `chat.access` to the §5.3 catalog (38 capabilities), FR-46 with its five parts (one capability, gated once per turn at the entry point, refused as `not_entitled` with the customer-visible wording specified rather than improvised, the per-operation map retained, and the surface decision recorded in shadow), and AC-38. The capability ships in the catalog and the tier matrix now; **the gate is Slice 2**. |
