...
**Created by:** BA
**Date:** 2026-09-19
**Status:** SA reviewed 2026-09-19 — APPROVED WITH CONDITIONS (see §21). Business decisions B-2 to B-15 were decided by the user on 2026-09-19 (B-3 revised the same day). T-1 to T-12 decided by SA (§21). B-1 is still open (waiting on Eyal). **Two items are PENDING USER CONFIRMATION: P-1 (default trial contents) and P-2 (launch-migration timing and open-ended champions). See §19.** **Scope of this delivery (B-13): entitlement infrastructure only.** No commercial tier assignments are implemented.
...
| **Champion** | A cohort that receives **every** catalog capability at its highest variant and quantity, including `beta` capabilities (B-14). Existing accounts become champions (B-3 revised). Under P-2 (PENDING USER CONFIRMATION) they become **open-ended** champions with no expiry, as of the enforcement switch-on date. |
...
| **Enforcement switch-on date** | The moment the Slice 2 enforcement flag is turned on in production. Under P-2 (PENDING USER CONFIRMATION), the launch migration runs at this moment. |
...
These are fixed inputs. They are listed so SA can trace each FR back to a decision. The follow-up decisions made on 2026-09-19 are recorded in §19 (B-2 to B-15). Because of B-13, this delivery is infrastructure-only: D-1's tiers are added later as configuration, and D-2's trial contents are cohort configuration (§6.4). B-14 fixes what champions get (D-4). Items marked **PENDING USER CONFIRMATION** are listed in §19 (P-1, P-2).
...
| D-2 | **Trial**: every new signup gets a trial of 14 days (**length configurable**) with a small AI allowance. It ends at 14 days or when the trial allowance runs out, whichever comes first. *(Per B-13, the trial is **no longer defined as "Growth-level"**. Its contents are **trial cohort config**, and the **default is all capabilities: PENDING USER CONFIRMATION (P-1)**. The trial clock starts at the first onboarding message by default, §6.4.)* |
...
- As **an admin**, I want to mark an account as a champion, extend or set its expiry, grant it AI credits or switch on a beta capability, without a UI and with an audit trail, so that design partners are looked after and every change can be traced.
- As **a new signup**, I want a trial with the configured contents (all capabilities by default), starting from my first onboarding message, so that I can judge the product before paying.
...
- As **an existing user when enforcement is switched on**, I want to become a champion with every capability and no expiry, so that I lose nothing when entitlements are introduced.
...
```json
{
  ...
  "cohorts": {
    ...
    "champion": { "grant_all": "highest", "include_lifecycle": ["beta"], "ai.actions": { "per_month": 3000 }, "grace_days": 30 }
  }
}
```

(All numbers are placeholders. The trial `grant_all` default is **PENDING USER CONFIRMATION (P-1)**. The trial `total` must be sized per FR-41, since it includes setup AI.)
...
| Champion expiry, existing accounts migrated at launch | **Open-ended (no expiry). PENDING USER CONFIRMATION (P-2)** | An admin can set an expiry on any champion later (§12). |
| Champion expiry, champions set later by an admin | Per account. Cohort default about 6 months (D-4) | Admin can set or extend it (§12). |
...
| **Trial contents** | **All capabilities at highest variant/quantity (`grant_all`). PENDING USER CONFIRMATION (P-1)** | Replaces D-2's "Growth-level". Whether the default includes `beta` capabilities is part of P-1 (BA suggestion: exclude `beta`, which stays champion-only). |
...
| 3 | **Cohort adjustments** | Cohort config (`trial`, `champion`). **Champion grants every catalog capability at its highest variant/quantity, including `beta` (B-14).** Trial grants its configured contents (default all capabilities, P-1 PENDING). | Champion: every capability at highest level, `ai.actions` = champion allowance |
...
- An override or grandfathered entitlement that has expired is ignored. It is never deleted silently.
...
|---|---|---|---|---|
| `trial` | New signup. The clock starts at the first onboarding message by default (§6.4). | Trial cohort contents: all capabilities by default (**P-1, PENDING USER CONFIRMATION**). Setup AI counts against the trial allowance, with a warning before a setup regeneration would use it up (B-12). | Live | `active` (pays); `grace` (trial expires **or** trial AI allowance used up) |
| `champion` | Admin sets cohort. Also, at the launch migration, every existing account (B-3 revised). Under **P-2 (PENDING USER CONFIRMATION)**, the migration runs **at the enforcement switch-on date** and the resulting champions are **open-ended**. | Every capability at highest level, including beta (B-14) | Live | `active` (converts, founder coupon); `grace` (only if an expiry is set and reached) |
...
- Expiry dates, grace lengths, trial length and trial clock start are **config or per-account values, never constants in code**. A champion may have **no expiry** (open-ended).
- An admin can move an account between states by setting or extending expiry or changing cohort (§12), and every such move is audited.
...
| Operation | Notes |
|---|---|
| Set / clear cohort (`champion`, `trial`) | Admin only (D-5). The launch migration puts every existing account on `champion` at enforcement switch-on (B-3 revised; P-2 PENDING). After that, champion is set by an admin. |
| Set / extend / clear expiry and grace for one account | Moves lifecycle state (§9). This includes setting an expiry on an open-ended champion. |
...
11. **FR-11** [now] Account lifecycle state follows §9 and is recorded per account. Trial length, trial clock start, grace lengths and champion expiry (including "none") are config or per-account values.
...
14. **FR-14** [now: build and dry-run; executed at enforcement switch-on] New signups enter `trial` automatically, with the clock starting at the configured start event (default: first onboarding message) and the trial contents from cohort config (default all capabilities, **P-1 PENDING USER CONFIRMATION**). A **launch migration** places **every Business OS account existing at the enforcement switch-on date** on the `champion` cohort as an **open-ended** champion with no expiry (B-3 revised; **timing and open-endedness PENDING USER CONFIRMATION, P-2**). No existing account gets a fresh trial. The migration is idempotent and built in this delivery, can be dry-run in shadow mode, and is executed only when enforcement is switched on.
...
22. **FR-22** [now] This delivery ships a **shadow mode**: the resolver runs and logs what *would* be gated per surface and account, while blocking nothing. It is controlled by a flag. It also reports the effect of the launch migration as a dry run (which accounts would become champions).
...
45. **FR-45** [now] (B-15) The policy values in §6.4 are configuration with documented defaults, not code: tier assignments including what lower tiers get for search via chat, champion level, champion expiry for migrated accounts (default open-ended, P-2 PENDING), trial clock start (default: first onboarding message), trial contents (default all capabilities, P-1 PENDING), and intake requests while paused (default: stop). Changing a default is a config change through a normal release.
...
|---|---|
| **Changeability** | A tier-matrix change is a one-entry config change shipped in a normal release (B-2). There is no live editing for now, but the design must allow moving to live edits later (FR-34). Adding the commercial tiers later is a configuration change only (B-13, FR-3). |
...
| **Testability** | Invariant tests (FR-8). Resolver unit tests covering every layer (including grandfathering, champion "all at highest" and trial `grant_all`) and lifecycle state, using a **test-fixture tier matrix** since no commercial tiers ship (B-13). Integration tests per enforcement surface type (later delivery): happy path, not entitled, limit reached, read-only, paused (keep-working vs stopped surfaces). A Playwright journey for trial → grace → paid once billing exists. |
...
Each slice ships on its own and leaves the product working. **This delivery = Slice 1 (B-13).** The launch migration is built in Slice 1 and executed at the Slice 2 enforcement switch-on (P-2 PENDING). Slices 2–4 are specified for later deliveries.

| Slice | Content | User-visible effect | Exit criteria |
|---|---|---|---|
| **1. Infrastructure: catalog, config mechanism, resolver, account state, cohorts, overrides, admin ops, shadow report** *(this delivery)* | Catalog (§5), tier matrix mechanism with **no fixed or commercial tiers**, cohorts (trial `grant_all` default P-1, champion "all at highest"), policy values (§6.4), resolver (§8), lifecycle state stored per account, admin ops (§12), invariant tests, shadow-mode logging and report, **launch migration built and dry-runnable** (not executed). New signups → `trial` (clock from first onboarding message). | None. Nothing is blocked. | AC-1 to AC-7, AC-26 (dry-run part), AC-36 and AC-37 pass. The shadow report shows, for real accounts, what would be gated. A test-fixture tier matrix in Eyal's shape loads and validates. |
| **2. Enforcement** *(later)* | Commercial tier configuration added (B-13), **launch migration executed at switch-on** (every existing account → open-ended champion, P-2), gating on all §10.2 surfaces, the §10.4 conflict fixes (reminder scheduling/sending, branding), grace/paused behavior including the transactional/marketing split and the B-11 paused split, failure policy, grandfathering | Accounts are held to their entitlements. Trial and champion lifecycle is live. | AC-8 to AC-16, AC-26 (executed part), AC-27 to AC-29 and AC-32 to AC-33 pass. Shadow report shows no unexplained denials before the flag flips. |
...
**Note for SA (B-3 revised + P-2):** if P-2 is confirmed, every account existing at switch-on, including trials started after Slice 1 shipped, becomes an open-ended champion. The §21.3 "reset `trial_ends_at` for every non-champion account at flag-flip" step then applies to no existing account, and only new signups after switch-on are trials. If the user does not confirm P-2, SA's §21.3 reset applies as written to the accounts left as trials.

---

## 17. Acceptance Criteria

ACs that name Basic / Growth / Pro are verified against a **test-fixture tier matrix** in this delivery, and against the real commercial tier configuration once it is added (B-13). ACs that depend on P-1 or P-2 are marked and must be re-checked once the user confirms.
...
- [ ] **AC-6** An admin can set the champion cohort, set or clear an expiry, assign a (fixture) tier and grant or revoke a capability through the sanctioned path. Each action produces an audit record with before/after values and reason. A non-admin gets 403, including a user whose `profiles.role` says admin.
- [ ] **AC-7** With the shadow flag on, nothing is blocked. The report lists, per account and surface, what would be gated under the current config. With the shipped config (no tiers), a champion shows no would-be denials, and so does a trial with the default contents (**P-1 PENDING**). An account on a fixture tier shows exactly the denials its fixture values imply. The report also includes the launch-migration dry run: which accounts would become open-ended champions at switch-on (**P-2 PENDING**).
...
- [ ] **AC-15** A champion with an admin-set expiry gets 30-day grace per cohort config. An open-ended champion never enters `grace` on its own.
...
**Decided business rules (2026-09-19)**
- [ ] **AC-26** (B-3 revised; timing and open-endedness **P-2 PENDING USER CONFIRMATION**) **Dry run (this delivery):** the launch migration can be run in shadow mode. It lists every Business OS account that would become a champion and changes no account state. Running it twice gives the same result. **Execution (at enforcement switch-on, Slice 2):** every Business OS account existing at the switch-on date, including accounts that started a trial after Slice 1 shipped, is in the `champion` cohort with **no expiry**. No pre-existing account is in `trial`, and none is left without a lifecycle state. An account that signs up after switch-on is in `trial`.
...
| B-3 | What happens to existing accounts at rollout? | **REVISED 2026-09-19:** **all** existing accounts become champions, not only design partners. No existing account gets a fresh trial. New signups still follow the trial lifecycle. *(Supersedes the earlier B-3: design partners → champion, everyone else → fresh 14-day trial.)* The exact timing (the enforcement switch-on date) and the champions being open-ended are **PENDING USER CONFIRMATION (P-2)**. See §9, §12, FR-14, AC-26. | **DECIDED** 2026-09-19 (decided by user, revised); timing detail pending P-2 |
...
These points come from the SA workplan review's reading of B-3, B-13 and B-14. The requirement is written to the default shown, and each is marked **PENDING USER CONFIRMATION** wherever it appears. If the user chooses otherwise, it is a config change (P-1) or a migration-parameter change (P-2), not a redesign.

| ID | Point to confirm | Default written into the requirement | Affects |
|---|---|---|---|
| **P-1** | What does a new signup's trial contain, now that it is no longer defined as "Growth-level" (B-13)? | **All capabilities** at highest variant/quantity (`grant_all` in trial cohort config). BA suggestion: exclude `beta` capabilities, which stay champion-only. | D-2, §6.2, §6.4, §8, §9, FR-14, FR-45, AC-7, AC-37 |
| **P-2** | When does the launch migration run, and do migrated champions expire? | Runs **at the enforcement switch-on date** (not when this infrastructure delivery ships), and covers every account existing at that moment, including trials started in between. Migrated champions are **open-ended** (no expiry) until an admin sets one. | B-3, D-4, §6.4, §9, §12, FR-14, §16 (SA note), AC-7, AC-15, AC-26 |
...
| 2026-09-19 | SA workplan-review follow-ups applied; two items marked PENDING USER CONFIRMATION | FR-3: no fixed set of tiers; tiers exist only as configuration added later, with no tier names in code. D-2: the trial is no longer defined as "Growth-level"; its contents are trial cohort config, default all capabilities (**P-1, PENDING USER CONFIRMATION**; BA suggests excluding beta). B-3 / FR-14: the launch migration is built and dry-runnable now, and executed at the **enforcement switch-on date**. It makes every account existing at that moment (including trials started in between) an **open-ended** champion with no expiry (**P-2, PENDING USER CONFIRMATION**). Updated AC-4 (tier-agnostic fixture diff), AC-7 (shadow report incl. migration dry run and default trial contents), AC-26 (dry run now, execution at switch-on), plus AC-14/15/27/28/37, §2, §3 (D-2, D-4), §4, §6.2, §6.4, §8, §9, §12, FR-5/11/22/43/45, NFRs, §16 (SA note on the §21.3 trial reset under P-2), §18. Added the "Pending user confirmation" table (P-1, P-2) to §19. §21 and T-1 to T-12 unchanged. |
