...
### Group D — Hardening / debt / follow-ups (LOW urgency, except the D9/D10 hotfixes + the D12 near-term deliverability bug).
...
| **D12** (NEW · **NEAR-TERM** — user-visible deliverability bug) | **Email PLUGIN discards the plaintext `body` and sends single-part MIME — same root class as D9, on the AGENT-facing path.** **Refined root cause:** the Gmail plugin definition (`lib/plugins/definitions/google-mail-plugin-v2.json` send_email → content) ALREADY declares BOTH `body` ("Plain text email body") AND `html_body` ("HTML formatted email body"). But the executor's `buildEmailMessage` (`lib/server/gmail-plugin-executor.ts` ~L622-644) treats them as **mutually exclusive** — when `html_body` is present it emits single-part `text/html` and **silently discards `body`** (the `else if (content?.body)` branch only fires when `html_body` is absent). So the plaintext field EXISTS in the schema but is thrown away whenever HTML is set → single-part HTML, no plaintext alternative (renders as raw text in some clients; hurts deliverability/spam-scoring + accessibility). **Refined fix:** in `buildEmailMessage`, when `html_body` is present, build a proper `multipart/alternative` — a `text/plain` part = `content.body` if provided, ELSE `htmlToText(html_body)`; and a `text/html` part = `html_body`. **Do NOT discard `body`.** Preserve the text-only path (body only, no `html_body`) and encoding/base64url. Reuse a SHARED `htmlToText` util (extract D9's from `lib/notifications/emailTransport.ts`). **Email-send-path audit — RESOLVED (replaces the prior "audit pending"):** sending is FRAGMENTED across paths, most single-part — (1) ✅ `lib/notifications/emailTransport.ts` shared transport (calibration/system) — already fixed (D9); (2) ❌ `lib/server/gmail-plugin-executor.ts` agent Gmail send — **D12 primary** (above); (3) ⚠️ `lib/server/outlook-plugin-executor.ts` agent Outlook send via MS Graph (`contentType:'HTML'`, no plaintext alternative) — **different fix shape** (Graph message model, not raw MIME) — include with a note; (4) ⚠️ `app/api/contact/route.ts` contact form uses its OWN `nodemailer` instance, `{ html }`-only (2 sends), bypassing the shared transport — non-agent path — SECONDARY / general email-hygiene (sub-decision: fix here or route through the shared transport); (5) ⚠️ `lib/plugins/google-mail/sendEmailDraft.ts` in the DEPRECATED V1 plugin tree (`lib/plugins/`), single-part `text/html` (even its PDF `multipart/mixed` branch has no `text/plain` alternative) — **verify reachability FIRST; DELETE if dead code (do NOT extend V1), else fix** — sub-decision flagged; (6) `app/api/admin/messages/[id]/replay/route.ts` — a stub/comment, not a real send — n/a. **Architectural recommendation:** converge email sending onto ONE shared utility (shared `htmlToText` + a single MIME/multipart builder, or route through the fixed transport) so plaintext-multipart is guaranteed everywhere and can't fragment again. **Sub-note (RESOLVED — confirmed):** V6 generation emits ONLY `html_body`; the plaintext `body` is NEVER wired (agent `0ee53785` step14 send_email has `content: { subject, html_body: "{{email_report.html_body}}" }` — no `body` field; the `html_body` comes from an upstream report-generate step that emits HTML only). **Consequence:** D12's executor auto-generate-plaintext fallback (`htmlToText(html_body)` when `body` is absent) is **ESSENTIAL, not optional** — it is the branch that ACTUALLY FIRES in practice, since there is no `body` to use. **Optional lower-priority generation-side follow-up (NOT part of D12 core):** have the report/delivery generation also emit a plaintext `body` alongside `html_body` — a nice-to-have in the contested V6 area, gated on coordination with the WP-62/63 team; NOT required for D12 to fully fix the user-visible bug (the executor fallback covers it). **Relates to D9** (identical root class, notification path); D12 completes the email-multipart fix on the agent-facing side. | plugin system + shared email/notification util (Outlook: plugin system; contact form: general/web) | **Med** | new (agent-facing deliverability; completes D9) |
| **D13** (NEW) | **`generatePDF` is a non-functional void stub; the dashboard PDF-export is a no-op.** `lib/pdf/generatePDF.ts` is `export function generatePDF(data: any, _outputSchema?: any[]): void {…}` — it returns `void` and never produces a PDF. Discovered during D12 (it was the source of the broken `sendEmailDraft` PDF branch, now removed). It still has ONE live caller: `components/dashboard/AgentSandBox/useAgentSandbox.ts:1083` — the dashboard "generate PDF" action — which is therefore a silent no-op (produces nothing usable). **Fix plan:** either IMPLEMENT `generatePDF` to actually produce a PDF (`Uint8Array`/`Buffer`) with a correct signature and wire the dashboard caller to use the output, OR REMOVE the dashboard PDF-export affordance if the feature isn't wanted — decide based on whether PDF export is a desired product feature. **Relates to D12** (which removed the email-side caller of this same stub). | dashboard UI + `lib/pdf` | Low | new (dead/no-op feature) |
...
- **D12:** all AGENT-facing send paths deliver `multipart/alternative` (Gmail primary — HTML + non-empty plaintext; Outlook noted, via the MS-Graph message model); the plaintext `content.body` is NO LONGER discarded when `html_body` is set (used as the plaintext part when provided, else `htmlToText(html_body)` — the fallback branch is the one that actually fires today, since generation emits only `html_body`); the `htmlToText` logic is a SINGLE shared util (no duplication); the DEPRECATED V1 sender (`sendEmailDraft.ts`) is verified — deleted if dead, fixed if live (never extended as V1); the text-only path is unchanged; tests cover multipart construction + the text-only path + the `body`-not-discarded behavior. (Contact-form send and the optional generation-side `body` population are flagged sub-decisions/follow-ups, not required for D12.)
- **D13:** the dashboard PDF-export either produces a valid PDF, or the non-functional affordance is removed; `generatePDF` is no longer a void stub with live callers relying on a return value.
...
| D12 | NEW (near-term — email plugin discards `body` / single-part MIME → multipart/alternative on the agent-facing path + email-send-path convergence; completes D9) |
| D13 | NEW (dead/no-op feature — `generatePDF` void stub / dashboard PDF-export no-op; relates to D12) |
...
| **Agent email plugin MIME (multipart/alternative) + shared `htmlToText` util + email-send-path convergence** | primary `lib/server/gmail-plugin-executor.ts` `buildEmailMessage` (~L622-644, discards `body`); plugin def `google-mail-plugin-v2.json` send_email content (`body`+`html_body`); shared `htmlToText` extracted from `lib/notifications/emailTransport.ts`. Audited paths: `lib/server/outlook-plugin-executor.ts` (MS-Graph HTML, no plaintext — different fix shape), `app/api/contact/route.ts` (own nodemailer, html-only — secondary), `lib/plugins/google-mail/sendEmailDraft.ts` (DEPRECATED V1 — verify/delete/fix), `app/api/admin/messages/[id]/replay/route.ts` (stub — n/a). Generation confirmed to emit only `html_body` → executor `htmlToText` fallback is the operative path. | D12 |
| **PDF export (dashboard)** | `lib/pdf/generatePDF.ts` (void stub); live caller `components/dashboard/AgentSandBox/useAgentSandbox.ts:1083`. Implement + wire, or remove the affordance. | D13 |
...
| 2026-07-13 | Backlog D12 added | Appended **D12** to Group D (near-term, user-visible deliverability bug) — the Gmail email PLUGIN `buildEmailMessage` builds single-part MIME so agent-sent HTML emails lack a plaintext part. Same root class as D9 but on the agent-facing path. |
| 2026-07-13 | Backlog D12 refined + audit folded in | Refined D12: **precise root cause** — the plugin def already declares BOTH `body` and `html_body`, but `buildEmailMessage` (`gmail-plugin-executor.ts` ~L622-644) treats them as mutually exclusive and **silently discards `body`** when `html_body` is set (single-part HTML). **Refined fix** — build `multipart/alternative` (plaintext = `content.body` if given else `htmlToText(html_body)`; html = `html_body`); do NOT discard `body`; preserve text-only + encoding; reuse a shared `htmlToText` util extracted from `emailTransport.ts`. **Folded in the completed email-send-path audit** (resolves the prior "audit pending"): shared transport (fixed, D9); Gmail plugin (D12 primary); Outlook plugin (MS-Graph HTML, different fix shape, noted); contact form (own nodemailer, html-only, secondary sub-decision); DEPRECATED V1 `sendEmailDraft.ts` (verify/delete/fix sub-decision); admin replay stub (n/a). Added the **architectural recommendation** to converge on one shared email utility, and a **sub-note** that generation-side `body` population is under investigation. Updated the D12 AC and the D12 Integration-Points row + backlog-map row + fix-owner accordingly. |
| 2026-07-13 | D12 sub-note finalized (generation investigation RESOLVED) | Confirmed: V6 generation emits ONLY `html_body`, never wires the plaintext `body` (agent `0ee53785` step14 send_email `content: { subject, html_body: "{{email_report.html_body}}" }` — no `body`; html_body from an upstream HTML-only report-generate step). Updated the D12 sub-note from "under investigation" to RESOLVED, stated that D12's executor `htmlToText(html_body)` fallback is therefore ESSENTIAL (the branch that actually fires today), and reframed the generation-side `body`-emission as an OPTIONAL lower-priority follow-up (nice-to-have, gated on WP-62/63 coordination, NOT required for D12). Reflected the "fallback is the operative path" note in the D12 AC and the D12 Integration-Points row. |
| 2026-07-13 | Backlog D13 added | Appended **D13** to Group D (Low priority) — `lib/pdf/generatePDF.ts` is a non-functional `void` stub, so the dashboard PDF-export (live caller `components/dashboard/AgentSandBox/useAgentSandbox.ts:1083`) is a silent no-op. Discovered during D12 (source of the now-removed `sendEmailDraft` PDF branch). Fix: either implement `generatePDF` (return a real `Uint8Array`/`Buffer`) and wire the dashboard caller, or remove the non-functional affordance — decide on whether PDF export is a wanted feature. Owner: dashboard UI + `lib/pdf`. Added its AC, backlog→Item map row, an Integration-Points row, and the fix-owner. |
