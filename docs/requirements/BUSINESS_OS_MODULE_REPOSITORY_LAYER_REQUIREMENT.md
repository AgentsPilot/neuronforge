...
| Payments | `PaymentRepository`, `PaymentPlanRepository`, `PaymentProcessorRepository`, `PaymentEventRepository`, `PaymentReminderRepository`, `PaymentAutomationRepository` | Payment tables. `PaymentRepository` → `payment_invoices`, `payment_transactions`, `stripe_connect_accounts`. `PaymentPlanRepository` → `payment_plans`, `payment_plan_installments`. Phase-2 repos → `payment_processors` + `saved_payment_methods` (`PaymentProcessorRepository`), `payment_events` (`PaymentEventRepository`), `payment_reminders` (`PaymentReminderRepository`), `payment_automation_rules` (`PaymentAutomationRepository`). Not owned: `payment_automation_executions` (deferred), `payment_methods` (dead table — flagged for a future DROP migration) |
