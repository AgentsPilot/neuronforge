...
**Created by:** BA
**Date:** 2026-09-17
**Status:** SA approved — changes applied, ready for Dev workplan (SA review 2026-09-17; RC-1 to RC-20 applied 2026-09-17; workplan-review amendments BA-1 and BA-2 applied 2026-09-18). **OQ-U2 / F-8 resolved by the user 2026-09-18 (D-6): keep the freeze** — Layer 1.5 converts none of the three touched non-compliant files; the conversions are open items OI-4 to OI-6.
...
**It does not change what anyone is charged.** Images are tracked, not charged; the charging decision is parked and must be revisited (see [The parked charging decision](#the-parked-charging-decision)). One consequence is user-visible and recorded as [UD-1](#for-the-user-business-decisions): a brand-new business will see part of its first month's allowance already used, for its own onboarding conversation.
...
- [BUSINESS_OS_LLM_LAYER1_5_WORKPLAN.md](/docs/workplans/BUSINESS_OS_LLM_LAYER1_5_WORKPLAN.md) — code-reality check (M-1 to M-6) and SA review §11 (BA-1, BA-2).
...
- As a **platform operator**, I want every AI-generated image to appear in the usage ledger with its cost, in its own area, so that image spend stops being invisible and can be read as one number.
- As a **platform operator**, I want the image model, its sizes and its price to be configurable, so that a price change or a model change doesn't need a code change.
...
- As a **business owner**, I want nothing about my bill or my credits to change yet, because the charging decision hasn't been made.
...
8. **FR-8 — Images go through the provider layer, in this shape (RC-7).** `lib/services/GeneratedImageService.ts` must no longer construct an OpenAI client itself (`:186-192`).
   - **Provider method:** an image method on `OpenAIProvider`, e.g. `generateImage(params: { model, prompt, size, quality, n }, context: CallContext, pricing: { usdPerImage: number })`, wrapped in the existing `callWithTracking` (`baseProvider.ts:70-145`) with `extractMetrics` returning `inputTokens: 0, outputTokens: 0, cost: usdPerImage × n`.
   ...
   - **No pricing policy in `lib/ai/**`:** the per-image price is resolved by the service (FR-13) and passed to the provider.
...
9. **FR-9 — One ledger row per generated image, and the edge cases (RC-2, RC-9).** Every image that is actually generated writes one `token_usage` row with the business account, the **`images` area and `image_generation` call name**, the grouping id (FR-11), **zero input and output tokens**, the estimated dollar cost (FR-13), the model, and success or failure.
   ...
   - **(c) A 200 response with no image data** (`:194-198`) **was billed**: the row is written **with its cost**, and the service still returns `{ ok: false, reason: 'failed' }`.
...
10. **FR-10 — No hardcoded model; one configuration read (RC-8, OQ-E, BA-2 / M-2).** The image model (today the literal `'gpt-image-1'`, `:188`), its size options, its quality **and its per-image price** live together in `system_settings_config` and are read through `SystemConfigRepository` in **one** round trip: a new `getImageGenerationConfig()` built on the existing **`getByKeys(keys)`** (`SystemConfigRepository.ts:79-98`), a single `.in('key', keys)` select.
    ...
    - **`quality` is pinned**: the request sends an explicit `quality` value from configuration. Today the call omits it (`:186-192`) and takes the provider's default, so a provider-side default change would silently invalidate the price.
...
13. **FR-13 — Per-image pricing: configuration → documented fallback → 0 (RC-8).** The dollar cost of an image never comes from the token price table (`ai_model_pricing` and `lib/ai/pricing.ts` are strictly per token, OQ-F).
    - **Precedence, in this order:**
      1. the configured price (FR-10), keyed by **model + size + quality**;
      2. a **documented in-code fallback map**, the codebase's own pattern for this case (`FALLBACK_PRICING` in `lib/ai/pricing.ts`, `DEFAULT_TOKENS_PER_CREDIT = 10`, `creditCostUsd ?? 0.00048`). A documented default that configuration overrides is not a "hardcoded price"; an unoverridable literal would be;
...
- **Configuration, not code:** the image model, its sizes, its quality and its per-image price are configuration values read at call time, with a documented in-code fallback (FR-13) and no unoverridable literal (CLAUDE.md mandatory rule 5). The onboarding calls' `'gpt-4o'` literals are a carried known issue (KI-C, F-9).
- **Logging:**
  - Pino everywhere in new code, with `correlationId` on request paths. New error paths (price resolution falling through to 0, a failed or truncated chat usage read) log at error or warn level with enough context to act on, and never include prompts.
...
- **Testability:** every new rule is unit-testable without a live provider: the attribution context, image price resolution and its precedence, the zero-token behaviour, the image edge cases (FR-9), and the chat report's truncation and failure states.
...
- [ ] **AC-8** (FR-10, FR-13) — Price resolution follows configuration → documented in-code fallback → 0 with an error-level log naming model, size and quality. The key includes model, size and quality, and the request sends an explicit `quality`. The model, sizes, quality and price come from **one** `getImageGenerationConfig()` read built on `getByKeys`, which logs no configuration values. No image model name or unoverridable price literal remains in the service.
...
- [ ] **AC-23** (all parts, RC-18, BA-1) — With the LLM Usage tab open on the test business and "Start now" clicked:
  ...
  - an image is generated from the website editor → one row appears under the **`images`** area with `image_generation`, zero tokens and a dollar cost, carrying the request's group;
...
| **OI-4** | **`lib/analytics/aiAnalytics.ts` — 16 `console.*` calls to convert to Pino** (`:97, :101, :129, :137, :186, :208, :209, :216, :217, :218, :235, :236, :246, :297, :323, :372`). **SA risk: highest** — every ledger row in the product passes through `trackAICall`, and its `console.*` calls sit inside the insert's success and failure branches. **How:** a separate, purely mechanical commit, with the insert payload, the UUID validation and the platform-account fallback untouched. (Formerly F-8.) | **Open — handle later** (user decision D-6, 2026-09-18). Deliberate exception to CLAUDE.md § Logging |
| **OI-5** | **`lib/orchestration/IntentClassifier.ts` — 16 `console.*` calls to convert to Pino.** **SA risk: medium** — an orchestration hot path with no Layer 1.5 test coverage, so a mechanical conversion would be unverified by anything this layer runs. Standalone commit | **Open — handle later** (D-6). Deliberate exception to CLAUDE.md § Logging |
| **OI-6** | **`lib/ai/providers/openaiProvider.ts` — 4 `console.*` calls to convert to Pino.** **SA risk: low** — all four sit in `getInstance`'s configuration guards, outside the image path, so a conversion cannot affect a tracked call. Standalone commit | **Open — handle later** (D-6). Deliberate exception to CLAUDE.md § Logging |
| **UD-1** | A brand-new business sees part of its first month's allowance already used, for its own onboarding conversation. Nothing is billed or blocked | Recorded; AC-23 measures the real number, decision with the deduction layer |
...
**Follow-ups tracked outside this layer:** F-7 (Check 3(c) → Fail), F-9 (onboarding model configuration), F-10 (`usage.category.*` dictionary entries), F-11 (image reuse hash), **F-12** (the unreachable `extractClientTracking`, KI-D). F-8 is superseded by **OI-4**.
...
- [x] **OQ-D — The provider-layer image path.** An image method on `OpenAIProvider` through `callWithTracking`, with an optional `requestType` on `CallContext` defaulting to `'chat'`; the tracker needs no change; the price is passed in by the service. Applied in FR-8, AC-6.
- [x] **OQ-E — Where the image configuration lives.** Model, sizes, quality **and** per-image price together in `system_settings_config`, read through `SystemConfigRepository` in one read — built on `getByKeys` (BA-2 correction). Applied in FR-10, AC-8.
...
**Status:** 🔄 Revision Required — approved in substance, conditional on RC-1 to RC-20 being applied by the BA before the Dev workplan. **BA applied RC-1 to RC-20 on 2026-09-17.** RC-16 was left open pending the user and is **resolved 2026-09-18 (D-6, keep the freeze)**. Workplan-review amendments **BA-1 and BA-2 applied 2026-09-18** (see [below](#amendments-from-the-workplan-sa-review-2026-09-17)).
...
- **OQ-D — the provider-layer image path: an image method on `OpenAIProvider`, through `callWithTracking`, with an optional request type.** Confirmed viable and minimal:
  ...
  - Shape: `generateImage(params: { model, prompt, size, quality, n }, context: CallContext, pricing: { usdPerImage: number })`. The price is resolved by the service and passed in, so `lib/ai/**` holds no pricing policy and stays product-agnostic.
