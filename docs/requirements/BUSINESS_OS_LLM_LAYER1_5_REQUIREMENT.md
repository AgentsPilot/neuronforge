...
| **KI-C** | The four onboarding methods pass `model: 'gpt-4o'` as a literal (`OnboardingConversationManager.ts:951`, `:996`, `:1098`, `:1398`), against CLAUDE.md rule 5. Layer 1.5 carries it unchanged rather than blessing it | ✅ **Closed 2026-09-21 by Layer 2 Step 2.** All four calls now resolve `{ model, temperature }` from the `onboarding` area row through `resolveBosLlmSettings`, with the default in `lib/business-os/llm/modelSettingsPolicy.ts`. Layer 2 Step 4 added `npm run check:bos-llm-literals`, which fails CI if a literal returns |
...
| 2026-09-21 | KI-C closed by Layer 2 | The four onboarding extractors no longer pass `model: 'gpt-4o'`: each resolves `{ model, temperature }` from the `onboarding` area row through `resolveBosLlmSettings`, with the default in `lib/business-os/llm/modelSettingsPolicy.ts` (the one place a model name is written). Layer 2 Step 4 added `npm run check:bos-llm-literals` to the existing `bos-llm-typecheck` CI job, so the literal cannot come back unnoticed. The onboarding area is deliberately **not switchable** — a half-built account is worse than an expensive one |
