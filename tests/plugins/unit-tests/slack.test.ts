...
describe('SlackPluginExecutor', () => {
  ...
  describe('[full]', () => {
    ...
    // ---- get_or_create_channel create path ----
    describe('get_or_create_channel', () => {
    ...
    runStandardErrorScenarios(
      ...
      PLUGIN_KEY,
      'send_message',
      { channel_id: 'C123', message_text: 'Hello' }
    );

    // ---- P3-T3: Malformed response tests ----
    describe('malformed responses', () => {
      it('handles response with missing expected fields', async () => {
        mockFetchSuccess({}); // Missing ok, ts, channel
        const result = await executor.executeAction(USER_ID, 'send_message', {
          channel_id: 'C123',
          message_text: 'Hello',
        });
        // Slack executor checks ok field — missing ok treated as failure
    ...
    describe('authentication edge cases', () => {
      it('handles empty access_token', async () => {
        ...
        mockFetchError(401, { error: 'invalid_auth' });
