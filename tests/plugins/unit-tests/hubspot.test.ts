  ...
  describe('[full]', () => {
    ...
    describe('authentication edge cases', () => {
      it('handles empty access_token', async () => {
        ...
        mockFetchError(401, { message: 'Authentication credentials not found' });
        const result = await ctx.executor.executeAction(USER_ID, 'search_contacts', {
          query: 'test@example.com',
        });
        expectErrorResult(result);
      });
    ...
    describe('pagination edge cases', () => {
      it('handles empty search results', async () => {
        ...
        expectSuccessResult(result);
        expect(result.data.data.contacts).toHaveLength(0);
      });
    });
  });
});
