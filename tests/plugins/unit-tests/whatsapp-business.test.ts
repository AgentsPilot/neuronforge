...
describe('WhatsAppPluginExecutor', () => {
  ...
  describe('[full]', () => {
    ...
    // ---- send_interactive_message list variant ----
    describe('send_interactive_message', () => {
    ...
    runStandardErrorScenarios(
      () => executor,
      WhatsAppPluginExecutor,
      PLUGIN_KEY,
      'send_text_message',
      { recipient_phone: '+1234567890', message_text: 'Hello' }
    );

    // ---- P3-T3: Malformed response tests ----
    describe('malformed responses', () => {
      it('handles response missing messages field', async () => {
        mockFetchSuccess({});
        const result = await executor.executeAction(USER_ID, 'send_text_message', {
          recipient_phone: '+1234567890',
          message_text: 'Hello',
    ...
    describe('authentication edge cases', () => {
      it('handles empty access_token', async () => {
        const ctx = await createTestExecutor(WhatsAppPluginExecutor, PLUGIN_KEY, {
          access_token: '',
          profile_data: { phone_number_id: 'mock-phone-123', waba_id: 'mock-waba-456' },
        });
        mockFetchError(401, { error: { code: 190, message: 'Invalid OAuth 2.0 Access Token' } });
        const result = await ctx.executor.executeAction(USER_ID, 'send_text_message', {
          recipient_phone: '+1234567890',
          message_text: 'Hello',
        });
        expectErrorResult(result);
