...
| 2026-09-26 | Admin handler count refreshed | Admin row now reads 80 handlers across 51 route files, 74 via `requireAdmin` and 6 hand-rolled (was 72 / 7). The audit-trail route moved to `requireAdmin` in the admin reorganisation slice 2 (PR #112); counts re-measured there |
| 2026-09-26 | Admin handler count: archiving | 81 handlers across 52 route files, 75 via `requireAdmin`, 6 hand-rolled. Adds `GET /api/admin/archiving` (Admin Archiving slice 1) |
| 2026-09-26 | Admin handler count refreshed (slice 4) | 81 handlers across 52 route files, 75 via `requireAdmin` and 6 hand-rolled: the Health landing's `health-summary#GET` was added, gated from birth |
| 2026-09-26 | Admin handler count re-measured after merging Health (slice 4) with Archiving (slice 1) | **82 handlers across 53 route files, 76 via `requireAdmin` and 6 hand-rolled**; 25 `/admin` pages. Both new handlers (`health-summary#GET`, `archiving#GET`) were gated from birth; no guard cap moved |
