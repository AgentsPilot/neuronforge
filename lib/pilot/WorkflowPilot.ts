...
export class WorkflowPilot {
  ...
  private async collectInsights(
    ...
    try {
      ...
      // Store technical patterns as insights (data_insight or technical_insight)
      for (const pattern of analysisResult.patterns) {
        try {
          ...
          // First: Store to execution_insight_runs (historical log for this execution)
          const runStored = await repository.createInsightRun({
            insight_id: null, // Will be linked after creating/updating execution_insights
            execution_id: executionId,
            agent_id: agentId,
            user_id: userId,
            ...
            this_run_count: analysisResult.execution_count,
            last_run_count: analysisResult.execution_count - 1,
          ...
          if (runStored) {
            console.log(`📝 [WorkflowPilot] Stored run snapshot for pattern: "${pattern.insight_type}"`);
          }

          // Then: Check if this pattern category already exists (deduplication by category, not insight_type)
          const existing = await repository.findExistingInsight(
            agentId,
            pattern.category,  // Use category (e.g., 'data_insight') not insight_type (e.g., 'data_unavailable')
            7 // Look for insights created in last 7 days
          );

          if (existing) {
            console.log(`💡 [WorkflowPilot] Updating existing insight for pattern: "${pattern.insight_type}"`);
            await repository.addExecutionToInsight(existing.id, executionId);
            // Link this run to the existing insight
            await repository.linkInsightRun(executionId, insightText.title, existing.id);
            continue;
          }

          // Create new insight in execution_insights (current active state)
          console.log(`💡 [WorkflowPilot] Creating new insight for pattern: "${pattern.insight_type}" (category: ${pattern.category})`);
          const createResult = await repository.create({
            user_id: userId,
            agent_id: agentId,
            execution_ids: pattern.execution_ids.length > 0 ? pattern.execution_ids : [executionId],
            insight_type: pattern.insight_type,
            category: pattern.category, // Use pattern's category (data_insight, business_insight, technical_insight)
            severity: pattern.severity,
            confidence: pattern.confidence_score, // Use numeric confidence from pattern
            title: insightText.title,
            description: insightText.description,
            business_impact: insightText.business_impact,
            recommendation: insightText.recommendation,
          ...
          if (createResult) {
            console.log(`✅ [WorkflowPilot] Successfully saved pattern insight: "${pattern.insight_type}"`);
            // Link this run to the newly created insight
            await repository.linkInsightRun(executionId, insightText.title, createResult.id);
          }
        } catch (err) {
          console.error(`⚠️  [WorkflowPilot] Failed to save pattern "${pattern.insight_type}":`, err);
      ...
      for (const insight of analysisResult.businessInsights) {
        try {
          ...
          const runStored = await repository.createInsightRun({
            ...
            confidence: insight.confidence, // Numeric confidence from LLM
            this_run_count: analysisResult.execution_count,
            last_run_count: analysisResult.execution_count - 1,
            // LLM metrics not available in InsightAnalyzer return (would need to be added)
          ...
          if (runStored) {
            console.log(`📝 [WorkflowPilot] Stored run snapshot for business insight: "${insight.title}"`);
          }

          // Then: Check for duplicate by title (insights are now context-aware and unique)
          ...
          if (existing) {
            console.log(`💡 [WorkflowPilot] Updating existing business insight: "${insight.title}"`);
            await repository.addExecutionToInsight(existing.id, executionId);
            // Link this run to the existing insight
          ...
          const createResult = await repository.create({
            ...
            category: 'business_insight',  // FIXED: Was 'growth', now using correct category
            severity: insight.severity,
            confidence: insight.confidence, // FIXED: Use numeric confidence from insight, not confidence_mode string
            title: insight.title,
            description: insight.description,
            business_impact: insight.business_impact,
            recommendation: insight.recommendation,
          ...
          if (createResult) {
            ...
            console.error(`❌ [WorkflowPilot] Failed to save business insight (no result returned)`);
          }
        } catch (err) {
          console.error(`⚠️  [WorkflowPilot] Failed to save business insight "${insight.title}":`, err);
