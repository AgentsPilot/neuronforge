  ...
  protected async executeSpecificAction(
    ...
    switch (actionName) {
      ...
      case 'get_deal':
        ...
        break;
      case 'create_contact':
        result = await this.createContact(connection, parameters);
        break;
      case 'create_contact_note':
        result = await this.createContactNote(connection, parameters);
        break;
      case 'create_deal':
        result = await this.createDeal(connection, parameters);
        break;
      case 'create_task':
        result = await this.createTask(connection, parameters);
        break;
      default:
        return {
          success: false,
  ...
  private async getContact(connection: any, parameters: any): Promise<any> {
    ...
    try {
      if (idType === 'email') {
        ...
        // Get by ID
        const url = new URL(`${hubspotApiUrl}/objects/contacts/${contact_identifier}`);
