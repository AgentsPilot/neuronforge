...
export abstract class BasePluginExecutor {
   ...
   * `params._calibration = { isCalibration, round }` marker. Messaging executors
   * (email/Slack/etc.) call this to get ready-to-prepend banners so the recipient
   * can tell the message came from a calibration test run and which round.
   *
