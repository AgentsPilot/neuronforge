...
const logger = createLogger({ module: 'V6', service: 'SchemaCompatibilityValidator' })
...
export class SchemaCompatibilityValidator {
  ...
  private buildVariableOutputMap(graph: ExecutionGraph): Map<string, VariableOutputInfo> {
    ...
    logger.info({
      ...
      nodeIds: Object.keys(graph.nodes).join(', ')
    }, '[SchemaCompatibilityValidator] Building variable output map')

    for (const [nodeId, node] of Object.entries(graph.nodes)) {
      logger.info({
        nodeId,
        nodeType: node.type,
        operationType: node.operation?.operation_type
      }, '[SchemaCompatibilityValidator] Processing node')
      ...
      if (node.type === 'operation' && node.operation?.operation_type === 'transform') {
        ...
        if (outputVar && transform?.output_schema) {
          ...
          logger.info({
            ...
            outputVar,
            inputVar,
            hasInputOutput: !!inputOutput,
            inputOutputNodeId: inputOutput?.node_id
          }, '[SchemaCompatibilityValidator] Filter transform - checking schema inheritance')
          ...
          if (inputOutput) {
            logger.info({
              ...
              outputVar,
              inputVar,
              inheritedSchema: !!inputOutput.output_schema
            }, '[SchemaCompatibilityValidator] Filter transform inheriting schema from input')
            ...
            logger.warn({
              ...
              outputVar,
              inputVar,
              availableKeys: Array.from(outputs.keys()).join(', ')
            }, '[SchemaCompatibilityValidator] Filter transform CANNOT inherit - input not found!')
          }
        }
      }
      ...
      if (node.type === 'loop' && node.loop) {
        ...
        logger.info({
          ...
          outputVar,
          iterateOver,
          currentOutputsSize: outputs.size,
          currentOutputKeys: Array.from(outputs.keys()).join(', ')
        }, '[SchemaCompatibilityValidator] Found loop node')
        ...
        if (itemVar) {
          ...
          logger.info({
            ...
            sourceOutputNodeId: sourceOutput?.node_id,
            hasOutputSchema: !!sourceOutput?.output_schema,
            hasItems: !!sourceOutput?.output_schema?.items,
            schemaType: sourceOutput?.output_schema?.type
          }, '[SchemaCompatibilityValidator] Looking up source for loop item variable')
          if (sourceOutput?.output_schema?.items) {
            const declaredFields = this.extractDeclaredFields(sourceOutput.output_schema.items)
            logger.info({
              nodeId,
              itemVar,
              iterateOver,
              sourceNodeId: sourceOutput.node_id,
              declaredFields: Array.from(declaredFields).join(', ')
            }, '[SchemaCompatibilityValidator] Loop item inheriting schema from source array')
            ...
            logger.warn({
              ...
              hasOutputSchema: !!sourceOutput?.output_schema,
              hasItems: !!sourceOutput?.output_schema?.items
            }, '[SchemaCompatibilityValidator] Loop item cannot inherit schema - source missing!')
          }
        }
  ...
  private extractActionVariableMappingRequirements(
    ...
    }, '[SchemaCompatibilityValidator] extractActionVariableMappingRequirements called')

    if (!this.pluginManager) {
      logger.info({ nodeId }, '[SchemaCompatibilityValidator] Skipping - no plugin manager')
      return
    }
    if (op.operation_type !== 'fetch' && op.operation_type !== 'deliver') {
      logger.info({ nodeId, operationType: op.operation_type }, '[SchemaCompatibilityValidator] Skipping - not fetch/deliver')
      return
    }
    ...
    if (!pluginKey || !actionName) {
      logger.info({ nodeId, pluginKey, actionName }, '[SchemaCompatibilityValidator] Skipping - no plugin/action')
      return
    }

    const schema = this.getPluginActionSchema(pluginKey, actionName)
    if (!schema?.parameters?.properties) {
      logger.info({ nodeId, pluginKey, actionName, hasSchema: !!schema }, '[SchemaCompatibilityValidator] Skipping - no schema parameters')
      return
    }
    ...
    logger.info({
      ...
      inputVars: inputVars.join(', ')
    }, '[SchemaCompatibilityValidator] Checking x-variable-mapping requirements')
    ...
    for (const [paramName, paramDef] of Object.entries(schema.parameters.properties)) {
      ...
      }, '[SchemaCompatibilityValidator]   → Parameter needs field from input')
      ...
      for (const inputVar of inputVars) {
        ...
        logger.info({
          inputVar,
          fieldName: mapping.field_path,
          requiredBy: `${pluginKey}.${actionName}.${paramName}`
        }, '[SchemaCompatibilityValidator]   → Added field requirement')
      }
    }
  }
    ...
    for (const [varName, requirements] of fieldRequirements.entries()) {
      ...
      for (const requirement of requirements) {
        ...
        logger.debug({
          varName,
          fieldName,
          hasField: output.declared_fields.has(fieldName),
          declaredFields: Array.from(output.declared_fields).join(', '),
          sourceType: output.source_type
        }, '[SchemaCompatibilityValidator] Checking field requirement')
        ...
        if (!output.declared_fields.has(fieldName)) {
          ...
          logger.warn({
            varName,
            fieldName,
            requiredBy: requirement.required_by_operation,
            sourceType: output.source_type
          }, '[SchemaCompatibilityValidator] MISMATCH DETECTED - field missing!')
          ...
          if (output.source_type === 'transform') {
            ...
            logger.debug({
              varName,
              targetNodeId: targetTransform.node_id
            }, '[SchemaCompatibilityValidator] Direct transform - will fix')
            ...
            if (loopNode?.type === 'loop' && loopNode.loop) {
              ...
              logger.debug({
                varName,
                loopNodeId: output.node_id,
                sourceArrayVar,
                hasSourceOutput: !!sourceOutput,
                sourceOutputType: sourceOutput?.source_type
              }, '[SchemaCompatibilityValidator] Loop item - tracing back to source')
              ...
              if (sourceOutput && sourceOutput.source_type === 'transform') {
                ...
                logger.debug({
                  ...
                  sourceArrayVar,
                  targetNodeId: targetTransform.node_id
                }, '[SchemaCompatibilityValidator] Loop item - found source transform to fix')
              }
            }
          }
