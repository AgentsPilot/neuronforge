...
interface CompilerContext {
  ...
  globalPostAIFiltersApplied?: boolean // Phase 2 Task 2.2: Track if global post-AI filters were applied (hybrid approach)
}
...
export class DeclarativeCompiler {
  ...
  async compile(ir: DeclarativeLogicalIR): Promise<CompilationResult> {
    ...
    try {
      ...
      // Step 4.5: Compile conditionals (branching logic) - NEW Phase 2 Task 2.3
      const conditionalSteps = this.compileConditionals(ir, ctx)
      ...
      // Step 5.5: Compile post-AI filters GLOBALLY (hybrid approach) - NEW Phase 2 Task 2.2
      // This ensures post_ai_filters are ALWAYS applied, even if delivery patterns don't call it
      if (ir.post_ai_filters && !ctx.globalPostAIFiltersApplied) {
        this.log(ctx, 'Compiling global post-AI filters...')
        const postAIFilterSteps = this.compilePostAIFilters(ir, ctx)
        steps.push(...postAIFilterSteps)
        ctx.globalPostAIFiltersApplied = true
      }
      ...
      // Step 6.5: Compile FILE operations (upload to Drive/S3/etc.) - NEW Phase 2 Task 2.1
      const fileOpSteps = this.compileFileOperations(ir, ctx)
      steps.push(...fileOpSteps)
  ...
  /**
   * Compile file operations (file generation and upload to Drive/S3/etc.)
   * Phase 2 Task 2.1: NEW method to support file_operations IR field
   */
  private compileFileOperations(ir: DeclarativeLogicalIR, ctx: CompilerContext): WorkflowStep[] {
    const steps: WorkflowStep[] = []

    // Check if file_operations exists (newly added IR field)
    const fileOps = (ir as any).file_operations
    if (!fileOps || fileOps.length === 0) {
      return steps
    }

    this.log(ctx, `Compiling ${fileOps.length} file operation(s)...`)

    for (const fileOp of fileOps) {
      // Step 1: Generate file if needed (PDF, CSV, Excel)
      if (fileOp.type !== 'upload_file') {
        const formatUpper = (fileOp.output_config?.format || 'file').toUpperCase()
        const generateMetadata = this.generateStepMetadata(
          `generate_${fileOp.output_config?.format || 'file'}`,
          `Generate ${formatUpper} file`,
          ctx
        )
        ...
        steps.push({
          ...
          input: `{{${fileOp.source_data || ctx.currentVariable}}}`,
          config: {
            format: fileOp.output_config?.format || 'pdf',
            filename: fileOp.output_config?.filename || 'file.pdf',
            columns: fileOp.output_config?.columns,
            template: fileOp.output_config?.template
          }
        })

        ctx.currentVariable = `${generateMetadata.id}.fileData`
        this.log(ctx, `✓ Compiled file generation: ${formatUpper}`)
      }

      // Step 2: Upload file to destination (Drive, S3, etc.)
      if (fileOp.upload_destination) {
        const dest = fileOp.upload_destination

        // Resolve plugin dynamically using PluginResolver (schema-driven, NO hardcoding!)
        let resolution
        try {
          resolution = this.pluginResolver.resolveDataSource(
            dest.plugin_key,
            dest.operation_type || 'upload'
          )
        } catch (error) {
          const errorMsg = `Failed to resolve file upload plugin: ${dest.plugin_key}.${dest.operation_type}`
          this.log(ctx, `✗ ${errorMsg}`)
          throw new Error(`${errorMsg}: ${error instanceof Error ? error.message : String(error)}`)
        }

        const uploadMetadata = this.generateStepMetadata(
          `upload_${dest.plugin_key}`,
          `Upload to ${dest.plugin_key}`,
          ctx
        )

        // Build upload params
        const uploadParams: any = {
          file_data: `{{${ctx.currentVariable}}}`,
          location: dest.location,
          filename: fileOp.output_config?.filename || 'file.pdf'
        }

        // Add optional parameters if present
        if (dest.overwrite !== undefined) {
          uploadParams.overwrite = dest.overwrite
        }
        if (dest.permissions) {
          uploadParams.permissions = dest.permissions
        }
        ...
        steps.push({
          ...
          type: 'action',
          plugin: resolution.plugin_name,
          action: resolution.operation,
          params: uploadParams
        })

        // Upload returns file URL/ID
        ctx.currentVariable = `${uploadMetadata.id}.fileUrl`
        this.log(ctx, `✓ Compiled file upload: ${dest.plugin_key}.${resolution.operation}`)
      }
    }
  ...
  /**
   * Compile conditional branching logic
   * Phase 2 Task 2.3: NEW method to support conditionals IR field
   */
  private compileConditionals(ir: DeclarativeLogicalIR, ctx: CompilerContext): WorkflowStep[] {
    const steps: WorkflowStep[] = []
    ...
    this.log(ctx, `Compiling ${ir.conditionals.length} conditional(s)...`)

    for (const conditional of ir.conditionals) {
      const metadata = this.generateStepMetadata(
        `conditional_${conditional.id || this.generateStepId('cond', ctx)}`,
        `Conditional: ${conditional.description || 'branching logic'}`,
        ctx
      )

      // Build condition structure
      const condition = this.buildConditionExpression(conditional.condition, ctx)

      // Build then/else branches (these contain actions to execute)
      const thenSteps = conditional.then_actions ? this.compileConditionalActions(conditional.then_actions, ctx) : []

      // Build elif branches if present (Phase 2 Task 2.8 - multi-branch support)
      const elifBranches = conditional.elif_branches?.map(elif => ({
        condition: this.buildConditionExpression(elif.condition, ctx),
        steps: elif.actions ? this.compileConditionalActions(elif.actions, ctx) : []
      })) || []

      const elseSteps = conditional.else_actions ? this.compileConditionalActions(conditional.else_actions, ctx) : []
      ...
      // Add elif branches if present
      if (elifBranches.length > 0) {
        conditionalStep.config.elif = elifBranches
      }

      steps.push(conditionalStep)

      const branchCount = 1 + elifBranches.length + (elseSteps.length > 0 ? 1 : 0)
      this.log(ctx, `✓ Compiled conditional with ${branchCount} branch(es)`)
    }
  ...
  /**
   * Build condition expression from IR conditional
   */
  private buildConditionExpression(expr: any, ctx: CompilerContext): any {
    if (expr.type === 'simple') {
      return {
        conditionType: 'simple',
        field: expr.field,
    ...
    if (expr.type === 'complex' && expr.conditions) {
      return {
        conditionType: expr.combineWith === 'AND' ? 'complex_and' : 'complex_or',
        conditions: expr.conditions.map((c: any) => this.buildConditionExpression(c, ctx))
      }
    }
  ...
  /**
   * Compile conditional actions to workflow steps
   */
  private compileConditionalActions(actions: any[], ctx: CompilerContext): any[] {
    const steps: any[] = []

    for (const action of actions) {
      if (action.type === 'send_to_recipient') {
        // Map send_to_recipient action to delivery step
        const params = action.params || {}
        const pluginKey = params.plugin_key
        const operationType = params.operation_type || 'send'

        // Resolve plugin
        let resolution
        try {
          resolution = this.pluginResolver.resolveDelivery(pluginKey, operationType as any)
        } catch (error) {
          this.log(ctx, `✗ Failed to resolve conditional action plugin: ${pluginKey}.${operationType}`)
          continue
        }

        const actionMetadata = this.generateStepMetadata(
          `cond_action_${pluginKey}`,
          `Conditional action: ${pluginKey}`,
          ctx
        )
        ...
        this.log(ctx, `✓ Compiled conditional action: ${resolution.plugin_name}.${resolution.operation}`)
      } else if (action.type === 'set_field') {
        // Map set_field action to transform step
        const metadata = this.generateStepMetadata('cond_set_field', 'Set field', ctx)
        steps.push({
          ...metadata,
          type: 'transform',
          operation: 'set_field',
...
{{${itemVar}}}`
  ...
  private logIgnoredIRFields(ir: DeclarativeLogicalIR, ctx: CompilerContext): void {
    ...
    // conditionals is now fully implemented (Phase 2 Task 2.3) - no warning needed
    // file_operations is now fully implemented (Phase 2 Task 2.1) - no warning needed
