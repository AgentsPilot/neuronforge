...
export class ConditionalBranchingRule extends BaseCompilerRule {
  ...
  async compile(context: CompilerContext): Promise<WorkflowStep[]> {
    ...
    const deliveryInConditionals = ir.conditionals.some(c =>
      c.then?.some((action: any) => typeof action === 'string' && action.includes('delivery')) ||
      c.else?.some((action: any) => typeof action === 'string' && action.includes('delivery'))
    )
    ...
    return actions.map((action, i) => {
      ...
      if (action.type === 'log') {
        return {
          ...
          type: 'action',
          plugin: 'logger',
          action: 'log',  // Use 'action' for PILOT executor compatibility
          config: {
  ...
  /**
   * Get plugin name for delivery method (schema-driven)
