...
export class IntentToIRConverter {
  ...
  private convertDataSource(step: DataSourceStep & BoundStep, ctx: ConversionContext): string {
    ...
    // NEW: Check if parameters were already mapped at binding time
    const boundStep = step as any
    if (boundStep.mapped_params && Object.keys(boundStep.mapped_params).length > 0) {
      logger.debug(
        `[IntentToIRConverter] Pre-mapped parameters from binding phase for ${step.plugin_key}.${step.action}`
      )

      // CRITICAL FIX: Normalize config objects to strings FIRST
      // CapabilityBinderV2 creates {kind: "config", key: "..."} objects
      // But runtime ExecutionContext.resolveAllVariables() expects "{{config...}}" strings
      const normalizedParams = this.normalizeConfigObjects(boundStep.mapped_params, ctx)

      // IMPORTANT: Still need to apply x-variable-mapping for parameter renaming
      // Binding phase skips x-variable-mapping, so we need to handle it here
      if (this.pluginManager && step.plugin_key && step.action) {
        const schema = this.getPluginActionSchema(step.plugin_key, step.action)
        if (schema) {
          logger.debug(`[IntentToIRConverter] Applying x-variable-mapping to pre-mapped parameters`)
          finalParams = this.mapParamsToSchema(normalizedParams, schema, ctx)
        } else {
          finalParams = normalizedParams
        }
      } else {
        finalParams = normalizedParams
      }
    } else if (this.pluginManager && step.plugin_key && step.action) {
      // FALLBACK: Map parameters the old way (Phase 3 mapping)
      const schema = this.getPluginActionSchema(step.plugin_key, step.action)
      if (schema) {
    ...
    // Extract input variables from config for SchemaCompatibilityValidator
    const inputVars = this.extractInputVariables(finalParams)
  ...
  private convertArtifact(step: ArtifactStep & BoundStep, ctx: ConversionContext): string {
    ...
    // PRIORITY 1: Use mapped_params from CapabilityBinderV2 if available
    if (step.mapped_params && Object.keys(step.mapped_params).length > 0) {
      logger.debug({ stepId: step.id, mappedParamCount: Object.keys(step.mapped_params).length },
        '[IntentToIRConverter] Using pre-mapped parameters from binding phase')
      // CRITICAL FIX: Normalize config objects to strings
      params = this.normalizeConfigObjects(step.mapped_params, ctx)
    } else {
      // FALLBACK: Process artifact options FIRST (may contain config references)
  ...
  private convertExtract(step: ExtractStep & BoundStep, ctx: ConversionContext): string {
    ...
    // OPTIMIZATION: If extract input references a deliver operation output (e.g., uploaded file),
    // check if that deliver operation used content from a previous step.
    // If so, extract should use the original content source instead of the deliver output.
    // This avoids unnecessary round-trips: download → upload → download-again-to-extract
    // Better pattern: download → [upload AND extract both use downloaded content]
    let extractInput = step.extract.input
    logger.debug({ stepId: step.id, extractInput, nodesCount: ctx.nodes.size },
      '[IntentToIRConverter] Checking for content source optimization')
    const originalContentSource = this.findOriginalContentSource(extractInput, ctx)
    if (originalContentSource) {
      logger.info({ stepId: step.id, originalInput: extractInput, optimizedInput: originalContentSource },
        '[IntentToIRConverter] ✅ OPTIMIZATION: Using original content source for extraction')
      extractInput = originalContentSource
    } else {
      logger.debug({ stepId: step.id, extractInput },
        '[IntentToIRConverter] No optimization applied - no content source found')
    }

    const genericConfig: Record<string, any> = {
      input: this.resolveRefName(extractInput, ctx),
      ...
      ? (() => {
          ...
          if (step.mapped_params && Object.keys(step.mapped_params).length > 0) {
            logger.debug({ stepId: step.id, mappedParamCount: Object.keys(step.mapped_params).length },
              '[IntentToIRConverter] Using pre-mapped parameters from binding phase')
            // CRITICAL FIX: Normalize config objects to strings
            const normalizedParams = this.normalizeConfigObjects(step.mapped_params, ctx)
            return {
              operation_type: 'deliver',
              deliver: {
          ...
          // FALLBACK: Schema-aware parameter mapping for plugin-based extraction
   ...
   *
   * IMPORTANT: Variable references must be wrapped in {{}} for runtime resolution
   */
  private resolveValueRef(valueRef: ValueRef | undefined, ctx: ConversionContext): any {
    ...
    switch (valueRef.kind) {
      ...
      case 'ref':
        ...
        // Wrap in {{}} for runtime variable resolution
        return `{{${ref}}}`
  ...
  /**
   * Find a schema parameter that matches a semantic field name
   * Uses x-semantic-aliases from plugin schema for schema-driven mapping
   */
  ...
  /**
   * Normalize config objects to string format
   * Converts {kind: "config", key: "..."} → "{{config...}}"
   * Converts {kind: "ref", ref: "...", field: "..."} → "{{var.field}}"
   *
   * This fixes the bug where CapabilityBinderV2 creates object format
   * but runtime ExecutionContext.resolveAllVariables() expects string format
   */
  private normalizeConfigObjects(params: Record<string, any>, ctx: ConversionContext): Record<string, any> {
    ...
    for (const [key, value] of Object.entries(params)) {
      ...
      if (typeof value === 'object' && value !== null && 'kind' in value) {
        ...
        logger.debug(`[IntentToIRConverter] Normalized ${key}: ${JSON.stringify(value)} → ${normalized[key]}`)
      }
  ...
  /**
   * Extract input variable names from operation config
   * This is critical for SchemaCompatibilityValidator to work correctly!
   */
  private extractInputVariables(config: Record<string, any>): string[] {
    const vars = new Set<string>()

    const scan = (obj: any) => {
      if (typeof obj === 'string') {
        // Extract from template strings like "{{var}}" or "{{var.field}}"
        ...
        // Also check plain references (IR format before template wrapping)
        // e.g., "attachment.id" or "email.subject"
        if (obj.includes('.') && !obj.includes('{{') && !obj.includes(' ')) {
          const varName = obj.split('.')[0]
          if (varName !== 'config' && varName !== 'item') {
            vars.add(varName)
          }
        }
      } else if (typeof obj === 'object' && obj !== null) {
        // Variable reference object like {"kind": "ref", "ref": "attachment", "field": "id"}
  ...
  private mapParamsToSchema(
    ...
    // This maps semantic field names to schema-specific parameter names based on descriptions and types
    ...
    // Handle structured refs that need x-variable-mapping application
    // Look for refs in genericParams that should map to schema parameters
    for (const [paramName, paramDef] of Object.entries(paramSchema)) {
      if (mappedParams[paramName]) continue  // Already mapped

      const mapping = (paramDef as ActionParameterProperty)['x-variable-mapping']
      if (!mapping?.field_path) continue

      // Look for a structured ref that has the right variable
      // We'll reconstruct the ref with the correct field from schema
      for (const [genericKey, genericValue] of Object.entries(genericParams)) {
        ...
        // Check if this is a structured ref
    ...
    // Copy over any other parameters that aren't generic
    // ONLY if they're defined in the plugin schema (no special exceptions)
    for (const [key, value] of Object.entries(genericParams)) {
      if (key !== 'data' && key !== 'destination' && key !== 'input_ref' && !mappedParams[key]) {
        // Only copy if it's in the schema (schema-driven, no hardcoded exceptions)
        if (paramSchema[key]) {
    ...
    // This handles parameters that should be injected from the workflow configuration
    for (const [paramName, paramDef] of Object.entries(paramSchema)) {
      ...
      if (contextBinding) {
        ...
        if (!configParam && ctx.config) {
          const semanticMatch = this.findBestSemanticConfigMatch(configKey, ctx.config)

          if (semanticMatch) {
            configParam = ctx.config.find(c => c.key === semanticMatch)

            if (configParam) {
              const score = this.calculateSemanticSimilarity(configKey, semanticMatch)
              logger.debug(
        ...
        if (configParam) {
          ...
          logger.debug(`  → Injected ${paramName} from workflow config: {{config.${configKey}}}`)
        } else {
    ...
    // FOURTH PASS: Apply format transformations based on schema metadata
    // This handles parameters that need special formatting (e.g., A1 notation for Google Sheets range)
    for (const [paramName, paramDef] of Object.entries(paramSchema)) {
      ...
      if (artifactField && !mappedParams[paramName]) {
        ...
        const sourceValue = mappedParams[artifactField] || genericParams[artifactField]

        if (sourceValue && typeof sourceValue === 'string') {
          // For Google Sheets range parameter, ensure proper format
          if (paramName === 'range') {
            // If value is already a config reference, keep it as-is
            if (sourceValue.includes('{{config.')) {
              mappedParams[paramName] = sourceValue
              logger.debug(`  → Mapped ${artifactField} → ${paramName} (config reference)`)
            } else if (!sourceValue.includes('!')) {
              // Add A1 notation if not present (full sheet range)
              mappedParams[paramName] = `${sourceValue}!A:Z`
              logger.debug(`  → Transformed ${artifactField} → ${paramName} with A1 notation`)
            } else {
              // Already has A1 notation, use as-is
              mappedParams[paramName] = sourceValue
              logger.debug(`  → Mapped ${artifactField} → ${paramName} (has A1 notation)`)
            }
          } else {
            // For other parameters, just copy the value
            mappedParams[paramName] = sourceValue
            logger.debug(`  → Mapped ${artifactField} → ${paramName}`)
          }
        }
      }
    }
  ...
  /**
   * OPTIMIZATION: Find the original content source for a deliver operation output
   *
   * This is a plugin-agnostic optimization that detects the pattern:
   *   Step A (fetch/download) → content_var
   *   Step B (deliver/upload) uses content_var.content → file_var
   *   Step C (extract) wants to use file_var
   *
   * In this case, Step C should use content_var instead of file_var to avoid re-downloading.
   *
   * @param varRef Variable reference that might be a deliver output (e.g., "drive_file")
   * @param ctx Conversion context
   * @returns Original content source variable, or null if not applicable
   */
  private findOriginalContentSource(varRef: string, ctx: ConversionContext): string | null {
    // Find the node that produces this variable
    let deliverNode: ExecutionNode | undefined
    for (const [_nodeId, node] of ctx.nodes.entries()) {
      // Check if this is an operation node that outputs our variable
      if (node.type === 'operation' && node.outputs) {
        const hasOutput = node.outputs.some(out => out.variable === varRef)
        if (hasOutput && node.operation?.operation_type === 'deliver') {
          deliverNode = node
          break
        }
      }
    ...
    if (!deliverNode || !deliverNode.operation || !deliverNode.operation.deliver) {
      return null
    }

    // Look through ALL config parameters to find any that reference another variable's content field
    // This is plugin-agnostic - works for any plugin that passes content via a parameter
    const config = deliverNode.operation.deliver.config
    if (!config) {
      return null
    }

    for (const [_paramName, paramValue] of Object.entries(config)) {
      if (typeof paramValue === 'string') {
        // Check if this parameter references another variable's content
        // Patterns: {{var.content}}, {{var.data}}, {{var.file_content}}, {{var.body}}
        const contentMatch = paramValue.match(/\{\{([a-zA-Z_][a-zA-Z0-9_]*)\.(content|data|file_content|body)\}\}/)
        if (contentMatch) {
          const sourceVar = contentMatch[1]
          logger.debug({
            deliverOutput: varRef,
            sourceVar,
            paramValue
          }, '[IntentToIRConverter] Found content source for deliver operation')
          return sourceVar
        }
      }
    }
