...
export class ExecutionGraphCompiler {
  ...
  private async compileOperationNode(
    ...
    switch (operation.operation_type) {
      ...
      case 'ai':
        ...
        break
      case 'deliver':
        workflowStep = this.compileDeliverOperation(stepId, node.id, operation, resolvedConfig, ctx, steps)
        break
      case 'file_op':
  ...
  private compileFetchOperation(
    ...
    return {
      ...
      action: fetch.action,  // WorkflowParser expects 'action' not 'operation'
      config
    }
  }
  ...
  private async compileTransformOperation(
    ...
    if (transformConfig && transformConfig.type) {
      if (transformConfig.type === 'filter') {
        ...
        // IR field: rules.field → DSL field: field
        // The field parameter specifies which nested array to extract from each item
        // Example: flatten emails to extract attachments → field: "attachments"
        if (transformConfig.rules && transformConfig.rules.field) {
          transformedConfig.field = transformConfig.rules.field
          this.log(ctx, `  → Compiled rules.field to field: "${transformConfig.rules.field}"`)
        } else if (transformConfig.custom_code) {
          // FALLBACK: Try to extract field name from description
          // Pattern: "Extract <field> array from <source>"
          const match = transformConfig.custom_code.match(/extract\s+(\w+)\s+(?:array|field|list)/i)
          if (match && match[1]) {
            const extractedField = match[1]
            transformedConfig.field = extractedField
            this.log(ctx, `  → Inferred flatten field from description: "${extractedField}"`)
          }
        }
  ...
  private compileDeliverOperation(
    ...
    resolvedConfig: any,
    ctx: CompilerContext,
    stepsContext?: WorkflowStep[]
  ): WorkflowStep {
    const deliver = operation.deliver!

    ctx.pluginsUsed.add(deliver.plugin_key)

    // OPTIMIZATION: For document extraction operations, check if input references
    // an uploaded file and use the pre-upload content source instead
    let configToUse = resolvedConfig.deliver?.config || {}

    // Check if this is document extraction by plugin name OR if config has file_content/file_url + fields
    // (plugin-agnostic check based on parameter pattern)
    const isExtraction = deliver.plugin_key === 'document-extractor' ||
                         (configToUse.fields && (configToUse.file_content || configToUse.file_url))

    if (isExtraction && stepsContext) {
      this.logger.debug({
        pluginKey: deliver.plugin_key,
        hasFields: !!configToUse.fields,
        hasFileParam: !!(configToUse.file_content || configToUse.file_url)
      }, 'Running extraction optimization')
      configToUse = this.optimizeExtractionConfig(configToUse, stepsContext)
    }
    ...
    return {
      ...
      action: deliver.action,  // WorkflowParser expects 'action' not 'operation'
      config
    }
  }
  ...
  private compileFileOperation(
    ...
    return {
      ...
      action: fileOp.action,  // WorkflowParser expects 'action' not 'operation'
    ...
    for (let i = 0; i < workflow.length; i++) {
      ...
      if (step.output_variable && step.plugin && step.operation) {
        ...
        if (detection.is2DArray) {
          ...
          // CRITICAL FIX: Update all subsequent steps in workflow array BEFORE they get pushed to normalized
          // This ensures the updates are preserved when steps are added to normalized array
  ...
  private updateVariableReferences(
    ...
    // CRITICAL FIX: Also replace direct references for filter/map/reduce operations
    // that need object access (e.g., filter with condition.field = "item.Stage")
    // Match both wrapped {{varName}} and unwrapped varName formats
  ...
  /**
   * Check if a step needs object access (has field references like "item.Stage")
   * These steps should use normalized object arrays instead of 2D arrays
   */
  private stepNeedsObjectAccess(step: WorkflowStep): boolean {
    // Check if it's a filter/map/reduce operation
    const operation = (step as any).operation
    if (!['filter', 'map', 'reduce'].includes(operation)) {
      return false
    }

    // Check if condition/config has field references like "item.field"
    ...
    if (config.condition) {
      ...
      // Check for field access in condition (e.g., "field": "item.Stage")
      if (condition.field && typeof condition.field === 'string' && condition.field.startsWith('item.')) {
        return true
      }
      // Check for variable access in condition (e.g., "variable": "item.Stage")
      if (condition.variable && typeof condition.variable === 'string' && condition.variable.startsWith('item.')) {
        return true
      }
    }
    ...
    if (operation === 'map' && config.transform) {
      // Map operations that reference item.field need objects
  ...
  private fixScatterGatherOutput(step: any, ctx: CompilerContext): any {
    ...
    // If no output_variable, ensure gather.operation is still present but minimal
    // (PILOT DSL validator requires gather.operation for all scatter_gather steps)
    if (!fixed.output_variable && fixed.gather) {
      // Keep gather with operation only, no output-related fields
      if (fixed.gather.operation === 'flatten') {
        // For side-effect loops (no collection), use 'collect' as a no-op
        this.log(ctx, `  → Scatter_gather ${fixed.step_id} has no output, using gather.operation='collect' (no-op)`)
    ...
    if (step.config && this.pluginManager) {
      ...
      if (pluginSchema?.parameters?.properties) {
        ...
        step.config = await this.normalizeActionConfigWithSchema(
          step.config,
          pluginSchema.parameters, // Pass full parameters object (includes properties and required array)
          variables,
          ctx
        )
      } else {
  ...
  private async normalizeActionConfigWithSchema(
    ...
    const paramSchema = parameterSchema.properties || parameterSchema // fallback to parameterSchema if no properties

    // NEW: Check if parameters are already fully mapped from binding phase
    const hasAllRequired = requiredParams.length === 0 || requiredParams.every((param) => param in config)

    if (hasAllRequired) {
      this.log(
        ctx,
        '✅ All required parameters present from binding phase, applying schema-based normalization'
      )
    } else {
      this.log(
        ctx,
        `⚠️  Missing required parameters, falling back to Phase 4 normalization (${requiredParams.length - Object.keys(config).length} missing)`
      )
    }
    ...
    if (ctx.workflowConfig && parameterSchema) {
      ...
      for (const [paramName, paramDef] of Object.entries(parameterSchema as Record<string, any>)) {
        ...
        const fuzzyMatch = this.findBestConfigMatch(searchKey, ctx.workflowConfig, 0.5, ctx)
  ...
  /**
   * OPTIMIZATION: For document extraction config, check if file_content/file_url references
   * a deliver operation output. If that deliver operation used content from another variable,
   * use that original content source instead.
   *
   * This is completely plugin-agnostic - it just follows data flow patterns in the config.
   *
   * Pattern:
   *   step A: → source_var (has content field)
   *   step B: uses {{source_var.content}} in config → file_var
   *   step C: extract uses {{file_var}} ❌
   *
   * Optimized:
   *   step C: extract uses {{source_var}} ✅
   */
  private optimizeExtractionConfig(config: Record<string, any>, compiledSteps: WorkflowStep[]): Record<string, any> {
    // Check if file_content or file_url parameter references a variable
    const fileParam = config.file_content || config.file_url
    if (!fileParam || typeof fileParam !== 'string') {
      this.logger.debug({ config }, 'Optimization: No file parameter found')
      return config
    }

    // Extract variable reference (e.g., "{{drive_file}}" → "drive_file")
    const varMatch = fileParam.match(/\{\{([a-zA-Z_][a-zA-Z0-9_]*)\}\}/)
    if (!varMatch) {
      this.logger.debug({ fileParam }, 'Optimization: File parameter is not a simple variable reference')
      return config
    }
    ...
    this.logger.debug({
      varName,
      compiledStepsCount: compiledSteps.length,
      stepOutputs: compiledSteps.map(s => s.output_variable).filter(Boolean)
    }, 'Optimization: Looking for variable source')

    // Look through compiled steps to find where this variable was created
    for (const step of compiledSteps) {
      if (step.output_variable === varName) {
        this.logger.debug({
          foundStep: step.step_id,
          hasConfig: !!step.config,
          configKeys: step.config ? Object.keys(step.config) : []
        }, 'Optimization: Found step that creates this variable')
      ...
      if (step.output_variable === varName && step.config) {
        // Check ALL config parameters to see if any reference another variable's content field
        // This is plugin-agnostic - works for any action that passes content via parameters
        for (const [_paramName, paramValue] of Object.entries(step.config)) {
          if (typeof paramValue === 'string') {
            // Look for patterns like {{var.content}}, {{var.data}}, {{var.file_content}}, {{var.body}}
            const contentMatch = paramValue.match(/\{\{([a-zA-Z_][a-zA-Z0-9_]*)\.(content|data|file_content|body)\}\}/)
            if (contentMatch) {
              const sourceVar = contentMatch[1]
              this.logger.info({
                extractionStep: 'optimizing',
                originalInput: varName,
                optimizedInput: sourceVar,
                intermediateStep: step.step_id,
                pattern: paramValue
              }, '✅ OPTIMIZATION: Using original content source for extraction (avoids re-download)')

              // Return optimized config with source variable
              const optimizedConfig = { ...config }
              if (config.file_content) {
                optimizedConfig.file_content = `{{${sourceVar}}}`
              } else if (config.file_url) {
                optimizedConfig.file_url = `{{${sourceVar}}}`
              }
