...
**Before generating ANY node configuration, you MUST validate data flow using these protocols. This prevents bugs where variables reference non-existent fields, wrong scopes, or AI operations include metadata fields.**
...
- Input MUST be array (except `flatten` which handles nested arrays)
- Output type changes based on transform:
  - Most transforms: array
  - `reduce`: **single value (NOT array!)**
  - `group_by`: object with arrays per group

**STEP 3: For `deduplicate` - MUST specify unique identifier**
- Requires `unique_field` or comparison logic
- Example: deduplicate by `"id"`, `"email"`, or composite key `["email", "date"]`

**STEP 4: Declare output variable with correct type**
