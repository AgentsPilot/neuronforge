...
export interface PluginActionInfo {
  ...
  capability: string
  description: string
  param_hints?: string[]  // Parameter names that this action expects (for LLM guidance)
  required_params?: string[]  // Required parameter names (for LLM guidance)
  config_params?: Array<{ param: string; config_key: string }>  // Parameters that map to workflow config
}
    ...
    for (const [pluginKey, actionablePlugin] of Object.entries(filteredPlugins)) {
      ...
      for (const [actionName, action] of Object.entries(plugin.actions)) {
        ...
        if (action.domain && action.capability) {
          ...
          const configParams: Array<{ param: string; config_key: string }> = []

          if (action.parameters?.properties) {
            paramHints.push(...Object.keys(action.parameters.properties))

            // Extract required parameters
            if (action.parameters.required) {
              requiredParams.push(...action.parameters.required)
            }

            // Extract config-bound parameters (x-context-binding)
            for (const [paramName, paramDef] of Object.entries(action.parameters.properties)) {
              ...
              if (contextBinding?.key) {
                configParams.push({
                  param: paramName,
                  config_key: contextBinding.key
                })
              }
            }
          }
          ...
          actions.push({
            ...
            param_hints: paramHints.length > 0 ? paramHints : undefined,
            required_params: requiredParams.length > 0 ? requiredParams : undefined,
            config_params: configParams.length > 0 ? configParams : undefined,
          })
        }
      }
