...
- Use **query** ONLY for natural language or semantic search strings
  - The query field should be PLUGIN-AGNOSTIC (works across different providers)
  - Use natural language: "unread emails with attachments", "open tickets", "recent documents"
  - DO NOT use provider-specific syntax like "is:unread has:attachment" (Gmail-specific) or "IsRead eq false" (Outlook-specific)
  - The binding phase will translate your semantic query to the bound plugin's native syntax
  - Example: "unread messages with attachments" works for Gmail, Outlook, Slack, etc.
- Use **filters** array for structured field-based filtering (preferred over query when possible)
  - Filters are more deterministic and plugin-agnostic than query strings
  - Example: filters: [{"field": "read_status", "op": "eq", "value": "unread"}]
...
**Decision tree:**
1. Can this be expressed as structured filters? → Use filters array (PREFERRED - most plugin-agnostic)
2. Is this a natural language search? → Use query with semantic/natural language (AVOID provider-specific syntax)
3. Does user specify resource identifiers or locations? → Use payload (object with ValueRefs)
4. Does this need data from a prior step? → Use inputs (array of RefNames)
...
- Download outputs: raw bytes/content (suitable for: content processing, extraction, analysis)
- Upload outputs: storage references (URLs, IDs, links) - suitable for: sharing, linking, metadata operations)

**Rule for extraction/processing operations:**
Check what format the extraction/processing capability requires by looking at its domain and capability:

1. **If extraction requires file bytes/content** (most document extraction operations):
...
2. **If extraction can work with URL/link** (some web-based extraction services):
   - Use the upload output (which has URLs)
   - Example: some web scraping or remote processing services

**When in doubt:** Default to using the output that contains the actual content/bytes for extraction operations, not just metadata/references. Storage outputs (from upload) typically lose the raw content.
...
function buildVocabularyInjection(vocabulary: PluginVocabulary): string {
  ...
  for (const plugin of vocabulary.plugins.slice(0, 10)) { // Show first 10 plugins
    ...
    // Extract unique config parameter names from actions
    const configParams = new Set<string>()
    for (const action of plugin.actions) {
      if (action.config_params) {
        for (const cp of action.config_params) {
          configParams.add(cp.config_key)
        }
      }
    }

    if (configParams.size > 0) {
      sections.push(`  Config Parameters: ${Array.from(configParams).sort().join(', ')}`)
    }
  }
