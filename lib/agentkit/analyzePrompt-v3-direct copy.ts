...
export interface AnalyzedWorkflowStep {
  ...
  // Conditional execution (for steps that should only run if condition is met)
  executeIf?: {
    field: string;
    operator: string;
    value: any;
  ...
  items?: string;  // Variable reference to array to iterate over (e.g., "{{step1.data.emails}}")
  ...
  operation?: 'join' | 'filter' | 'aggregate' | 'map' | 'sort';  // Transform operation type
  config?: Record<string, any>;  // Transform configuration (join keys, filter conditions, etc.)
}
...
[
  ...
  {"id": "step2", "operation": "Process each email", "type": "loop", "items": "{{step1.data.emails}}", "steps": [{"id": "step2_process", "operation": "Summarize email", "type": "ai_processing", "plugin": "ai_processing", "plugin_action": "process", "params": {"prompt": "Summarize: {{item.subject}}"}, "dependencies": []}], "dependencies": ["step1"]}
]
...
**TRANSFORM STEP FORMAT (FILTER):**
{
  ...
  "dependencies": ["step1"],
  "reasoning": "Only process VIP customers"
}

**EXAMPLE - Multi-Source Analysis:**
[
  {"id": "step1", "operation": "Get Stripe subscriptions", "type": "plugin_action", "plugin": "stripe", "plugin_action": "listSubscriptions", "dependencies": []},
  {"id": "step2", "operation": "Get HubSpot customers", "type": "plugin_action", "plugin": "hubspot", "plugin_action": "listCustomers", "dependencies": []},
  {"id": "step3", "operation": "Join data by email", "type": "transform", "operation": "join", "config": {"left": "{{step1.data}}", "right": "{{step2.data}}", "on": ["customerEmail"]}, "dependencies": ["step1", "step2"]},
  {"id": "step4", "operation": "Filter high-value customers", "type": "transform", "operation": "filter", "config": {"condition": {"field": "total_revenue", "operator": ">", "value": 1000}}, "dependencies": ["step3"]}
]
