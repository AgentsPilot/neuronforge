...
export class ExecutionGraphValidator {
     ...
     */
    findReachableNodes(graph, startId) {
    ...
    identifyLoopBodyNodes(graph) {
        ...
        for (const [loopId, loopNode] of loopNodes) {
            if (!loopNode.loop?.body_start)
                continue;
            const visited = new Set();
            const queue = [loopNode.loop.body_start];
            while (queue.length > 0) {
     ...
     */
    validateVariableDeclarations(graph) {
        if (!graph.variables)
            return;
    ...
    validateDataFlow(graph) {
        ...
        const availableVariables = new Set(this.declaredVariables);
     ...
     */
    extractVariableName(variableRef) {
     ...
     */
    extractConditionVariables(condition) {
        const vars = [];
    ...
    validateOperationNode(nodeId, node) {
        if (!node.operation) {
    ...
    validateChoiceNode(nodeId, node) {
        if (!node.choice) {
    ...
    validateParallelNode(nodeId, node) {
        if (!node.parallel) {
    ...
    addError(category, nodeId, message, suggestion) {
        this.errors.push({
            type: 'error',
            category,
            node_id: nodeId,
    ...
    addWarning(category, nodeId, message, suggestion) {
        this.warnings.push({
            type: 'warning',
            category,
            node_id: nodeId,
