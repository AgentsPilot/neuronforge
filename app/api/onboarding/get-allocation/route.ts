...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: subscription, error: subError } = await supabaseAdmin
      ...
      .select('balance, storage_quota_mb, storage_used_mb, executions_quota, executions_used, free_tier_granted_at, free_tier_expires_at, free_tier_initial_amount, account_frozen')
      .eq('user_id', user.id)
      .single();
