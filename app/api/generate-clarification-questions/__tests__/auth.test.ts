/**
 * Identity lock for POST /api/generate-clarification-questions (F13).
 *
 * The route was unauthenticated and took the user id from the request body, then
 * handed it to PluginManagerV2.getUserActionablePlugins and returned the resulting
 * keys as `connectedPlugins` — i.e. an anonymous caller could enumerate any user id's
 * connected plugins.
 *
...
import { POST } from '@/app/api/generate-clarification-questions/route';
...
const post = (body: unknown, headers: Record<string, string> = {}) =>
  new NextRequest('http://localhost/api/generate-clarification-questions', {
    method: 'POST',
    headers: { 'content-type': 'application/json', ...headers },
    body: JSON.stringify(body),
  });

// No `connectedPlugins` and no `analysis.requiredServices`, so the route falls through
// to the user-plugin lookup — the branch that leaked.
const BODY = { prompt: 'Email me a summary every morning', agentName: 'Summariser' };
...
describe('POST /api/generate-clarification-questions — identity is server-derived', () => {
