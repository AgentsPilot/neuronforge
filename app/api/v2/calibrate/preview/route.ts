...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    requestLogger.warn({ agentId, userId: user.id }, 'DEPRECATED: preview endpoint called - use /api/v2/calibrate/batch instead');
    ...
    requestLogger.error({ err: error }, 'Preview request failed');
    return NextResponse.json(
      {
        success: false,
