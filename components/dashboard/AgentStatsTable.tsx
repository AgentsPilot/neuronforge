    ...
    <div className="h-full flex flex-col">
      ...
      <div className="p-6 flex-1">
        {stats.length === 0 ? (
          ...
          /* Modern Cards View - Matching AgentList Design */
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {stats.map((stat) => {
              ...
              const isActive = activityStatus.status === 'active'

              return (
                <div key={stat.agent_id} className="group relative h-full">
                  {isActive && (
                    <div className="absolute -inset-0.5 bg-gradient-to-r from-emerald-400 via-green-400 to-teal-400 rounded-lg blur opacity-25 group-hover:opacity-40 transition duration-1000 animate-pulse" />
                  )}

                  <div className="relative bg-white/95 backdrop-blur-sm rounded-lg border border-gray-200/80 hover:border-emerald-300/60 transition-all duration-300 overflow-hidden hover:shadow-lg hover:-translate-y-1 h-full flex flex-col">

                    {/* Top gradient line */}
                    <div className="h-0.5 bg-gradient-to-r from-emerald-500 via-green-500 to-teal-500 opacity-60 group-hover:opacity-100 transition-opacity duration-300" />
                    ...
                    <div className="relative p-4">
                      ...
                      <div className="flex items-start gap-3 mb-3">
                        <div className="relative flex-shrink-0">
                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-sm transition-all duration-300 group-hover:scale-105 ${
                            isActive
                              ? 'bg-gradient-to-br from-emerald-500 via-green-500 to-teal-500 animate-pulse'
                          ...
                          {!isActive && (
                            <div className={`absolute -bottom-1 -right-1 w-3 h-3 ${
                              activityStatus.status === 'recent' ? 'bg-blue-500' :
                        ...
                        <div className="min-w-0 flex-1">
                          <h3 className="text-base font-semibold text-gray-900 mb-1.5 group-hover:text-emerald-700 transition-colors truncate">
                            {stat.agent_name}
                          </h3>
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            <div className={`inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium ${
                              activityStatus.status === 'active' ? 'bg-green-50 text-green-700' :
                              activityStatus.status === 'recent' ? 'bg-blue-50 text-blue-700' :
                              activityStatus.status === 'idle' ? 'bg-yellow-50 text-yellow-700' :
                              'bg-gray-50 text-gray-700'
                            }`}>
                              <Activity className="w-3 h-3" />
                              {activityStatus.label}
                            </div>
                            <div className={`px-2 py-1 rounded text-xs font-medium ${getSuccessRateColor(successRate)}`}>
                              {successRate}%
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="space-y-2 mb-3">
                        <div className="flex items-center justify-between p-2 bg-gray-50/50 rounded-lg">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-xs text-gray-600 font-medium">Total Runs</span>
                          </div>
                          <span className="font-semibold text-gray-900 text-sm">{stat.run_count.toLocaleString()}</span>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <div className="flex items-center justify-between p-2 bg-green-50/50 rounded-lg">
                            ...
                            <span className="font-semibold text-green-700 text-sm">{stat.success_count}</span>
                          </div>
                          ...
                          <div className="flex items-center justify-between p-2 bg-red-50/50 rounded-lg">
                            ...
                            <span className="font-semibold text-red-700 text-sm">{(stat.run_count - stat.success_count)}</span>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between p-2 bg-blue-50/50 rounded-lg">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-xs text-blue-700 font-medium">Last Run</span>
                          </div>
                          <span className="text-xs text-blue-700 font-semibold">{formatTimeAgo(stat.last_run_at)}</span>
                        </div>
                      </div>
                      ...
                      <div className="mt-3">
                        <div className="flex items-center justify-between mb-1.5">
                          ...
                          <span className="text-xs text-gray-900 font-semibold">{successRate}%</span>
                        </div>
