-- Website Builder Tables (FIXED ORDER)
-- Purpose: Store website pages, blocks, and templates for the no-code website builder
-- Last Updated: 2026-07-21
-- FIX: Create website_templates BEFORE website_pages to avoid FK constraint error

-- =============================================
-- WEBSITE TEMPLATES TABLE (CREATE FIRST)
-- =============================================
...
CREATE TABLE IF NOT EXISTS website_pages (
  ...
  -- Template and theme
  template_id UUID REFERENCES website_templates(id), -- NOW THIS FK WORKS
...
-- Policies for website_templates (read-only for users, writable by admin)
CREATE POLICY "Anyone can view templates"
  ON website_templates FOR SELECT
  USING (true);

-- Admin-only insert/update/delete policies
-- Note: These will be set up via service role key or admin panel
...
CREATE INDEX idx_website_pages_page_type ON website_pages(page_type);
...
CREATE INDEX idx_website_blocks_type ON website_blocks(block_type);
...
CREATE TRIGGER update_website_templates_timestamp
...
-- COMMENTS
-- =============================================
COMMENT ON TABLE website_templates IS 'Pre-built website templates by vertical (therapist, coach, etc.)';
COMMENT ON TABLE website_pages IS 'User-created website pages (homepage, landing pages, etc.)';
COMMENT ON TABLE website_blocks IS 'Block-based content for pages (hero, services, testimonials, etc.)';

COMMENT ON COLUMN website_pages.template_id IS 'Reference to the template this page was created from';
COMMENT ON COLUMN website_pages.slug IS 'URL path for this page (e.g., /adhd-course)';
COMMENT ON COLUMN website_pages.theme IS 'Page-specific theme overrides (colors, fonts, etc.)';

COMMENT ON COLUMN website_blocks.block_type IS 'Type of block (hero, services, cta, testimonials, etc.)';
COMMENT ON COLUMN website_blocks.content IS 'Block-specific content (JSONB for flexibility)';
COMMENT ON COLUMN website_blocks.position IS 'Order of block on page (0 = first, 1 = second, etc.)';
