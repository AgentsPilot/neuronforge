...
--
-- The facts are taken from the tenant's own history so the derived dates are
...
-- champions is the launch operation's job, at enforcement switch-on, not this
-- migration's.
...
--
-- This fills NULL facts from the tenant's own history. It can only write where
...
-- `champion`. That is the launch operation's job at enforcement switch-on, which
